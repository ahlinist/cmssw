...
{
  l1ExtraMu_ = iConfig.getParameter<InputTag>("l1ExtraMu");
...
BsToJpsiPhiAnalysis::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  bsRootTree_->eventNumber_ = iEvent.id().event();
 ...
 const  edm::TriggerNames & triggerNames_ = iEvent.triggerNames(*hltresults);  
  ...
  for (int itrig = 0; itrig != ntrigs; ++itrig){
    ...
    if (trigName=="HLT_Mu0_TkMu0_OST_Jpsi") bsRootTree_->triggerbit_HLTmu3_ = hltresults->accept(itrig);
    if (trigName=="HLT_Mu3_TkMu0_OST_Jpsi") bsRootTree_->triggerbit_HLTmu5_ = hltresults->accept(itrig); 
    if (trigName=="HLT_Mu5_TkMu0_OST_Jpsi") bsRootTree_->triggerbit_HLTmu9_ = hltresults->accept(itrig);
    if (trigName=="OpenHLT_DoubleMu0_Quarkonium")  bsRootTree_->triggerbit_HLTdoubleIsoMu3_   = hltresults->accept(itrig);
    if (trigName=="OpenHLT_DoubleMu0_Quarkonium_LS")     bsRootTree_->triggerbit_HLTdoubleMu3_      = hltresults->accept(itrig);
    if (trigName=="HLT_Mu5_Track0_Jpsi")     bsRootTree_->triggerbit_HLTdoubleMu0_      = hltresults->accept(itrig);
    if (trigName=="HLT_Mu3_Track3_Jpsi")bsRootTree_->triggerbit_HLTdoubleMu3_JPsi_ = hltresults->accept(itrig);
    if (trigName=="OpenHLT_Mu3_Track5_Jpsi ")bsRootTree_->triggerbit_HLTL1DoubleMuOpen_ = hltresults->accept(itrig);
    if (trigName=="HLT_Mu5_L2Mu0 ")bsRootTree_->triggerbit_HLTL1MuOpen_ = hltresults->accept(itrig);//DoublMuOpen
  }
  ...
  //if(saveL1DoubleMuOpenTriggeredOnly_ == true)
    ...
    //if(bsRootTree_->triggerbit_HLTdoubleMu0_!= 1 ) return;//test double Mu0 USED for lot of luminosity

  //////////////////////////////////////
  // match reco/L1-HLT fired events
  //////////////////////////////////////
  ...
  iEvent.getByLabel(l1ExtraMu_,L1Muons);
  
  edm::InputTag tag("hltTriggerSummaryAOD","","HLT");
  // edm::InputTag tag("hltTriggerSummaryAOD","","REDIGI36X");
  edm::Handle<trigger::TriggerEvent> trgEvent;
  iEvent.getByLabel(tag,trgEvent);
  ...
  bool passed2mu3 = false;
   for ( trigger::size_type i = 0; i != numFilterObjects; ++i  ) {
     ...
     // if (TString(module) == "hltDiMuonL3PreFiltered0::REDIGI36X" )
     if (TString(module) == "hltDiMuonL3PreFiltered0::HLT" )
       passed2mu0 = true;
     if (TString(module) == "hltDiMuonL3PreFiltered::HLT" )
       //if (TString(module) == "hltDiMuonL3PreFiltered::REDIGI36X" )
       passed2mu3 = true;  
   }
   ...
   InputTag tag2mu3 = InputTag("hltDiMuonL3PreFiltered::HLT");
    //InputTag tag2mu0 = InputTag("hltDiMuonL3PreFiltered0::REDIGI36X");
    //InputTag tag2mu3 = InputTag("hltDiMuonL3PreFiltered::REDIGI36X");
   if ( passed2mu0 ) {
   ...
   int match2mu31=0;
   int match2mu32=0;
   double delta2mu0Eta1=0;
   double delta2mu0Phi1=0;
   double delta2mu0Eta2=0;
   double delta2mu0Phi2=0;
   double delta2mu3Eta1=0;
   double delta2mu3Phi1=0;
   double delta2mu3Eta2=0;
   double delta2mu3Phi2=0;
   double deltaR2mu01=0;
   double deltaR2mu02=0;
   double deltaR2mu31=0;
   double deltaR2mu32=0;
  ...
  //  /////////////////////////////////////////
   ...
   for(size_t i=0; i < allmuons->size(); ++i){
     ...
     for (size_t j=i+1; j < allmuons->size(); ++j){
       ...
       if (allmuons->size() < 0)
	 cout << "muon list size: " << allmuons->size() << endl;      
	 ...
	 bsRootTree_->MuonType_ = 3;
       ...
       bsRootTree_->MuRecoChg1_[j] = mu2.charge();
       ...
       //      int match1 = matcher.match(mu1,*L1Muons,deltaRs,deltaPhis, propagated);
       //      int match2 = matcher.match(mu2,*L1Muons,deltaRs,deltaPhis, propagated);
       //
       //      /*      
       ...
       //      }
       //      if (match != -1) {
       //	std::cout << "Matched! L1 pt code is " << (*L1Muons)[match].pt() << " , deltaPhi = " << deltaPhis << std::endl;
       //      }
       //      */
       //      /* 
       //      if (match1 != -1) {
       //	if (matchL11 > 0) continue;
       //	matchL11++;
      //      }
      //      
      //      bsRootTree_->matchL11_ = matchL11;
      //      
      //      if (match2 != -1) {
      //	if (matchL12 > 0) continue;
      //	matchL12++;
      //      }
      //      bsRootTree_->matchL12_ = matchL12;
      //      */
      //      // matching hlt 2mu0
      //
      //      for ( size_t candNum = 0; candNum < hlt2mu0Cands.size(); candNum++ ) {
      //	
      //	if (match2mu01 > 0) continue;
      //	double eta= hlt2mu0Cands[candNum].eta();
      //	double phi= hlt2mu0Cands[candNum].phi();  
      //	//	cout << "HLT 2mu0 eta,phi = " << eta << "," << phi << endl;
      //	
      //	delta2mu0Eta1 = mu1.eta()-eta;
      //	delta2mu0Phi1 = deltaPhi(mu1.phi(),phi);
      //	
      //	deltaR2mu01 = deltaR(mu1.eta(),mu1.phi(),eta,phi);
      //
      //	if (match2mu01 > 0) continue;
      //	if (deltaR2mu01<0.02) match2mu01++;
      //	
      //      }
      //
      //      bsRootTree_->match2mu01_ = match2mu01;
      //
      //      for ( size_t candNum = 0; candNum < hlt2mu0Cands.size(); candNum++ ) {
      //
      //	if (match2mu02 > 0) continue;
      //	double eta= hlt2mu0Cands[candNum].eta();
      //	double phi= hlt2mu0Cands[candNum].phi();  
      //	//	cout << "HLT 2mu0 eta,phi = " << eta << "," << phi << endl;
      //
      //	delta2mu0Eta2 = mu2.eta()-eta;
      //	delta2mu0Phi2 = deltaPhi(mu2.phi(),phi);
      //
      //        deltaR2mu02 = deltaR(mu2.eta(),mu2.phi(),eta,phi);
      //
      //	if (match2mu02 > 0) continue;
      //	if (deltaR2mu02<0.02) match2mu02++;
      //	
      //      }
      //
      //      bsRootTree_->match2mu02_ = match2mu02;
      //
      //      // matching hlt 2mu3
      //
      //      for ( size_t candNum = 0; candNum < hlt2mu3Cands.size(); candNum++ ) {
      //
      //	if (match2mu31 > 0) continue;
      //	double eta= hlt2mu3Cands[candNum].eta();
      //	double phi= hlt2mu3Cands[candNum].phi();  
      //	//	cout << "HLT 2mu3 eta,phi = " << eta << "," << phi << endl;
      //
      //	delta2mu3Eta1 = mu1.eta()-eta;
      //	delta2mu3Phi1 = deltaPhi(mu1.phi(),phi);
      //
      //        deltaR2mu31 = deltaR(mu1.eta(),mu1.phi(),eta,phi);
      //
      //	if (match2mu31 > 0) continue;
      //        if (deltaR2mu31<0.02) match2mu31++;
      //
      //      }
      //
      //      bsRootTree_->match2mu31_ = match2mu31;
      //
      //      for ( size_t candNum = 0; candNum < hlt2mu3Cands.size(); candNum++ ) {
      //
      //	if (match2mu32 > 0) continue;
      //	double eta= hlt2mu3Cands[candNum].eta();
      //	double phi= hlt2mu3Cands[candNum].phi();  
      //	//	cout << "HLT 2mu3 eta,phi = " << eta << "," << phi << endl;
      //
      //        delta2mu3Eta2 = mu2.eta()-eta;
      //        delta2mu3Phi2 = deltaPhi(mu2.phi(),phi);
      //
      //        deltaR2mu32 = deltaR(mu2.eta(),mu2.phi(),eta,phi);
      //
      //if (match2mu32 > 0) continue;
      //if (deltaR2mu32<0.02) match2mu32++;
      //
      // }
      //
      // bsRootTree_->match2mu32_ = match2mu32;
      
      ///////////////////////////////////////
      // end L1/HLT-reco matching
      ///////////////////////////////////////

      // requirements on muons
	...
	std::cout<<"****** MUONS HAVE VALID VERTEX FIT"<< std::endl;
      ...
      bsRootTree_->JpsiP_alone_ = Jpsi.p();	 
      bsRootTree_->JpsiMu1P_alone_ = mu1.p();	    
      bsRootTree_->JpsiMu2P_alone_ = mu2.p();	
      ...
      for (int i=0; i<pp1.numberOfHits(); i++) {
	uint32_t hit = pp1.getHitPattern(i);
	...
	if (pp1.validHitFilter(hit) && pp1.pixelEndcapHitFilter(hit)) pixhits1++;
      ...
      for (int i=0; i<pp2.numberOfHits(); i++) {
	uint32_t hit = pp2.getHitPattern(i);
	...
	for (size_t l=k+1; l< allTracks->size(); ++l){
	  ...
	  if (vtxprob_Bs > minVtxP){
	    ...
	    // ctau 3D
	    bsRootTree_->BsCt3D_ = b_par[6]*((bVertex->position().x()-PVx)*Bsvec.x()+
					     +(bVertex->position().y()-PVy)*Bsvec.y()+
					     +(bVertex->position().z()-PVz)*Bsvec.z())/
	      (Bsvec.x()*Bsvec.x()+Bsvec.y()*Bsvec.y()+Bsvec.z()*Bsvec.z());
	    // ctau 2d
	    bsRootTree_->BsCt2D_ = b_par[6]*((bVertex->position().x()-PVx)*Bsvec.x()+
					     +(bVertex->position().y()-PVy)*Bsvec.y())/
	  ...
	  for(int k=0; k< numBsDaughtersDaughters; k++){
		...
		pkminus.SetXYZM(Km->px(),Km->py(),Km->pz(),Km->mass());
	      ...
	      */
	      ...
	      bsRootTree_->costhetaMC_[arrayIndex] = p3_JPsi_muplus.Unit() * z;
	      double cos_phi = p3_JPsi_muplus.Unit() * x / TMath::Sqrt(1 - angle_costheta*angle_costheta);
	      double sin_phi = p3_JPsi_muplus.Unit() * y / TMath::Sqrt(1 - angle_costheta*angle_costheta);
	      angle_phi = TMath::ACos(cos_phi);
	      if (sin_phi < 0){
		angle_phi =  -angle_phi;
	      ...
	      bsRootTree_->phiMC_[arrayIndex] = angle_phi;
