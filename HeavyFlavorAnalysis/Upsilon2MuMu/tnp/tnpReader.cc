...
// ----------------------------------------------------------------------
tnpReader::tnpReader(TChain *tree, TString evtClassName): treeReaderTNP(tree, evtClassName) {
  cout << "--> tnpReader> This is the start ..." << endl;
}
// ----------------------------------------------------------------------
tnpReader::~tnpReader() {
  cout << "--> tnpReader> This is the end ..." << endl;
}

// ----------------------------------------------------------------------
void tnpReader::startAnalysis() {
  cout << "--> tnpReader> startAnalysis: ..." << endl;
}

void tnpReader::eventProcessing() {
  ...
  int npt = 12;
  double pTbin[] = {0., 2., 2.5, 3., 3.5, 4., 4.5, 5., 5.5, 6., 7., 9., 20.};		
  ...
  cout << "----------------------------------------------------------------------" << endl;
  cout << "tnpReader> Found " << fpEvt->nGenCands() << " gen cands in event" << endl;
  cout << "tnpReader> Found " << fpEvt->nSigTracks() << " sig tracks in event" << endl;
  cout << "tnpReader> Found " << fpEvt->nRecTracks() << " rec tracks in event" << endl;
  cout << "tnpReader> Found " << fpEvt->nCands() << " cands in event" << endl;
  cout << "----------------------------------------------------------------------" << endl;
  ...
  for (int it = 0; it < fpEvt->nCands(); ++it) {
	...
	if ( pMom1->fID == RESTYPE && pMom2->fID == RESTYPE ){
  ...
  cout << "------------------------------ T&P stuff ----------------------------------------" << endl;
  bool cut = false;
  TString Label = "hltSingleMu5L3Filtered5:REDIGI36X::";
  if ( fpEvt->nCands() > 0){
    for (int it = 0; it < fpEvt->nCands(); ++it) {
      ...
      if  ( !isPathFired(HLTPATH) ) continue; 
      if  ( !isMatchedToTrig(pTrack_tag, HLTLABEL) ) continue;
	...
	if ( abs(pGEn1->fID) == 13 && abs(pGEn2->fID) == 13 ){
	  ...
	  cout << "pMom2->fID = " << pMom2->fID << endl;
	   ...
	   if ( pMom1->fID == RESTYPE && pMom2->fID == RESTYPE ){
	     ...
	     if ( RESTYPE  ==  553 || RESTYPE ==  100553 || RESTYPE ==  200553){
  ...
  for( int ig = 0; ig < fpEvt->nGenCands(); ig++ ){
    ...
    if (pGen1->fID == RESTYPE && pGen1->fStatus == 2){
...
bool tnpReader::isPathFired( TString Path ){
  bool HLT_Path = false;
  for (int a = 0; a < NHLT ; ++a) {
    if ( fpEvt->fHLTNames[a] ==  HLTPATH  && fpEvt->fHLTResult[a] == 1  ) {
      HLT_Path = true;
      cout << HLTPATH << " fired!!!! "  << endl;
    }
  }
  return HLT_Path;
}

bool tnpReader::isMatchedToTrig(TAnaTrack *pTag, TString Label){
  bool HLTlabel = false;
  TTrgObj *pTrig;
  TLorentzVector tag;
  ...
  for (int s = 0; s < fpEvt->nTrgObj() ; ++s) {
    ...
    if ( !(Label.CompareTo(pTrig->fLabel)) ) {
      ...
      if ( (dPhi < 0.1) && (dEta < 0.2)) {
	HLTlabel = true;				
      	((TH1D*)fpHistFile->Get("trig_dR_aftercuts"))->Fill(dR);
      }
    }
  }
  return HLTlabel;
...
// ----------------------------------------------------------------------
void tnpReader::fillHist() {

  
}

// ----------------------------------------------------------------------
void tnpReader::bookHist() {
  ...
  if ( RESTYPE == 443 ){
    ...
    highmass = 3.4;
    binnum = 30;
    cout << "tnpReader: Running over J/psi: " << lowmass << " " << highmass << endl;
  } else {
    cout << "tnpReader: Running over Ups: " << lowmass << " " << highmass << endl;
  ...
  h = new TH1D("trig_dR_aftercuts",  "trig_dR_aftercuts", 50, 0., 1.);
  ...
  k = new TH2D("tEtaPt_pos", "tEtaPt_pos", Neta, Etabin, Npt, PTbin);	
  k = new TH2D("mEtaPt_neg", "mEtaPt_neg", Neta, Etabin, Npt, PTbin);	
...
// --------------------------------------------------------------------------------------------------
void tnpReader::readCuts(TString filename, int dump) {
  char  buffer[200];
  fCutFile = filename;
  if (dump) cout << "Reading " << fCutFile.Data() << " for tnpReader cut settings" << endl;
