...
HFDumpJets::HFDumpJets(const edm::ParameterSet& iConfig):
  ...
  fCorrJetsLabel(iConfig.getUntrackedParameter<string>("corrjetsLabel", string("MCJetCorJetIcone5"))),
...
void HFDumpJets::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  // -- get the collection of RecoJets 
  edm::Handle<reco::CaloJetCollection> corrcaloJets;
  iEvent.getByLabel( fCorrJetsLabel.c_str(), corrcaloJets );
  ...
  /*  for( reco::CaloJetCollection::const_iterator cal = caloJets->begin(); cal != caloJets->end(); ++ cal ) {
    ...
    const JetCorrector* corrector = JetCorrector::getJetCorrector ("MCJetCorrectorIcone5", iSetup);
    double scale = corrector->correction (*cal);
    pCaloJet->finvisibleEnergy = scale;
  ...
  //Loop over CorrCaloJets 
  jetIndex=0;
  for( reco::CaloJetCollection::const_iterator cal = corrcaloJets->begin(); cal != corrcaloJets->end(); ++ cal ) {
...
// 	}
  ...
  for( reco::GenJetCollection::const_iterator gen = genJets->begin(); gen != genJets->end(); ++ gen ) { 
    ...
    //-------------------- 
  //   TLorentzVector jetvect;
//     jetvect.SetPtEtaPhiE(gen->pt(), gen->eta(), gen->phi(), gen->energy());
        
//     std::vector <const GenParticleCandidate*> mcparts = gen->getConstituents();
...
// 	  muonvect.SetPtEtaPhiM(mcpart->pt(), mcpart->eta(), mcpart->phi(), mMuon);
// 	  jetvect = jetvect - muonvect;
	  
// 	}
// 	//cout << "MCpart pdgid " << mcpart->pdgId() << " pt " << mcpart->pt() << endl;
//       //*****************************************************
// 	found = find( cands_.begin(), cands_.end(), mcpart );
// 	if ( found != cands_.end() ) idx = found - cands_.begin() ;
// 	//cout << "index " << idx << endl;
// 	pGenJet->addTrack(idx);
//       //****************************************************
//       } 
//     }
   
  
//     pGenJet->fPlab.SetPtEtaPhi(jetvect.Pt(),
// 			       jetvect.Eta(),
// 			       jetvect.Phi()
// 			       );

//     pGenJet->fE  = jetvect.E();
//     pGenJet->fEt = jetvect.Et();
//     pGenJet->fM  = jetvect.M();
//     pGenJet->fMt = jetvect.Mt(); 
    //-------------------- 

    //cout << "***************************************************" << endl;
    std::vector <const GenParticleCandidate*> mcparts = gen->getConstituents(); 
    //cout << "size  " << mcparts.size () << endl; 
    ...
    // cout << "---------------------------------------------------" << endl;
    //cout << "NTRACKS " << pGenJet->getNtracks() << endl;
    //cout << "---------------------------------------------------" << endl;
    ...
    pGenJet->fEMEnergy         = gen->emEnergy(); //not corrected
    pGenJet->fHADEnergy        = gen->hadEnergy();//not corrected
    pGenJet->finvisibleEnergy  = gen->invisibleEnergy();//not corrected
