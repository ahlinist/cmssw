...
// $Id: DiMuonAnalyzer.cc,v 1.2 2008/06/24 10:13:09 dde Exp $
//
//
...
private:
  ...
  virtual  void GenAnalyze(const edm::Event& iEvent, const edm::EventSetup& iSetup);
...
void
DiMuonAnalyzer::GenAnalyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace HepMC;
  using namespace edm;
...
//***********************************Compare************************************************
void
DiMuonAnalyzer::Compare(const edm::Event &iEvent, const edm::EventSetup &iSetup)
{
  ...
  for (HepMC::GenEvent::particle_const_iterator p = genEvent->particles_begin(); p != genEvent->particles_end(); ++p)
	...
	ppy2[imuminus]=gpy;
	ppz2[imuminus]=gpz;
	ener2[imuminus]=gener;
	imuminus++;
	nmn++;
      }

      if(imuplus>0 && imuminus>0){
        for (  unsigned int i=0; i< imuplus; i++ ) {
          for ( unsigned int j=0; j< imuminus; j++ ){
            double eptot=ener1[i]+ener2[j];
            double ppxtot=ppx1[i]+ppx2[j];
...
void DiMuonAnalyzer::beginJob(const edm::EventSetup&)
{
  ...
  fHistDiMuonGen_InvM = new TH1D("fHistDiMuonGen_InvM ","Gen_dimu minv (GeV)",150,7,12.0);
