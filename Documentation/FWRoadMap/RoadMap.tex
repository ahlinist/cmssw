...
%% 
%%
%% $Id: RoadMap.tex,v 1.35 2005/09/01 16:35:02 wmtan Exp $
%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\newcommand{\ESSource}{\classname{ES\-Source}}
\newcommand{\ESProducer}{\classname{ES\-Producer}}
...
\newcommand{\EventSetup}{\classname{Event\-Setup}}
...
\newcommand{\Record}{\classname{Record}}
...
and would be an (admittedly minor) annoyance to the user of the \Root prompt.

Solution two would allow the use of simple things 
like \cppcode{std::vector<CaloJet>} directly as an \EDProduct.
Solution one requires writing a class that could \emph{contain}
a \cppcode{std::vector<CaloJet>}.  This container would appear as another level
of indirection in the \Root output files,
and would be an (admittedly minor) annoyance to the user of the \Root prompt.
...
In Solution two, the ``wrapped'' \EDProduct has this functionality, but
the ``bare'' \EDProduct does not.

The initial implementation of \EDProduct used a base class.  However,
a decision subsequently was made to use wrappers, and an implementation
using wrappers has supplanted the original implementation.
...
For these reasons,
we see the need for an \emph{input source},
which is not a module,
and for \emph{output modules}.

In general,
the framework will invoke the appropriate input source.
As a special case,
a \emph{mixing module} could invoke
an additional input source or sources.
...
The mixing module closely collaborates with a specialised secondary input source in order to fulfill the following 2 tasks: 
...
before and after) and asks the secondary input source to deliver them. The 
...
\end{verbatim}
\subsection {Configuration of the secondary input source}
It is assumed that the secondary input source is able to provide randomly 
...
\subsubsection {Demands on the secondary input source}
\begin{itemize}
\item
A specific method of the input source will be needed, allowing to get a 
vector of events 
...
If not specified otherwise, the secondary input source delivers events randomly, it is its own task to guarantee maximum 
efficiency for this.
...
process PROD  = {
        ...
        source minbias = SecondaryInputSource {
...
\item 
\cppcode{getBySelector} takes a \Selector object
...
\item
\cppcode{getByType} returns the unique \EDProduct
of the appropriate type.
If there is not exactly one such match
this \cppcode{get}
throws an appropriate exception.
...
\item
\cppcode{getManyByType} returns all the \EDProduct instances
of the appropriate type.
\EDProducer{}s shall not use \cppcode{getManyByType}.
...
Note that \emph{input} provided by a source,
...
The only function of an \EDProducer is to produce \EDProduct instances
and placing them in an \Event.  This function is
...
\begin{verbatim}
   ...
   ParameterSet main_input = { /* jbk - is this pset name correct? */
     ...
     string module_type = ``FileBasedInputSource'';
     untracked string fileName = ``myFile'';
     untracked bool buffered = true;
   }
\end{verbatim}
...
two types of \IOV{}s which are distinguished by whether or not the DAQ system initiated the interval of validity transition.  \IOV{}s initiated by DAQ (such as the Event or a Run transition) are to be handled by
the Event system.  All other \IOV{}s are handled by the \EventSetup{} System.

\subsection{\EventSetup{} System }

\begin{figure}[h]
\includegraphics[width=\textwidth]{EventSetup_Diagram.eps}
\caption{%
The \EventSetup{} is formed from the Records that have an \IOV{} that overlaps with the moment in time that is being studied.}\label{fig:eventsetup_diagram}
\end{figure}
...
\begin{enumerate}
\item
Record: holds data and services which have identical \IOV{}s.
\item
\EventSetup{}: holds all Records that have an \IOV{} which overlap with the 'time' of the Event being studied.

\end{enumerate}


\subsection{\EventSetup{} }
The \EventSetup{} class provides type-safe access to the various \Record{}s it contains.  This access is done through the \EventSetup{}'s \cppcode{get<RecordT>()} method. If the requested Record is not available, an exception will be thrown.  There is also an interface to get data directly from an \EventSetup{} instead of from a \Record{} for the case where the data type has  been assigned a 'default' \Record{}.  The direct data access interface is discussed in a section~\vref{sec:recordcontent}.

In addition to access to \Record{}s, the \EventSetup{} has a method \cppcode{timestamp()} which return information about the 'instance in time' for which the \EventSetup{} is describing.

\subsection{\Record{}s}
\Record{}s provide type-safe access to the objects it contains.    The access is handled through the \Record{}'s \cppcode{get(ESHandle<T>& )} method.  This is analogous to data access from the \Event{}.

A \Record{} also provides access to its interval of validity ( \IOV{} ) through its \cppcode{validityInterval()} method.

\subsection{Contents of a \Record{}}\label{sec:recordcontent}
...
In the case where a data/service C++ type is only meant to come from one Record type, then the 'default' Record type can be declared at compile time.  If a 'default' Record has been declared for a data type, then users can access that data directly from the \EventSetup{} via the \cppcode{get(ESHandle<T>& )}. 
 

\subsection{\EventSetup{} System Components}

The \EventSetup{} system design, shown in figure~\vref{fig:eventsetup_design}, uses two categories of components to do its work: \ESSource{} and \ESProducer{}.  These categories are 'high-level abstractions' built on top of low-level interfaces.

\begin{figure}[h]
\includegraphics[width=\textwidth]{eventsetup_design.eps}
\caption{%
The \EventSetup{} system design.}\label{fig:eventsetup_design}
\end{figure}

\subsubsection{Low-level Interfaces}

\emph{DataProxies}: When a \cppcode{get} call is made to a \Record{}, a DataProxy is looked-up in the \Record{} and then that DataProxy is asked to handle the request.

\emph{DataProxyProvider}: A container of related DataProxies.  Advertises what \Record{}s it has DataProxies for and is capable of creating those DataProxies on request.  The DataProxyProviders are given to the EventSetupProvider who hands them off to the proper EventSetupRecordProvider.  The EventSetupRecordProvider then insert the DataProxies from the DataProxyProvider into the proper \Record{} instance.

\emph{ProxyFactoryProducer}: A DataProxyProvider which uses pre-registered Proxy factories to create
the necessary DataProxies.  This is a convenience class for developers who need to be able to write their own DataProxies.

\emph{ESProducer}: The easiest to implement DataProxyProvider.  Simply by writing a \cppcode{produce} method, the \ESProducer{} will use the argument to the method to determine the \Record{} the algorithm is dependent upon and will use the return value to determine the type of data/service being created.  This deduction is done by calling the \cppcode{setWhatProduced(this)} from the class' constructor.

\emph{EventSetupRecordIntervalFinder}: Interface for determining the proper \IOV{} of a \Record{}, or a group of \Record{}s.    The EventSetupRecordIntervalFinders are given to the EventSetupProvider who hands them off to the proper EventSetupRecordProvider.  The EventSetupRecordProvider uses its EventSetupRecordIntervalFinder to set the proper \IOV{} of its \Record{}.


\subsubsection{EventSetup Source }
...
\subsubsection{ EventSetup Producer }
...
Sometimes an algorithm in the \EventSetup{} is dependent on data coming from more than one \Record{}.  For example, the tracking geometry is dependent on the 'ideal geometry' and on the tracking alignment values.  In such a case the \Record{} used by that algorithm needs to be declared 'dependent' on the other \Record{}s.  This dependency declaration is accomplished by having the dependent \Record{} inherit from \cppcode{DependentRecordImplementation<T,List>}.  The template parameter \cppcode{List} is a compile time list of the \Record{}s upon which this \Record{} is dependent.

Dependent \Record{}s allow access to the \Record{}s to which they are dependent via the \cppcode{getRecord<T>()} method.
...
\subsection{\EventSetup{} configuration}

\EventSetup{} components are configured using the same configuration mechanism as their \Event{} counterparts, i.e., via the \ParameterSet{} system.  Two additional keywords were added to the configuration language to allow the configuration of the \EventSetup{}:   \cppcode{es_source} and \cppcode{es_module}.
