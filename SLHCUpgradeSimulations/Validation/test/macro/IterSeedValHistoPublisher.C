void IterSeedValHistoPublisher(char* newFile="NEW_FILE",char* refFile="REF_FILE")
{
 ...
 // select newCombinedSeeds to make the usual validation plots for
 int num_col = sl->GetSize();
 for(int i=0; i<num_col;i++){
   collname2 =sl->At(i)->GetName();
   if(collname2.Contains("CombinedSeeds")) break;
   //if(collname2.Contains("newSeedFromTriplets")) break;
 }
 ...
 for(int i=0; i<num_col;i++){
   ...
   if(collname1.Contains("CombinedSeeds")) break;
 }
...
//   sh2->GetYaxis()->SetRangeUser(0.,.70);
   ...
   l->Draw();
   canvas->Print("seed_building.pdf");   
   delete l;
   ...
   l->AddEntry(sh1,newLabel,"LPF");
   l->Draw();
   canvas->Print("seed_hitsAndPt.pdf");
   delete l;
...
//    delete l;
   ...
   TH1F * rr[5]={r[1],r[3],r[2],r[4],r[0]};
   TH1F * ss[5]={s[1],s[3],s[2],s[4],s[0]};

   canvas = new TCanvas("Tracks10","Seeds: efficiency for each step",1000,1400);
   ...
   l->Draw();
   canvas->Print("seed_steps_eff.pdf");
   delete l;
   ...
   canvas = new TCanvas("Tracks11","Seeds: fakerate for each step",1000,1400);
   ...
   l->AddEntry(s[0],newLabel,"LPF");
   l->Draw();
   canvas->Print("seed_steps_fakerate.pdf");
   delete l;
   ...
   canvas = new TCanvas("Tracks12","Seeds: efficiency for each step",1000,1400);
   ...
   l->AddEntry(s[0],newLabel,"LPF");
   l->Draw();
   canvas->Print("seed_steps_effpt.pdf");
   delete l;
   ...
   canvas = new TCanvas("Tracks13","Seeds: fakerate for each step",1000,1400);
   ...
   l->AddEntry(s[0],newLabel,"LPF");
   l->Draw();
   canvas->Print("seed_steps_fakeratePt.pdf");
   delete l;
