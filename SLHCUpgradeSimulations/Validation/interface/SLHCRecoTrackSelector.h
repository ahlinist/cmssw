#ifndef RecoSelectors_SLHCRecoTrackSelector_h
#define RecoSelectors_SLHCRecoTrackSelector_h
/* \class SLHCRecoTrackSelector
 *
 * Taken from RecoTrackSelector with modification
 * to enable selection as for b-tagging tracks
 *
 *  $Revision: 1.1 $
 *
 */
...
class SLHCRecoTrackSelector {
 public:
  ...
  /// Constructors
  SLHCRecoTrackSelector() {}
  SLHCRecoTrackSelector ( const edm::ParameterSet & cfg ) :
    ...
    minPxHit_(cfg.getParameter<int>("minPxHit")),
  ...
  SLHCRecoTrackSelector ( double ptMin, double minRapidity, double maxRapidity,
		      double tip, double lip, int minHit, int min3DHit, int minPxHit, double maxChi2, 
    ...
    tip_( tip ), lip_( lip ), minHit_( minHit ), min3DHit_( min3DHit), minPxHit_(minPxHit), maxChi2_( maxChi2 ) 
    { 
      for (unsigned int j=0;j<quality.size();j++) quality_.push_back(reco::TrackBase::qualityByName(quality[j]));
      for (unsigned int j=0;j<algorithm.size();j++) algorithm_.push_back(reco::TrackBase::algoByName(algorithm[j]));
    }
  ...
  bool operator()( const reco::Track & t, const reco::BeamSpot* bs) {
    ...
    return
      (t.hitPattern().trackerLayersWithMeasurement() >= minHit_ &&
       ...
       t.hitPattern().pixelLayersWithMeasurement() >= minPxHit_ &&
       fabs(t.pt()) >= ptMin_ &&
