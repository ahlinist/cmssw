...
//
SLHCPrimaryVertexAnalyzer::SLHCPrimaryVertexAnalyzer(const ParameterSet& iConfig)
{
...
SLHCPrimaryVertexAnalyzer::~SLHCPrimaryVertexAnalyzer()
{
...
// member functions
//
void SLHCPrimaryVertexAnalyzer::beginJob(){
  std::cout << " SLHCPrimaryVertexAnalyzer::beginJob  conversion from sim units to rec units is " << simUnit_ << std::endl;
...
void SLHCPrimaryVertexAnalyzer::endJob() {
  rootFile_->cd();
...
// helper functions
bool SLHCPrimaryVertexAnalyzer::matchVertex(const simPrimaryVertex  &vsim, 
				       const reco::Vertex       &vrec){
  return (fabs(vsim.z*simUnit_-vrec.z())<0.0500); // =500um
}

bool SLHCPrimaryVertexAnalyzer::isResonance(const HepMC::GenParticle * p){
...
bool SLHCPrimaryVertexAnalyzer::isFinalstateParticle(const HepMC::GenParticle * p){
  return ( !p->end_vertex() && p->status()==1 );
}

bool SLHCPrimaryVertexAnalyzer::isCharged(const HepMC::GenParticle * p){
...
void SLHCPrimaryVertexAnalyzer::printRecVtxs(const Handle<reco::VertexCollection> recVtxs){
    int ivtx=0;
...
void SLHCPrimaryVertexAnalyzer::printSimVtxs(const Handle<SimVertexContainer> simVtxs){
    int i=0;
...
void SLHCPrimaryVertexAnalyzer::printSimTrks(const Handle<SimTrackContainer> simTrks){
...
std::vector<SLHCPrimaryVertexAnalyzer::simPrimaryVertex> SLHCPrimaryVertexAnalyzer::getSimPVs(
				      const Handle<HepMCProduct> evtMC, std::string suffix="")
{
  std::vector<SLHCPrimaryVertexAnalyzer::simPrimaryVertex> simpv;
...
void
SLHCPrimaryVertexAnalyzer::analyze(const Event& iEvent, const EventSetup& iSetup)
{
