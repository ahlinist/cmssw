...
class HitMatchingAlgorithm_pixelray : public HitMatchingAlgorithm<T>
{
    public:
        ...
        /* WARNING: This only works without clusters, i.e. with one hit in the cluster.
         * It needs to be fixed for clustering
         */
		bool CheckTwoMemberHitsForCompatibility(const cmsUpgrades::LocalStub<T> & aLocalStub) const
		{
			std::vector<double> * rayEndpoints;
			...
			if (rayEndpoints) {
				...
				if ((rayEndpoints->at(1) > negativeZBoundary) &&
			        (rayEndpoints->at(0) < positiveZBoundary))
			    {
					delete rayEndpoints;
