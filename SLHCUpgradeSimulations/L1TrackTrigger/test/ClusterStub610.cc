...
//
class ClusterStub610 : public edm::EDAnalyzer
{
  /// Public methods
  public:
    /// Constructor/destructor
    explicit ClusterStub610(const edm::ParameterSet& iConfig);
    virtual ~ClusterStub610();
  ...
  private:
    ...
    TH2D* hSimVtx_XY;
    TH2D* hSimVtx_RZ;
//    TH2D* hSimVtx_XY_Corr;
//    TH2D* hSimVtx_RZ_Corr;
    ...
    /// TO CHECK GEOMETRY
    TH2D* hGeomBarrel_Layer_R;
    TH2D* hGeomBarrel_Rod_Phi;
    TH2D* hGeomBarrel_Mod_Z;

    TH2D* hGeomEndcap_Disk_Z;
    TH2D* hGeomEndcap_Ring_R;
    TH2D* hGeomEndcap_Mod_Phi;
...
//////////////
// CONSTRUCTOR
ClusterStub610::ClusterStub610(edm::ParameterSet const& iConfig) : 
  config(iConfig)
{
...
//////////
// END JOB
void ClusterStub610::endJob()//edm::Run& run, const edm::EventSetup& iSetup
{
  /// Things to be done at the exit of the event Loop
  std::cerr << " ClusterStub610::endJob" << std::endl;
  /// End of things to be done at the exit from the event Loop
}

////////////
// BEGIN JOB
void ClusterStub610::beginJob()
{
...
//  double trkMPt = 99999.9;
  ...
  std::cerr << " ClusterStub610::beginJob" << std::endl;
  ...
  double MinPt = 0.0;
  double MaxPt = 100.0;
//  double BinWidth = (MaxPt - MinPt) / NumBins;
...
//  hSimVtx_XY_Corr = fs->make<TH2D>( "hSimVtx_XY_Corr",  "SimVtx y vs. x, BS Corr",  140, -0.7, 0.7, 140, -0.7, 0.7 );
  hSimVtx_XY->Sumw2();
//  hSimVtx_XY_Corr->Sumw2();

  hSimVtx_RZ      = fs->make<TH2D>( "hSimVtx_RZ",       "SimVtx #rho vs. z",           200, -50, 50, 140, 0, 0.7 );
//  hSimVtx_RZ_Corr = fs->make<TH2D>( "hSimVtx_RZ_Corr",  "SimVtx #rho vs. z, BS Corr",  200, -50, 50, 140, 0, 0.7 );
  hSimVtx_RZ->Sumw2();
//  hSimVtx_RZ_Corr->Sumw2();
  ...
  for ( unsigned int stackIdx = 0; stackIdx < 10; stackIdx++ ) {
  ...
  /// To test the Geometry
  hGeomBarrel_Layer_R = fs->make<TH2D>( "hGeomBarrel_Layer_R", "Barrel iStack vs. r",  480, -0.5, 119.5, 12, -0.5, 11.5 );
  hGeomBarrel_Rod_Phi = fs->make<TH2D>( "hGeomBarrel_Rod_Phi", "Barrel iPhi vs. #phi", 180, -M_PI, M_PI, 68, -0.5, 67.5 );
  hGeomBarrel_Mod_Z   = fs->make<TH2D>( "hGeomBarrel_Mod_Z",   "Barrel iZ vs. z",      150, -300, 300, 54, -0.5, 53.5);
  hGeomBarrel_Layer_R->Sumw2();
  hGeomBarrel_Rod_Phi->Sumw2();
  hGeomBarrel_Mod_Z->Sumw2();

  hGeomEndcap_Disk_Z  = fs->make<TH2D>( "hGeomEndcap_Disk_Z",  "Endcap iStack vs. z",  150, -300, 300, 12, -0.5, 11.5 );
  hGeomEndcap_Ring_R  = fs->make<TH2D>( "hGeomEndcap_Ring_R",  "Endcap iRing vs. r",   480, -0.5, 119.5, 30, -0.5, 29.5 );
  hGeomEndcap_Mod_Phi = fs->make<TH2D>( "hGeomEndcap_Mod_Phi", "Endcap iPhi vs. #phi", 180, -M_PI, M_PI, 90, -0.5, 89.5);
  hGeomEndcap_Disk_Z->Sumw2();
  hGeomEndcap_Ring_R->Sumw2();
  hGeomEndcap_Mod_Phi->Sumw2();
...
//////////
// ANALYZE
void ClusterStub610::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  // GET BS //
  ////////////
  //edm::Handle< std::vector< L1TkBeam > > BeamHandle;
  //iEvent.getByLabel( "L1TkBeams", BeamHandle );
  //GlobalPoint beamPos = BeamHandle->at(0).getBeamPosition();
...
/*
for (unsigned int q=0; q<iterL1TkCluster->getHits().size(); q++)
{
  std::cerr<< q<< "   >>> "<<iterL1TkCluster->findHitGlobalPosition( theStackedGeometry,  q);
  std::cerr<<std::endl;
}
std::cerr<<std::endl;
*/
//      double xClu         = iterL1TkCluster->findAverageLocalCoordinates().x();
//      double yClu         = iterL1TkCluster->findAverageLocalCoordinates().y();
      GlobalPoint posClu  = iterL1TkCluster->findAverageGlobalPosition( theStackedGeometry );
  ...
  if ( PixelDigiL1TkStubHandle->size() > 0 )
  {
          ...
          ++iterL1TkStub )
    {
      ...
      int displStub       = iterL1TkStub->getTriggerDisplacement();
      int offsetStub      = iterL1TkStub->getTriggerOffset();
      GlobalPoint posStub = iterL1TkStub->findGlobalPosition( theStackedGeometry );
      ...
      double recPt = iterL1TkStub->findRoughPt( mMagneticFieldStrength, theStackedGeometry );
      double recEta = iterL1TkStub->findGlobalDirection( theStackedGeometry ).eta();
      double recPhi = iterL1TkStub->findGlobalDirection( theStackedGeometry ).phi();
...
//      GlobalPoint trkVtxCorr = GlobalPoint( trkVtxPos.x() - beamPos.x(), trkVtxPos.y() - beamPos.y(), trkVtxPos.z() - beamPos.z() );

      /// First of all, check beamspot and correction
      hSimVtx_XY->Fill( trkVtxPos.x(), trkVtxPos.y() );
//      hSimVtx_XY_Corr->Fill( trkVtxCorr.x(), trkVtxCorr.y() );
      hSimVtx_RZ->Fill( trkVtxPos.z(), trkVtxPos.rho() );
//      hSimVtx_RZ_Corr->Fill( trkVtxCorr.z(), trkVtxCorr.perp() );

      /// Select only tracks within few cm from beam
      //if ( trkVtxCorr.perp() > 2.0 ) continue;
  ...
  if (testedGeometry==false) {
    ...
    for ( unsigned int k=0; k<stackContainer.size(); k++ ) {
      ...
      if (stackDetId.isBarrel()) {
        int iStack = stackDetId.iLayer();
        int iPhi   = stackDetId.iPhi();
        int iZ     = stackDetId.iZ();

        const GeomDetUnit* detUnit = theStackedGeometry->idToDetUnit( stackDetId, iStack%2 );
        GlobalPoint zeroZeroPoint = detUnit->toGlobal( detUnit->topology().localPosition( MeasurementPoint( 0, 0) ) );

        hGeomBarrel_Layer_R->Fill( zeroZeroPoint.perp(), iStack );
        hGeomBarrel_Rod_Phi->Fill( zeroZeroPoint.phi(), iPhi );
        hGeomBarrel_Mod_Z->Fill( zeroZeroPoint.z(), iZ );
      }
      else if (stackDetId.isEndcap()) {
        int iStack = stackDetId.iDisk();
        int iRing  = stackDetId.iRing();
        int iPhi   = stackDetId.iPhi();

        const GeomDetUnit* detUnit = theStackedGeometry->idToDetUnit( stackDetId, iStack%2 );
        GlobalPoint zeroZeroPoint = detUnit->toGlobal( detUnit->topology().localPosition( MeasurementPoint( 0, 0) ) );

        hGeomEndcap_Disk_Z->Fill( zeroZeroPoint.z(), iStack );
        hGeomEndcap_Ring_R->Fill( zeroZeroPoint.perp(), iRing );
        hGeomEndcap_Mod_Phi->Fill( zeroZeroPoint.phi(), iPhi );        
      }
...
DEFINE_FWK_MODULE(ClusterStub610);
