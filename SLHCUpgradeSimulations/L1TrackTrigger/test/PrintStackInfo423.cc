...
//
class PrintStackInfo423 : public edm::EDAnalyzer
{
  /// Public methods
  public:
    /// Constructor/destructor
    explicit PrintStackInfo423(const edm::ParameterSet& iConfig);
    virtual ~PrintStackInfo423();
...
//////////////
// CONSTRUCTOR
PrintStackInfo423::PrintStackInfo423(edm::ParameterSet const& iConfig) : 
  config(iConfig)
{
  ...
  outputFile.open("GeometrySummary.log",std::ios::out);
}

/////////////
// DESTRUCTOR
PrintStackInfo423::~PrintStackInfo423()
{
...
//////////
// END JOB
void PrintStackInfo423::endJob()//edm::Run& run, const edm::EventSetup& iSetup
{
  /// Things to be done at the exit of the event Loop
  outputFile.close();

  std::cerr << " PrintStackInfo423::endJob" << std::endl;
  /// End of things to be done at the exit from the event Loop
}

////////////
// BEGIN JOB
void PrintStackInfo423::beginJob()
{
  ...
  std::cerr << " PrintStackInfo423::beginJob" << std::endl;
...
//////////
// ANALYZE
void PrintStackInfo423::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  for ( StackedTrackerIterator = theStackedGeometry->stacks().begin();
    ...
    if ( stackDetId.isBarrel() ) {
      ...
      if ( rodsPerLayer.find(iStack) == rodsPerLayer.end() )
        rodsPerLayer.insert( make_pair(iStack, 0) );
      if ( iPhi >= rodsPerLayer[iStack] )
        rodsPerLayer[iStack] = iPhi;//+1;

      if ( modsPerRodPerLayer.find(iStack) == modsPerRodPerLayer.end() )
        modsPerRodPerLayer.insert( make_pair(iStack, 0) );
      if ( iZ >= modsPerRodPerLayer[iStack] )
        modsPerRodPerLayer[iStack] = iZ;//+1;
      ...
      if ( inOutPerRodPerLayer.find(iStack) == inOutPerRodPerLayer.end() ) {
        ...
        tempMap.insert( make_pair(iPhi, (iModule1 == (iModule0 + 1))) );
        inOutPerRodPerLayer.insert( make_pair(iStack, tempMap) );
      }
      else {
        /// Existing Layer
    ...
    if ( stackDetId.isEndcap() ) {
      ...
      if ( ringsPerDisk.find(iStack) == ringsPerDisk.end() )
        ringsPerDisk.insert( make_pair(iStack, 0) );
      ...
      if ( modsPerRingPerDisk.find(iStack) == modsPerRingPerDisk.end() ){
        ...
        tempMap.insert( make_pair(iRing, 0) );
        modsPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      if ( modsPerRingPerDisk.find(iStack)->second.find(iRing) == modsPerRingPerDisk.find(iStack)->second.end() )
        modsPerRingPerDisk.find(iStack)->second.insert( make_pair( iRing, 0 ) );
      if ( iRing >= ringsPerDisk[iStack] )
        ringsPerDisk[iStack] = iRing+1;
      if ( iPhi >= modsPerRingPerDisk.find(iStack)->second.find(iRing)->second )
        modsPerRingPerDisk.find(iStack)->second.find(iRing)->second = iPhi;//+1;
      ...
      if ( inOutPerRingPerDisk.find(iStack) == inOutPerRingPerDisk.end() ) {
        /// New Disk, new Ring
        std::map< uint32_t, bool > tempMap;
        tempMap.insert( make_pair(iRing, (iModule1 == (iModule0 + 1))) );
        inOutPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
        /// Existing Disk
      ...
      if ( innerModZPerRingPerDisk.find(iStack) == innerModZPerRingPerDisk.end() ) {
        ...
        tempMap.insert( make_pair(iRing, 0) );
        innerModZPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( outerModZPerRingPerDisk.find(iStack) == outerModZPerRingPerDisk.end() ) {
        std::map< uint32_t, double > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        outerModZPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( separationPerRingPerDisk.find(iStack) == separationPerRingPerDisk.end() ) {
        std::map< uint32_t, double > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        separationPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( innerModXPitchPerRingPerDisk.find(iStack) == innerModXPitchPerRingPerDisk.end() ) {
        ...
        tempMap.insert( make_pair(iRing, 0) );
        innerModXPitchPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( outerModXPitchPerRingPerDisk.find(iStack) == outerModXPitchPerRingPerDisk.end() ) {
        std::map< uint32_t, float > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        outerModXPitchPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( innerModYPitchPerRingPerDisk.find(iStack) == innerModYPitchPerRingPerDisk.end() ) {
        std::map< uint32_t, float > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        innerModYPitchPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( outerModYPitchPerRingPerDisk.find(iStack) == outerModYPitchPerRingPerDisk.end() ) {
        std::map< uint32_t, float > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        outerModYPitchPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( innerModChannelsPerRingPerDisk.find(iStack) == innerModChannelsPerRingPerDisk.end() ) {
        std::map< uint32_t, uint32_t > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        innerModChannelsPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( outerModChannelsPerRingPerDisk.find(iStack) == outerModChannelsPerRingPerDisk.end() ) {
        std::map< uint32_t, uint32_t > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        outerModChannelsPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( innerModRowsPerRingPerDisk.find(iStack) == innerModRowsPerRingPerDisk.end() ) {
        std::map< uint32_t, uint32_t > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        innerModRowsPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( outerModRowsPerRingPerDisk.find(iStack) == outerModRowsPerRingPerDisk.end() ) {
        std::map< uint32_t, uint32_t > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        outerModRowsPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( innerModColumnsPerRingPerDisk.find(iStack) == innerModColumnsPerRingPerDisk.end() ) {
        std::map< uint32_t, uint32_t > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        innerModColumnsPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( outerModColumnsPerRingPerDisk.find(iStack) == outerModColumnsPerRingPerDisk.end() ) {
        std::map< uint32_t, uint32_t > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        outerModColumnsPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( activeSurfacePerRingPerDisk.find(iStack) == activeSurfacePerRingPerDisk.end() ) {
        std::map< uint32_t, double > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        activeSurfacePerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( innerModXROCPerRingPerDisk.find(iStack) == innerModXROCPerRingPerDisk.end() ) {
        std::map< uint32_t, uint32_t > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        innerModXROCPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( outerModXROCPerRingPerDisk.find(iStack) == outerModXROCPerRingPerDisk.end() ) {
        std::map< uint32_t, uint32_t > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        outerModXROCPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( innerModYROCPerRingPerDisk.find(iStack) == innerModYROCPerRingPerDisk.end() ) {
        std::map< uint32_t, uint32_t > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        innerModYROCPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( outerModYROCPerRingPerDisk.find(iStack) == outerModYROCPerRingPerDisk.end() ) {
        std::map< uint32_t, uint32_t > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        outerModYROCPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( innerModTotROCPerRingPerDisk.find(iStack) == innerModTotROCPerRingPerDisk.end() ) {
        std::map< uint32_t, uint32_t > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        innerModTotROCPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
      ...
      if ( outerModTotROCPerRingPerDisk.find(iStack) == outerModTotROCPerRingPerDisk.end() ) {
        std::map< uint32_t, uint32_t > tempMap;
        tempMap.insert( make_pair(iRing, 0) );
        outerModTotROCPerRingPerDisk.insert( make_pair(iStack, tempMap) );
      }
      else {
...
DEFINE_FWK_MODULE(PrintStackInfo423);
