...
vector<float> GenericMinL3Algorithm::iterate(const vector<vector<float> >& eventMatrix, const vector<float>& energyVector, const int nIter)
{
  ...
  for(int i=0; i<theCalibVector.size(); i++){
//    cout << "THE VERY BEGINNING i=" << i << " " << theCalibVector[i] << endl; 
  }
  ...
  for (int iter=1;iter<=nIter;iter++) 
    {
    for(int i=0; i<theCalibVector.size(); i++){
//     cout << " IterationMode (" << iter << ") i=" << i << " " << theCalibVector[i] << endl;
    }
      // make one iteration
...
//        theCalibVector[i] = solution[i];
    ...
    for(int i=0; i<theCalibVector.size(); i++){
//     cout << "theCalib i=" << i << " " << theCalibVector[i] << endl; 
     cout << theCalibVector[i] << endl; 
    }
...
{
  cout << "Starting minimization................................ My intervention " << endl; 
  vector<float> solution;
...
//           if(solution[i]>40){  cout << "i=" << i << " WARNING SOLUTION " << solution[i] << endl;} 
        }
      else 
	{ 
//          cout << "supposed 0 " << " i= " << i << " " << solution[i] << endl; 
