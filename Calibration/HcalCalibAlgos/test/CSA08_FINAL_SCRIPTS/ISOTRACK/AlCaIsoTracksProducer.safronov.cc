...
//  
  ...
  m_pCut = iConfig.getUntrackedParameter<double>("pCut",15.);
  m_ptCut = iConfig.getUntrackedParameter<double>("ptCut",5);
  m_ecalCut = iConfig.getUntrackedParameter<double>("ecalCut",8.);
  m_histoFlag = iConfig.getUntrackedParameter<int>("histoFlag",0);
  
  ////////////Safronov
  isolE_ = iConfig.getUntrackedParameter<double>("maxNearbyTrackEnergy",2);
  etaMax_= iConfig.getUntrackedParameter<double>("maxEtaTrack",2.2);
  allowTracksInCone_ = iConfig.getParameter<bool>("AllowTracksInCone");
  ////////////
...
//               if( ddir < m_ddirCut*factor*factor1 ) {isol = 0; break;}
               ...
               if( ddir < dAngle*factor*factor1 ) 
		 {
//		   isol = 0;
//		   break;
		   ////////////// Safronov:
		   if (track1->p()>isolE_&&allowTracksInCone_) 
		     {
		       isol = 0;
		       break;
		     }
		   else if (!allowTracksInCone_)
		     {
		       isol = 0;
		       break;
		     }
