...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   FILE *Out1 = fopen("coef_12mln.txt", "w+");  
   FILE *Out2 = fopen("var_noise_12mln.txt", "w+");
...
//      cout<<" Mysubd "<<mysubd<<" "<<ieta<<" "<<depth<<" "<<mom0<<endl;
      if(mom0 == 0) continue;
...
// Calculation of dispersion ===============================
	 ...
	 minmeang[abs(ieta)][iphi][depth][mysubd]=minmeang[abs(ieta)][iphi][depth][mysubd] + (float)disp;
         minmeang_nn[abs(ieta)][iphi][depth][mysubd]=minmeang_nn[abs(ieta)][iphi][depth][mysubd] + (float)disp - minnoise[abs(ieta)][iphi][depth][mysubd];         
	 minmean[abs(ieta)][depth][mysubd]=minmean[abs(ieta)][depth][mysubd] + (float)disp;
	 minmean_nn[abs(ieta)][depth][mysubd]=minmean_nn[abs(ieta)][depth][mysubd] + (float)disp - minnoise[abs(ieta)][iphi][depth][mysubd];
         minneveta[abs(ieta)][depth][mysubd]++;
	 minnevetaphi[abs(ieta)][iphi][depth][mysubd]++;
	 
         minerrg[abs(ieta)][iphi][depth][mysubd] = minerrg[abs(ieta)][iphi][depth][mysubd] + disp*disp;
         minerr[abs(ieta)][depth][mysubd] = minerr[abs(ieta)][depth][mysubd] + disp*disp;
...
// Calculation of dispersion ===============================
	  ...
	  plmeang[ieta][iphi][depth][mysubd]=plmeang[ieta][iphi][depth][mysubd] + (float)disp;
          plmeang_nn[ieta][iphi][depth][mysubd]=plmeang_nn[ieta][iphi][depth][mysubd] + (float)disp - plnoise[abs(ieta)][iphi][depth][mysubd];
	  plmean[ieta][depth][mysubd]=plmean[ieta][depth][mysubd] + (float)disp;
          plmean_nn[abs(ieta)][depth][mysubd]=plmean_nn[abs(ieta)][depth][mysubd] + (float)disp - plnoise[abs(ieta)][iphi][depth][mysubd];
	  plneveta[abs(ieta)][depth][mysubd]++;
	  plnevetaphi[abs(ieta)][iphi][depth][mysubd]++;
	  
	  plerrg[abs(ieta)][iphi][depth][mysubd] = plerrg[abs(ieta)][iphi][depth][mysubd] + disp*disp;
	  plerr[abs(ieta)][depth][mysubd] = plerr[abs(ieta)][depth][mysubd] + disp*disp;
...
//    if( idep == 1 && ietak == 16) cout<<" My "<< ietak<<" "<<idep<<" "<<plneveta[ietak][idep][isub]<<" "<<plerr[ietak][idep][isub]<<" "<<plmean_mean<<" "<<perr<<endl;
    
    for(Int_t iphik = 1; iphik != 73; iphik++ )
    {

          if( idep == 1 && ietak == 16 ) cout<<" 16th ring "<<iphik<<" "<<plmeang[ietak][iphik][idep][isub]<<endl;
...
//            if( isub == 4 ) fprintf(Out1,"%s %d %d %d %.5f %.5f\n","HF",ietak,iphik,idep,plmeang[ietak][iphik][idep][isub],plmeang[ietak][iphik][idep][isub]);
            fprintf(Out1,"%d %d %d %d %.5f %.5f\n",isub,idep, ietak,iphik,plmeang[ietak][iphik][idep][isub],plmeang[ietak][iphik][idep][isub]); 
            continue;
           }
	
	   
       plmeang_nn_mean = plmeang[ietak][iphik][idep][isub]/plnevetaphi[ietak][iphik][idep][isub];
...
// Mean value and dispersion
  ...
  if( idep == 1 && ietak == 1) {
    ...
    cout<<" plerr[ietak][idep][isub] "<<plerr[ietak][idep][isub]<<" plerrg[ietak][iphik][idep][isub] "<<plerrg[ietak][iphik][idep][isub]<<endl;
    cout<<" Nev "<<plneveta[ietak][idep][isub]<<" "<<plnevetaphi[ietak][iphik][idep][isub]<<endl;
  } 	  
...
//            if( isub == 4 ) fprintf(Out1,"%s %d %d %d %.5f  %.5f\n","HF",iieta,iphik,idep,minmeang[ietak][iphik][idep][isub],minmeang[ietak][iphik][idep][isub]);
            ...
            fprintf(Out1,"%d %d %d %d %.5f %.5f\n",isub,idep, ietak,iphik,minmeang[ietak][iphik][idep][isub],minmeang[ietak][iphik][idep][isub]); 

            continue;
           }
	
	   
       minmeang_nn_mean = minmeang[ietak][iphik][idep][isub]/minnevetaphi[ietak][iphik][idep][isub];
...
// Mean value and dispersion
  ...
  if( idep == 1 && ietak == 1) {
    ...
    cout<<" minerr[ietak][idep] "<<minerr[ietak][idep][isub]<<" minerrg[ietak][iphik][idep][isub] "<<minerrg[ietak][iphik][idep][isub]<<endl;
    cout<<" Nev "<<minneveta[ietak][idep][isub]<<" "<<minnevetaphi[ietak][iphik][idep][isub]<<endl;
  } 	  
...
//         if( isub == 4 ) fprintf(Out1,"%s %d %d %d %.5f %.5f\n","HF",iieta,iphik,idep,minmeang_mean_corr,zz);
            ...
            fprintf(Out1,"%d %d %d %d %.5f %.5f\n",isub,idep, ietak,iphik,minmeang_mean_corr,zz); 
