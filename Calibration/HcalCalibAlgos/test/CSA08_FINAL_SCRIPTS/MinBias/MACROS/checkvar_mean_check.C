...
gStyle->SetOptFit(111);
   ...
   FILE *Out1 = fopen("coef_with_noise_mean_check.txt", "w+");  
   FILE *Out2 = fopen("coef_without_noise_10mln_mean_check.txt", "w+");
...
*/   
   std::ifstream in21( "MINBIAS_TEST/mean.txt" );
   
   while( std::getline( in21, line)){
   istringstream linestream(line);
   Float_t var,err;
   int subd,eta,phi,dep;
   linestream>>subd>>dep>>eta>>phi>>var;
   if( eta > 0 )
   {
...
//   if(subd == 2 && eta == 17) cout<<"Minbias subd "<<subd<<" "<<eta<<" "<<phi<<" "<<dep<<" "<<plmeang_nn[eta][phi][dep][subd]<<endl;   
...
//   Int_t idep = 1;
   ...
   Int_t plnumchan[50][5][5];
   Int_t minnumchan[50][5][5];
...
//    if( i > 20 ) nch1 = 36.; 
     ...
     if(plmeang_nn[i][j][idep0][k]<=0.) continue;
     if( k ==1 && i == 1 ) cout<<" excludede "<<excluded_min[i][j][idep0][k]<<endl;
     if(excluded_min[i][j][idep0][k]<=0){
      cout<<" ncp = "<<nchp<<endl;
      nchp++;
      plmean1[i][idep0][k] = plmean1[i][idep0][k] + plmeang_nn[i][j][idep0][k];
      plmean1_nn[i][idep0][k] = plmean1_nn[i][idep0][k] + plmeang_nn[i][j][idep0][k];
      }
     } // j   

    plnumchan[i][idep0][k] = nchp;
 
    for(Int_t j=1; j<73; j++){
      if(minmeang_nn[i][j][idep0][k]<=0.) continue;
      if(excluded_min[i][j][idep0][k]<=0){
      nchm++;
      minmean1[i][idep0][k] = minmean1[i][idep0][k] + minmeang_nn[i][j][idep0][k];
      minmean1_nn[i][idep0][k] = minmean1_nn[i][idep0][k] + minmeang_nn[i][j][idep0][k];
      }
     } // j   


 
    minnumchan[i][idep0][k] = nchm;    
          
    if(nchp > 0) plmean1[i][idep0][k] = plmean1[i][idep0][k]/nchp;
    if(nchm > 0) minmean1[i][idep0][k] = minmean1[i][idep0][k]/nchm;
    if(nchp > 0) plmean1_nn[i][idep0][k] = plmean1_nn[i][idep0][k]/nchp;
    if(nchm > 0) minmean1_nn[i][idep0][k] = minmean1_nn[i][idep0][k]/nchm;

    if(i==1 && k==1 )cout<<" i, idep0, k "<<i<<" "<<idep0<<" "<<k<<" "<<nchp<<" "<<nchm<<" "<<plmean1_nn[i][idep0][k]<<" "<<
...
//    " "<<plmeang_nn[i][j][idep0][k]<<" "<<plnoise[i][j][idep0][k]<<endl;
    ...
    if( k != 3 ) {
    for(Int_t j=1;j<73;j++) {
       if( plmeang_nn[i][j][idep0][k] > 0) {       
       Float_t tt0, tt1; 
       if( i != 16 && i != 15 ) {
       if(plmeang_nn[i][j][idep0][k]>0.) {
         tt0 = plmean1_nn[i][idep0][k]/plmeang_nn[i][j][idep0][k];
         tt1 = plmean1[i][idep0][k]/plmeang_nn[i][j][idep0][k];
       }
       } else {       
	                  tt0 = 1.; tt1 = 1.;
       }
...
//    cout<<" Here "<<endl;
    for(Int_t j=1;j<73;j++) {   
       if( minmeang_nn[i][j][idep0][k] > 0 ) {       
       Float_t tt0, tt1;        
       if( i != 16 && i != 15) {
       if(minmeang_nn[i][j][idep0][k]>0.){
       tt0 = minmean1_nn[i][idep0][k]/minmeang_nn[i][j][idep0][k];
       tt1 = minmean1[i][idep0][k]/minmeang_nn[i][j][idep0][k];
       }
