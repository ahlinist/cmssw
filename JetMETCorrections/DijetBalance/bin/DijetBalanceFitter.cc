...
/////////////////////////////////////////////////////////////////////////
  ...
  double pt_vec[MAX_NPtBins],eta_vec[MAX_NETA];
  if (NETA>MAX_NETA)
    {
  ...
  for(i=0;i<=NPtBins;i++)
    pt_vec[i] = DijetPtBoundaries[i];
  for(i=0;i<=NETA;i++)
    eta_vec[i] = EtaBoundaries[i];        
  TFile *inf;
  TFile *outf; 
  TH1F *MeanPt[MAX_NETA];  
  TH1F *RelativeResponse[MAX_NPtBins];
  TH1F *h;
  ...
  for(etabin=0;etabin<NETA;etabin++)
    { 
      ...
      MeanPt[etabin] = new TH1F(name,name,NPtBins,pt_vec);
    }
  for (ptbin=0; ptbin<NPtBins; ptbin++)//loop over DijetPt bins
    { 
