...
#include <string>
  ...
  gROOT->ProcessLine(".! mkdir output_PhotonJets");
  for (map<string, double>::iterator it = signal.begin();
    ...
    gjettree *sig = new gjettree(getchain("inputfiles_tmp.txt"), Form("output_PhotonJets/output_PhotonJets_%s.root", it->first));
    sig->Loop(it->second, true);
    // Add new root tuple to output file list
    gROOT->ProcessLine(Form(".! echo \"output_PhotonJets/output_PhotonJets_%s.root\" >> output_PhotonJets_all.txt", it->first));
    delete sig;
  } // for signal
  ...
  gROOT->ProcessLine(".! mkdir output_QCD");
  for (map<string, double>::iterator it = qcd.begin();
    ...
    // Produce a root tuple for the pT hat bin
    gjettree *bkg =new gjettree(getchain("inputfiles_tmp.txt"), Form("output_QCD/output_QCD_%s.root", it->first));
    bkg->Loop(it->second, false);
    // Add new root tuple to output file list
    gROOT->ProcessLine(Form(".! echo \"output_QCD/output_QCD_%s.root\" >> output_QCD_all.txt", it->first));
    delete bkg;
  } // for qcd
