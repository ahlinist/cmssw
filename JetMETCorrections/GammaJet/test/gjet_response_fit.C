#define gjet_response_fit_cxx
#include "gjet_response_fit.h"
#include <TF1.h>
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void gjet_response_fit::Loop(bool usew)
{
//   In a ROOT session, you can do:
//      Root > .L gjet_response_fit.C
//      Root > gjet_response_fit t
...
void gjet_response_fit::BookHistos()
{
  double bins[56];
  for (int i=0;i<18;i++){
    bins[i] = 30.+ 10*i;
  ...
  for (int i=1;i<4;i++){
    bins[52+i] = 1000.+ 333.3*i;
  }
  for (int i=1;i<56;i++){
    cout << i << "   " << bins[i] << endl;
  }
  
  ptphot_nocut = new TH1D("ptphot_nocut","ptphot_nocut",55, bins);  ptphot_nocut->Sumw2();
  ptphot_allcut = new TH1D("ptphot_allcut","ptphot_allcut",55, bins); ptphot_allcut->Sumw2();
  ...
  responsevspt_nocut = new TH2D("responsevspt_nocut","responsevspt_nocut",55, bins,YBINS,0.,2.); responsevspt_nocut->Sumw2();
  responsevspt_allcut = new TH2D("responsevspt_allcut","responsevspt_allcut",55, bins,YBINS,0.,2.); responsevspt_allcut->Sumw2();
  ...
  ptphot_nocut_sig = new TH1D("ptphot_nocut_sig","ptphot_nocut_sig",55, bins);  ptphot_nocut_sig->Sumw2();
  ptphot_allcut_sig = new TH1D("ptphot_allcut_sig","ptphot_allcut_sig",55, bins); ptphot_allcut_sig->Sumw2();
  ...
  responsevspt_nocut_sig = new TH2D("responsevspt_nocut_sig","responsevspt_nocut_sig",55, bins,YBINS,0.,2.); responsevspt_nocut_sig->Sumw2();
  responsevspt_allcut_sig = new TH2D("responsevspt_allcut_sig","responsevspt_allcut_sig",55, bins,YBINS,0.,2.); responsevspt_allcut_sig->Sumw2();
  ...
  ptphot_nocut_bkg = new TH1D("ptphot_nocut_bkg","ptphot_nocut_bkg",55, bins);  ptphot_nocut_bkg->Sumw2();
  ptphot_allcut_bkg = new TH1D("ptphot_allcut_bkg","ptphot_allcut_bkg",55, bins); ptphot_allcut_bkg->Sumw2();
  ...
  responsevspt_nocut_bkg = new TH2D("responsevspt_nocut_bkg","responsevspt_nocut_bkg",55, bins,YBINS,0.,2.); responsevspt_nocut_bkg->Sumw2();
  responsevspt_allcut_bkg = new TH2D("responsevspt_allcut_bkg","responsevspt_allcut_bkg",55, bins,YBINS,0.,2.); responsevspt_allcut_bkg->Sumw2();
...
void gjet_response_fit::ResetHistos()
{
  ptphot_nocut->Reset();
  ptphot_allcut->Reset();
...
void gjet_response_fit::Optimum()
{
  BookHistos();
  for(int j=0; j<11; j++){
    for(int i=0; i<21; i++){
...
void gjet_response_fit::Fit()
{
  ...
  TH1D *biassum_sig = new TH1D("biassum_sig","biassum",55,bins);
  TH1D *resosum_sig = new TH1D("resosum_sig","resosum",55,bins);
  TH1D *distrsum_sig = new TH1D("distrsum_sig","distrsum",55,bins);
  
  TH1D *biassum_bkg = new TH1D("biassum_bkg","biassum",55,bins);
  TH1D *resosum_bkg = new TH1D("resosum_bkg","resosum",55,bins);
  
  TH1D *biassum_all = new TH1D("biassum_all","biassum",55,bins);
  TH1D *resosum_all = new TH1D("resosum_all","resosum",55,bins);
