...
void L3Correction_transf()
{
 ...
 char  buffer[200];
//  ifstream L3MC("CondFormats/JetMETObjects/data/iCSA08_1PBV2_L3Absolute_Kt6.txt");
//  ifstream L3gamjet("CondFormats/JetMETObjects/data/iCSA08_1PBV2_GammaJet_L3Absolute_Kt6.txt");
 ifstream L3MC("CondFormats/JetMETObjects/data/iCSA08_10PB_L3Absolute_Kt6.txt");
 ifstream L3gamjet("/afs/cern.ch/user/d/delre/public/html/gammajet/csa08/kt_10pb_dij/CSA08_10PB_L2CORDIJ_L3Absolute_kt6.txt");
 float ParMC0, ParMC1, ParMC2, ParMC3, ParMC4, ParMC5, ParMC6, ParMC7, ParMC8;
 float Pargamjet0, Pargamjet1, Pargamjet2, Pargamjet3, Pargamjet4, Pargamjet5, Pargamjet6, Pargamjet7, Pargamjet8;
 int ok(0);
 L3MC.getline(buffer, 200, '\n');
 sscanf(buffer, "%f %f %f %f %f %f %f %f %f", &ParMC0, &ParMC1, &ParMC2, &ParMC3, &ParMC4, &ParMC5, &ParMC6, &ParMC7, &ParMC8);
 L3gamjet.getline(buffer, 200, '\n');
 sscanf(buffer, "%f %f %f %f %f %f %f %f %f", &Pargamjet0, &Pargamjet1, &Pargamjet2, &Pargamjet3, &Pargamjet4, &Pargamjet5, &Pargamjet6, &Pargamjet7, &Pargamjet8);
 TF1 *ratio = new TF1("ratio_Z_my","([4]+[5]/(pow(log10(x),[6])+[7]))/([0]+[1]/(pow(log10(x),[2])+[3]))",10,2000);
 ratio->SetParameter(0,Pargamjet5);
 ratio->SetParameter(1,Pargamjet6);
 ratio->SetParameter(2,Pargamjet7);
 ratio->SetParameter(3,Pargamjet8);
 ratio->SetParameter(4,ParMC5);
 ratio->SetParameter(5,ParMC6);
 ratio->SetParameter(6,ParMC7);
 ratio->SetParameter(7,ParMC8);

 const int NPtBins = 25; 
 char name[100];
 int i,auxi;
 TFile *inf2;
 inf2 = new TFile("/afs/cern.ch/user/d/delre/scratch0/CMSSW/CMSSW_2_0_7_new/src/csa08_finalplots_16_06_10pb_dij_kt.root");
 TH1F *h2;
 sprintf(name,"biassum_sig");
 h2 = (TH1F*)inf2->Get(name);
 TFile *inf;
 inf = new TFile("/afs/cern.ch/user/d/delre/scratch0/CMSSW/CMSSW_2_0_7_new/src/csa08_finalplots_16_06_10pb_dij_kt.root");
 TH1F *h;
 sprintf(name,"biassum_sig");
 h = (TH1F*)inf->Get(name);
 ...
 for(i=0;i<NPtBins;i++)
   { 
     ...
     if (gen_pt>0)//If gen_pt=0 (pathological case) the point is not taken into account.
       {
         Correction[auxi] = gen_pt/calo_pt*ratio->Eval(calo_pt);
