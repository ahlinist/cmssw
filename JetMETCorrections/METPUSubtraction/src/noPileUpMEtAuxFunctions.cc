  ...
  if ( fabs(pfCandidate.charge()) > 0.5 ) {
    ...
    for ( PFCandidateToVertexAssociationMap::const_iterator pfCandToVertexAssociation = pfCandToVertexAssociations.begin();
      ...
      for ( PFCandidateQualityPairVector::const_iterator pfCandidate_vertex = pfCandidates_vertex.begin();
        ...
        if ( deltaR(pfCandidate.p4(), pfCandidate_vertex->first->p4()) < 1.e-2 ) {
...
reversedPFCandidateToVertexAssociationMap reversePFCandToVertexAssociation(const PFCandidateToVertexAssociationMap& pfCandToVertexAssociations)
{
			    ...
			    int& numWarnings, int maxWarnings)
{
  ...
  if ( fabs(pfCandidate->charge()) > 0.5 ) {
    vtxAssociationType = 1;
    typedef std::vector<std::pair<reco::VertexRef, float> > VertexQualityPairVector;
