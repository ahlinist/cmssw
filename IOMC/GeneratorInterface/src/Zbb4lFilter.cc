...
Zbb4lFilter::~Zbb4lFilter()
{

  if (verbosity_) {
    cout<<"=== Zbb->4l filter summary ==="<<endl;
    cout<<" "<<endl;
...
bool Zbb4lFilter::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
	...
	if (abs((*p)->pdg_id()) == leptonID[i]) {
	  ...
	  if (accepted) {
	    ...
	    for (pa = ancestors.begin(); pa != ancestors.end(); ++pa) {	 
	      ...
	      if ((abs((*pa)->pdg_id()) == qqMode_ || (*pa)->pdg_id()==23) && (*pa)->status() == 3) accepted = true;	      
	    }
	  }
