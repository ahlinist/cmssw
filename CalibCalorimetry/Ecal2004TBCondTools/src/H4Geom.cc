 ...
 *   eta range is [0, 84]
 *   phi range is [0, 19]
 *   electronics channel numbering is [0, 1699]
...
ClassImp(H4Geom);  // doxygen needs the ;
...
bool H4Geom::init()
{
  geometry_ = H4Geom::Automn2004;
  return true;
}

//! Retuns the crystal number in the super module for a given
//! tower number in the super module (supermodule notation 1 -> 68) and 
//! crystal number in the tower (readout number 0 -> 24)
...
//! Returns the crystal number in a tower (readout) and tower number 
//! for a given crystal number in the supermodule
...
//! Returns crystal number in electronics numbering from crystal number in 
//! supermodule numbering
int H4Geom::SmNumberingToElectronicsNumbering(int smCrystal) const
{
  int smTower, crystal;
  getTowerCrystalNumber(smTower, crystal, smCrystal);
  int elecChannel = (smTower - 1)*kCrystalsPerTower + crystal;
  return elecChannel;
}

//! Returns crystal number in supermodule numbering from crystal number in 
//! electronics numbering
int H4Geom::ElectronicsNumberingToSmNumbering(int elecChannel) const
{
  int smTower = elecChannel/kCrystalsPerTower + 1;
  int crystal = elecChannel%kCrystalsPerTower;
  int smCrystal = getSMCrystalNumber(smTower, crystal);
  return smCrystal;
}
