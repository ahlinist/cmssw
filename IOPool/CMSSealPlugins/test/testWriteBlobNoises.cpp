...
int main(int argc, char *argv[]){
  ...
  boost::program_options::options_description visible("Usage: testWriteBlobNoises [options] \n");
  visible.add_options()
  ...
  try{
    ...
    for (i = 0; i <numberOfObj ; ++i ){
      BlobNoises* me = new BlobNoises;
      cond::TypedRef<BlobNoises> myref(pooldb,me);
      me->fill(i, bsize);
	myref.markWrite("BlobNoises");
    }
    pooldb.commit();
