// This is CSCRecHitCBuilder.cc


#include <RecoLocalMuon/CSCRecHitC/src/CSCRecHitCBuilder.h>
/*
  #include <RecoLocalMuon/CSCRecHitC/src/CSCHitFromStripOnly.h>
  #include <RecoLocalMuon/CSCRecHitC/src/CSCHitFromWireOnly.h>
  #include <RecoLocalMuon/CSCRecHitC/src/CSCWireSegments.h>
  #include <RecoLocalMuon/CSCRecHitC/src/CSCStripSegments.h>
  #include <RecoLocalMuon/CSCRecHitC/src/CSCMake2DRecHit.h>
  #include <RecoLocalMuon/CSCRecHitC/interface/CSCWireHitCollection.h>
  #include <RecoLocalMuon/CSCRecHitC/interface/CSCStripHitCollection.h>
  #include <RecoLocalMuon/CSCRecHitC/interface/CSCRangeMapForRecHit.h>
*/
 ...
 *
 */
CSCRecHitCBuilder::CSCRecHitCBuilder( const edm::ParameterSet& ps ) {//: geom_(0) {
 ...
 *
 */
CSCRecHitCBuilder::~CSCRecHitCBuilder() {
...
void CSCRecHitCBuilder::printWhereIm(std::string s)
{
  if (debugPrint) 
    std::cout << "[CSCRecHitCBuilder]: " << s << std::endl;
}




void CSCRecHitCBuilder::initializations(const edm::ParameterSet& ps)
{
...
int CSCRecHitCBuilder::whatCscType(CSCDetId myDetId)
{
...
int CSCRecHitCBuilder::isCscON(CSCDetId myDetId)
{
...
*/
void CSCRecHitCBuilder::findValidALCTs(const CSCALCTDigiCollection* alcts, 
...
*/
void CSCRecHitCBuilder::findValidCLCTs(const CSCCLCTDigiCollection* clcts, 
...
*/
void CSCRecHitCBuilder::matchALCTandCLCT(std::vector<int>& keyWG0,
					 std::vector<int>& keyWG1,
					 std::vector<CSCDetId>& keyWGDetId0,
					 std::vector<CSCDetId>& keyWGDetId1,
					 std::vector<int>& keyHS0,
...
}//void CSCRecHitCBuilder::matchALCTandCLCT(





int CSCRecHitCBuilder::sameDetId(const CSCDetId id1, const CSCDetId id2)
{
...
void CSCRecHitCBuilder::update_tab_Pedestals(const CSCStripDigiCollection* strips)
{
  CSCDetId myDetId;
...
double CSCRecHitCBuilder::tabPedestal(const int endcap, 
				      const int station, 
				      const int ring, 
				      const int chamber, 
				      const int layer, 
...
void CSCRecHitCBuilder::quickCoordinate(const CSCStripDigiCollection* strips,
...
int CSCRecHitCBuilder::isKeyWGgood(const int keyWG, const int id)
{
  ...
  for (int ii=0; ii<nWGSegmentsMax; ii++)
    {
      wgSegmentMin[ii] = -1000;
      wgSegmentMax[ii] =  1000;
    }
  ...
  for (int ii=0; ii<tmpIndexMax; ii++)
    {
...
int CSCRecHitCBuilder::defineWGSegmentNum(const int keyWG, const int id, int& nWGSegments)
{
  ...
  if (keyWG > wgSegmentMax[tmpIndexMax-1])
    {
      //std::cout << "large KeyWG = " << keyWG << std::endl;
      wgSegmentNum = tmpIndexMax-1;
    }
...
double CSCRecHitCBuilder::stripWidth(const int wg, const int id) //in mm
{
...
double CSCRecHitCBuilder::stripWidth_ME11(int wg, int strip) //in mm
{
...
double CSCRecHitCBuilder::stripWidth_4_1(const int wg) //in mm
{
...
double CSCRecHitCBuilder::stripWidth_3_1(const int wg) //in mm
{
...
double CSCRecHitCBuilder::stripWidth_2_1(const int wg) //in mm
{
...
double CSCRecHitCBuilder::stripWidth_1_2(const int wg) //in mm
{
...
double CSCRecHitCBuilder::stripWidth_1_3(const int wg) //in mm
{
...
double CSCRecHitCBuilder::stripWidth_23_2(const int wg) //in mm
{
...
double CSCRecHitCBuilder::QuickCoordCorr(double x, double wg, const int id)
{
...
int CSCRecHitCBuilder::defineStripPart(double x)
{
...
double CSCRecHitCBuilder::stripShift(int layer, const int id)
{
...
void CSCRecHitCBuilder::defineLinFitParam5of6(double &slope, 
					      double &constant,
					      const int refLayer,
...
double CSCRecHitCBuilder::defineResid5of6(const double slope, 
...
//aaa
void CSCRecHitCBuilder::trackSegmentLinearFit5of6(std::vector<double> quickCoord,
						  std::vector<double> quickCoordLayer,
						  std::vector<double> quickCoordY,
						  std::vector<double> quickCoordStrip,
						  std::vector<int> quickCoordKeyWG,
...
}//void CSCRecHitCBuilder::trackSegmentLinearFit5of6(...
...
double CSCRecHitCBuilder::tabulatedLinFitSigma(int sp, int hv, const int id)
{
...
double CSCRecHitCBuilder::tabulatedLinFitSigma_1_1(int sp, int hv)
{
  // STATISTICS LIMITED... 
...
double CSCRecHitCBuilder::tabulatedLinFitSigma_2_1(int sp, int hv)
{
...
double CSCRecHitCBuilder::tabulatedLinFitSigma_1_2(int sp, int hv)
{
  // STATISTICS LIMITED...
...
double CSCRecHitCBuilder::tabulatedLinFitSigma_23_2(int sp, int hv)
{
 ...
 *
 */
void CSCRecHitCBuilder::build( const CSCStripDigiCollection* strips, 
			       ...
			       CSCRecHit2DCollection& oc )
{
  ...
  */
  if (0) {
    ...
    if (debug) std::cout << "[CSCRecHitCBuilder]  sorting hits by layer" << std::endl;
    ...
    if (debug) std::cout << "[CSCRecHitCBuilder] Done sorting strip hits" << std::endl;
    ...
    if (debug) std::cout << "[CSCRecHitCBuilder] Done sorting wire hits" << std::endl;
    ...
    if (debug) std::cout << "[CSCRecHitCBuilder] Now trying to build 2D hits" << std::endl;
 ...
 */
/*
  const CSCLayer* CSCRecHitCBuilder::getLayer( const CSCDetId& detId )  {
  if ( !geom_ ) throw cms::Exception("MissingGeometry") << "[CSCRecHitCBuilder::getLayer] Missing geometry" << std::endl;
