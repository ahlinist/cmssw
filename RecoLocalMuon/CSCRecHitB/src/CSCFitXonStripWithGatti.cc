...
float CSCFitXonStripWithGatti::chisqrFromGatti( float x ) {
  ...
  // Compute Gatti function for 3 positions
  getGatti( x );

  float chi2 = 0.;

  // What is the error on these adc counts ?  
  // Assume poisson stats ...

  for ( int i = 0; i < 3; i++) {
	chi2 += (d[i][1] - q[i][1])*(d[i][1] - q[i][1]) / (d[i][1] + q[i][1]);
  }
  return chi2;
}
 ...
 *
 */
void CSCFitXonStripWithGatti::getGatti( float x ) {

  double r = h / stripWidth;

  double g1 = Q_tot * norm * r * atan( sqrt_k_3 * tanh( k_2 * (-x - 1.5)/r ) );
  double g2 = Q_tot * norm * r * atan( sqrt_k_3 * tanh( k_2 * (-x - 0.5)/r ) );
  double g3 = Q_tot * norm * r * atan( sqrt_k_3 * tanh( k_2 * (-x + 0.5)/r ) );
  double g4 = Q_tot * norm * r * atan( sqrt_k_3 * tanh( k_2 * (-x + 1.5)/r ) );
