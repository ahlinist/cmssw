#include <RecoLocalMuon/CSCRecHitUF/src/CSCRecHitUFProducer.h>
...
CSCRecHitUFProducer::CSCRecHitUFProducer( const edm::ParameterSet& ps ) : iev( 0 ) {
  ...
  recHitBuilder_     = new CSCRecHitUFBuilder( ps ); // pass on the Parameter Settings
...
CSCRecHitUFProducer::~CSCRecHitUFProducer()
{
...
void  CSCRecHitUFProducer::produce( edm::Event& ev, const edm::EventSetup& setup )
{
  LogDebug("CSC") << "[CSCRecHitUFProducer::produce] start producing rechits for event " << ++iev;
...
DEFINE_FWK_MODULE(CSCRecHitUFProducer);
