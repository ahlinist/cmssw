void plotHitResAlphaSim(){
  ...
  for (int i = 0; i<n_tot; i++){
    ...
    predm = simTrk->GetBinContent(i);
    prede = simTrk->GetBinError(i);
    hitm = simRec->GetBinContent(i);
    hite = simRec->GetBinError(i);
    ...
    //plot only for TOB layers    
    if(layer->GetBinContent(i)>4.5 && layer->GetBinContent(i)<10.5 ) {
  ...
  plot(alpha2VsAngle_allTOB);
    c1->SaveAs("MCposA_alpha2_Sim.eps");
 // c1->Print("MCposA_alpha2_Sim.pdf","pdf");
 // do epstopdf instead

}

void plot(TGraphErrors* tg, int n)
{
  tg->SetTitle(TString("TOB")+n+TString(" only"));
  tg->SetMarkerColor(4);
