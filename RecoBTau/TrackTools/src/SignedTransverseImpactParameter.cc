...
pair<bool,Measurement1D> SignedTransverseImpactParameter::apply(const TransientTrack & track,
 ...
 const {
  ...
  if(!TSOS.isValid()){
    ...
    theError = sqrt(E1+E2);
    LogDebug("TrackTools") << "Tk error : " << E1 << " , Vtx error : " << E2 << "  tot : " << theError;
...
pair<bool,Measurement1D> SignedTransverseImpactParameter::zImpactParameter ( const TransientTrack & track, 
  ...
  if ( !TSOS.isValid() ) {
    LogDebug("TrackTools") << "====>>>> SignedTransverseImpactParameter::zImpactParameter : TSOS not valid"  ;
  ...
  if ( !statePCA.isValid() ) {
    LogDebug("TrackTools") << "====>>>> SignedTransverseImpactParameter::zImpactParameter : statePCA not valid"  ;
    return pair<bool,Measurement1D> (false,Measurement1D(0.0,0.0)) ;
  }
