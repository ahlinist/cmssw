...
void overlapCheck::rms(TH1F *plot, float min, float max)
{
  if (plot==0) plot = meanDiffs;
  TH1F *h;
  if (min<max) {
    cout <<min<<max<<endl;
    h = new TH1F("h","h",100,min, max);
  } else {
    h = new TH1F("h","h",100,plot->GetMinimum()/sqrt(2), plot->GetMaximum()/sqrt(2));
  }
  TH2F *phiScan = new TH2F("rzScan", "rzScan", 200,plot->GetMinimum(), plot->GetMaximum(), 200, -180, 180);

  for (int i = 1; i < plot->GetEntries()+1;++i)
    {
      //       if (fabs (phiHisto->GetBinContent(i)-3.141592/2)<0.17)
      h->Fill(plot->GetBinContent(i)/sqrt(2));
      phiScan->Fill(plot->GetBinContent(i),phiHisto->GetBinContent(i)/3.141592*180);
    }
  TCanvas * scanC = new TCanvas("resolution","resolution", 600, 600);
  cout << "Mean x: "<<h->GetMean()<<endl;
  cout << "Meadian x: "<<getMedian(h)<<endl;
  cout << "RMS  x: "<<h->GetRMS()<<endl;
  cout << "Fit:\n";
  scanC->cd(1);
  h->Fit("gaus");h->Draw();
  //   scanC->Divide(2,1);
  //   scanC->cd(2);  phiScan->SetMarkerStyle(1);    //20
  //   phiScan->Draw();
}
...
float getMedian(const vector<double>& vx)
{
