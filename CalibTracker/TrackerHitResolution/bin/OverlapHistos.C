...
using namespace std;
   ...
   * Invoke with with: 
	  ...
	  root> ol->Loop(-1)
   */
void OverlapHistos::Loop(int redEntries, int nCut){
  ...
  cout << "Running over binning method: "<<nCut<<endl;
  ...
  cutflow_hitEx = 0;
  cutflow_acceptDet = 0;
  cutflow_acceptLayer = 0;
  cutflow_acceptStero = 0;
  //
  //////////////
  ...
  //sprintf(hn,"TrackMomentum");
  TH1* trackMomentum = new TH1F("TrackMomentum","TrackMomentum",200,0,100);
  trackMomentum->GetXaxis()->SetTitle("Track Momentum");
  //predMomentum->GetYaxis()->SetTitle("<#sigma(#Deltax_{pred}> [#mum]");

  TH1* clusterWidthX0 = new TH1F("clusterWidthX0","clusterWidthX0",100,0,20);
  clusterWidthX0->GetXaxis()->SetTitle("clusterWidthX0");

  TH1* clusterWidthX1 = new TH1F("clusterWidthX1","clusterWidthX1",100,0,20);
  clusterWidthX1->GetXaxis()->SetTitle("clusterWidthX1");

  TH2F* clusterWidthVsOverlap = new TH2F("clusterWidthVsOverlap","clusterWidthVsOverlap",520,0,520,10,0,10);
  clusterWidthVsOverlap->GetXaxis()->SetTitle("Overlap");
  clusterWidthVsOverlap->GetYaxis()->SetTitle("Cluster Width");
        
  TH2F* TrackAnglevsClusterWidth = new TH2F("TrackAnglevsClusterWidth", "Track angle vs Cluster width", 100, 0, 10, 400, -0.75, 0.75);
  TrackAnglevsClusterWidth->GetXaxis()->SetTitle("Cluster Width");
  TrackAnglevsClusterWidth->GetYaxis()->SetTitle("Track Angle");
  
  TH1F* TrackAngle = new TH1F("TrackAngle", " Track angle", 400, -0.75, 0.75);
  TrackAngle->GetXaxis()->SetTitle("Track Angle");

  TTree* HitResTree_;
  HitResTree_ = new TTree("HitResTracks","HitResTracks");

  double TrackAngles;
  double ClusterWidth;

  HitResTree_->Branch("TrackAngles",&TrackAngles,"TrackAngles/D");
  HitResTree_->Branch("ClusterWidth",&ClusterWidth,"ClusterWidth/D");
  ...
  TH1* predErrHisto_BC = new TH1F("predErrHisto_BC", " predErrHisto_BC", 200, 0, 0.3);	
  TH1* EX_BC = new TH1F("EX_BC", " EX_BC", 200, 0, 0.5);	
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    predErrHisto_BC->Fill(predEDeltaX);
    EX_BC->Fill(hitEX[0]);
    if (!cut(nCut)) continue;
  
    decode(detids[0],subdet,layer,part,stereo);
    if ((!acceptDet[0]) && (!acceptDet[subdet])){ cutflow_acceptDet++ ; continue; }
    if ((acceptLayer>0) && (acceptLayer!=layer)){ cutflow_acceptLayer++; continue; }
    if (!acceptStereo[stereo&1]) {  cutflow_acceptStero++; continue; }
...
//       ++((*it).second);
  ...
  /// Note to myself::: Main event cuts come from predEDeltaX and hitEx. check the distributions and you see that not much can be done to improve here. 
  ///                   Main overlap cuts from threshold, if you cout the threshold values most are around 1-4 so we cut nearly all away 

  cout << endl;
  cout << "CUT FLOW TABLE:::::::" << endl;
  cout << "   Total  = "<<cutflow_total << endl;
  cout << "   predEDeltaX = "<<cutflow_predEDeltaX << endl;
  cout << "   edge = "<<  cutflow_edge << endl;
  cout << "   chi2 = "<<  cutflow_chi2 << endl;
  cout << "   momentum = "<<  cutflow_momentum << endl;
  cout << "   path = "<<  cutflow_path << endl;
  cout << "   hitEx = "<<  cutflow_hitEx << endl;
   
  cout << "   acceptDet = "<<  cutflow_acceptDet  << endl;
  cout << "   acceptLayer = "<<  cutflow_acceptLayer << endl;
  cout << "   acceptStero = "<<  cutflow_acceptStero << endl;
  cout << endl;
...
// 	simTrkHistos_.push_back(simTrkHisto);   
	...
	dPreddSimVsdHitdSimHistos_.push_back(dPreddSimVsdHitdSim);
      ...
      curDir->cd();
      clusterWidthVsOverlap->Fill(ind,clusterWidthX[0]);
      ++ind;
    }
  }
  cout << "Overlaps identified (after threshold): " << ind<<endl;
  nbytes = 0; nb = 0;
  ctr=0;
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    trackMomentum->Fill(momentum);
    clusterWidthX0->Fill(clusterWidthX[0]);
    clusterWidthX1->Fill(clusterWidthX[1]);
    //clusterWidthVsOverlap->Fill(ctr,clusterWidthX[0]);
    
    ClusterWidth = clusterWidthX[0];
    TrackAngles = predDX[0];
      
    TrackAnglevsClusterWidth->Fill(clusterWidthX[0], predDX[0]);
    TrackAngle->Fill(predDX[0]);    
...
//       fillWidth(i+1,simTrk,simTrkHisto,10000);
      ...
      //trackMomentum->SetBinContent(i+1,momentum);
...
OverlapHistos::cut(int nCut)
{
  ...
  */
  ...
  cutflow_total ++;
  if ( predEDeltaX>0.0025 ) {cutflow_predEDeltaX++; return false;}
  if ( (edge[0]>0) || (edge[1]>0) ) {cutflow_edge++; return false;}
  if ( TMath::Prob((Double_t)chi2[0],(Int_t)chi2[1])<1.e-3 ){cutflow_chi2++;  return false;}
  ...
  //strip only cuts here:
    ...
    if (nCut == 0 && ( fabs(predDX[0])>0.17 || fabs(predDX[1])>0.17 ))  return false;
    if (nCut == 1 && ( fabs(predDX[0])<0.17 || fabs(predDX[1])<0.17 || fabs(predDX[0])>0.36 || fabs(predDX[1])>0.36 )) return false;
    if (nCut == 2 && ( fabs(predDX[0])<0.36 || fabs(predDX[1])<0.36 || fabs(predDX[0])>0.60 || fabs(predDX[1])>0.60 ))  return false;
    if (nCut == 3 && ( fabs(predDX[0])<0.60 || fabs(predDX[1])<0.60 ))  return false;  //COMMENTED THIS OUT
    //we can also instead cut on the cluster size
    if (nCut == 4 && ( clusterWidthX[0]!=1 || clusterWidthX[1]!=1)) return false;
    if (nCut == 5 && ( clusterWidthX[0]!=2 || clusterWidthX[1]!=2)) return false;
    if (nCut == 6 && ( clusterWidthX[0]!=3 || clusterWidthX[1]!=3)) return false;
    if (nCut == 7 && ( clusterWidthX[0]!=4 || clusterWidthX[1]!=4)) return false;
    

    /*
    if ( momentum < 5 || momentum > 1000 ) return false; //for strips    //CHANGED FROM 15
    if ( fabs(path) > 7 ) return false; //for strips
    if ( hitEX[0] > 0.006 || hitEX[1] > 0.006 ) return false; // for strips
    */

    if ( momentum < 5 || momentum > 1000 ){cutflow_momentum++; return false;} 
    if ( fabs(path) > 7 ){ cutflow_path++; return false; }
    if ( hitEX[0] > 0.006 || hitEX[1] > 0.006 ){cutflow_hitEx++; return false; }
    //if ( hitEX[0] > 0.1 || hitEX[1] > 0.1 ){cutflow_hitEx++; return false; }
