...
L1TOMDSHelper::L1TOMDSHelper() : attempts(0){
...
//_____________________________________________________________________
int L1TOMDSHelper::getFillNumber (int runNumber, int ls ,int &error){
  // Parameters
  string rtlSchema = "CMS_RUNTIME_LOGGER";
  string table     = "LUMI_SECTIONS";
  std::string attribute1 = "RUNNUMBER";
  // Fields to retrieve in the query
  vector<std::string> qStrings ;
  qStrings.push_back("LHCFILL");
  int retval = 0;
  error                 = NO_ERROR;
  l1t::OMDSReader::QueryResults qResults = m_omdsReader->basicQuery(qStrings,rtlSchema,table,attribute1,m_omdsReader->singleAttribute(runNumber));

  // Check query successful
  if(qResults.queryFailed()){error = WARNING_DB_QUERY_FAILED;}
  else{
    if(qResults.numberRows() == 0){error = WARNING_DB_QUERY_FAILED;}
    else
      qResults.fillVariableFromRow("LHCFILL",0,retval);
  }
  return retval;
}
...
//_____________________________________________________________________
float L1TOMDSHelper::getHFDeliveredLumi (int runNumber, int &ls ,int &error){
  // Parameters
  string rtlSchema = "CMS_RUNTIME_LOGGER";
  string table     = "LUMI_SECTIONS";
  std::string attribute1 = "RUNNUMBER";
  // Fields to retrieve in the query
  vector<std::string> qStrings ;
  qStrings.push_back("DELIVLUMISECTION");
  float retval = 0.;
  error                 = NO_ERROR;
  l1t::OMDSReader::QueryResults qResults = m_omdsReader->basicQuery(qStrings,rtlSchema,table,attribute1,m_omdsReader->singleAttribute(runNumber));
  ...
  if(qResults.queryFailed()){error = WARNING_DB_QUERY_FAILED;}
    ...
    if(ls>qResults.numberRows()){
      std::cout << "Query Retrieved " << qResults.numberRows() << " rows" << std::endl;
      ls = qResults.numberRows();
    }
    qResults.fillVariableFromRow("DELIVLUMISECTION",ls-1,retval);
  }
  return retval;
}
...
//_____________________________________________________________________
float L1TOMDSHelper::getHFRecordedLumi (int runNumber, int &ls ,int &error){
  // Parameters
  string rtlSchema = "CMS_RUNTIME_LOGGER";
  string table     = "LUMI_SECTIONS";
  std::string attribute1 = "RUNNUMBER";
  // Fields to retrieve in the query
  vector<std::string> qStrings ;
  qStrings.push_back("LIVELUMISECTION");
  float retval = 0.;
  error                 = NO_ERROR;
  l1t::OMDSReader::QueryResults qResults = m_omdsReader->basicQuery(qStrings,rtlSchema,table,attribute1,m_omdsReader->singleAttribute(runNumber));
  ...
  if(qResults.queryFailed()){error = WARNING_DB_QUERY_FAILED;}
    ...
    qResults.fillVariableFromRow("LIVELUMISECTION",ls-1,retval);
  }
  return retval;
}
