...
void PFPretendTrackProducer::beginJob(const edm::EventSetup& iSetup) {
	...
	transformer_= new PFTrackTransformer(math::XYZVector(magneticField->inTesla(GlobalPoint(0,0,0))));
...
void PFPretendTrackProducer::produce(edm::Event& event,
	...
	std::auto_ptr<PFRecTrackCollection> pfRecTrackColl(new PFRecTrackCollection());
	std::auto_ptr<GsfPFRecTrackCollection>
			gsfpfRecTrackColl(new GsfPFRecTrackCollection());
	std::auto_ptr<PFClusterCollection> pfPSColl(new PFClusterCollection());
	std::auto_ptr<MuonCollection> muonColl(new MuonCollection());
	std::auto_ptr<TrackExtraCollection>
			trackExtrasColl(new TrackExtraCollection());
	...
	TrackExtraRefProd trackExtrasRef =
			event.getRefBeforePut<TrackExtraCollection>();

	TrackExtra extra(originXYZ, dir, true, caloSurface, dir, true, m, 0, m, 1,
			alongMomentum);
	trackExtrasColl->push_back(extra);
	OrphanHandle<TrackExtraCollection> theRecoTrackExtras =
	...
	PFRecTrack pftrackAlternative(track.charge(), reco::PFRecTrack::KF, 0,
			trackRef);
	Trajectory fakeTraj;
	bool valid = transformer_->addPoints(pftrackAlternative, *trackRef, fakeTraj);
	if (!valid)
		std::cout << "Transformer failed to make transformation!" << std::endl;
	if (debug_ > 1) {
		std::cout << "PFRecTrack: " << pftrackAlternative << std::endl;
		const reco::PFTrajectoryPoint
				& atECAL =
						pftrackAlternative.extrapolatedPoint(reco::PFTrajectoryPoint::ECALEntrance);
