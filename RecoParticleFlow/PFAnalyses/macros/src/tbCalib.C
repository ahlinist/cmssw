...
public:
	...
	Fit(double minE, double maxE) :
		minE(minE), maxE(maxE), Denom(0.), NumA(0.), NumB(0.), ACoeff(0.), BCoeff(0.), CCoeff(0.), DCoeff(0.), SigmaA(0.),
				SigmaB(0.), SigmaC(0.), SigmaD(0.), RhoAB(0.) {
	}
	...
	bool fill(double e, double h, double t, double eta, unsigned input = 0) {
		...
		if (t > minE && t < maxE) {
			double thresh = 0.;
			if (input == 0)
				thresh = e > 0 ? (h > 0 ? threshE : threshE0) : threshH;
	...
	bool eMatrices() {
		...
		if (H1(0, 0) != 0.)
			success = success && H1.Invert();
		if (E1(0, 0) != 0.)
			success = success && E1.Invert();
	...
	bool etaMatrices() {
		...
		success = Eta2.Invert() && Eta3.Invert() && Eta4.Invert() && Eta6.Invert();
...
double findABC(std::vector<Fit*>& fits, double tE0, double tE, double tH) {
	...
	double rmsC = 0.;
	unsigned nfit = 0;
	//Jamie - include all available energies. ifit=5 -> ifit = 0
	//and terminates at fits.size(), not size() - 1
	//Actually, this needs some thought. It changes the thresholds a lot.
	for (unsigned ifit = 0; ifit < fits.size(); ++ifit) {
		//if ( !fits[ifit]->size() ) continue;
		if (!fits[ifit]->size())
			cout << __PRETTY_FUNCTION__ << ": Problem!" << endl;
		// cout << "Fit " << ifit << " has " << fits[ifit]->size() << " entries " << endl;
		fits[ifit]->eMatrices();
		//if (ifit<20) continue;
	...
	cout << tE << " " << tH << " RMS 0/A+B/C = " << rms0 << ", " << sqrt(rmsA * rmsA + rmsB * rmsB) << ", " << rmsC
			<< endl;
...
computeBarrelCoefficients(const char* calibFile) {
	...
	TH2F* rawEvsP = new TH2F("rawEvsP", "ECAL+HCAL raw Energy vs P", 301, 0., 301., 150, -1.5, 1.5);
	TH2F* colEvsP = new TH2F("colEvsP", "ECAL+HCAL col Energy vs P", 301, 0., 301., 150, -1.5, 1.5);
	TH2F* jamEvsP = new TH2F("jamEvsP", "ECAL+HCAL jam Energy vs P", 301, 0., 301., 150, -1.5, 1.5);
	...
	if (!isEndcaps) {
		fits.push_back(new Fit(1.5, 2.5));
	}
	for (double i = 2.5; i < 9.5; i = i + 1.0) {
		cout << "Making fit for " << i << ", " << i + 1.0 << "\n";
		fits.push_back(new Fit(i, i + 1.0));
	}
	fits.push_back(new Fit(19.5, 20.5));
	fits.push_back(new Fit(29.5, 30.5));
	fits.push_back(new Fit(49.5, 50.5));
	fits.push_back(new Fit(99.5, 100.5));
	fits.push_back(new Fit(149.5, 150.5));
	if (!isEndcaps)
		fits.push_back(new Fit(199.5, 200.5));
	else
		fits.push_back(new Fit(224.5, 225.5));
	fits.push_back(new Fit(299.5, 300.5));

	//  for ( double bin=3.; bin<10.; bin=bin+1) {
	//    fits.push_back(new Fit(bin,bin+0.5));
	//  }
	//
	//  for ( double bin=10.; bin<400.; bin=bin+10) {
	...
	if (!isEndcaps) {
		...
		for (double te0 = 0.; te0 < 10; te0 = te0 + 0.1) {
			...
			cout << "loop " << te0 << ", rmsCoeff " << rmsCoeff << "\n";
	...
	//Jamie - minor bug, loop terminator should just be fits.size(), not size()-1
	for (unsigned ifit = 0; ifit < fits.size(); ++ifit) {
		if (!fits[ifit]->size())
			continue;
		//fits[ifit]->eMatrices();
		cout << "Bin " << ifit << "; a, b, c, a0  = " << fits[ifit]->E2Coeffs(0) << " +/- " << sqrt(fits[ifit]->E2(0, 0))
				<< ", " << fits[ifit]->E2Coeffs(1) << " +/- " << sqrt(fits[ifit]->E2(1, 1)) << ", " << fits[ifit]->H1Coeffs(0)
				<< " +/- " << sqrt(fits[ifit]->H1(0, 0)) << ", " << fits[ifit]->E1Coeffs(0) << " +/- " << sqrt(fits[ifit]->E1(
				0, 0)) << endl;
	...
	JGraph* gA = new JGraph("ecal", true);
	for (unsigned ai(0); ai < xab.size(); ai++)
		gA->addPoint(xab[ai], sxab[ai], a[ai], sa[ai]);
	gA->streamToFile("ecalCoeffs.dat", true);

	JGraph* gB = new JGraph("hcal", true);
	for (unsigned ai(0); ai < xab.size(); ai++)
		gB->addPoint(xab[ai], sxab[ai], b[ai], sb[ai]);
	gB->streamToFile("hcalCoeffs.dat", true);

	JGraph* gC = new JGraph("hcalOnly", true);
	for (unsigned ai(0); ai < xc.size(); ai++)
		gC->addPoint(xc[ai], sxc[ai], c[ai], sc[ai]);
	gC->streamToFile("hcalOnlyCoeffs.dat", true);
	...
	TH2F *h = new TH2F("AAndB", "", 100, 0., 301., 10, -0.5, 2.5);
	...
	gra->GetXaxis()->SetTitle("Beam momentum (GeV/c)");
	gra->GetXaxis()->SetMoreLogLabels(true);
	grb->GetXaxis()->SetTitle("Beam momentum (GeV/c)");
	grb->GetXaxis()->SetMoreLogLabels(true);
	...
	gPad->SaveAs("ecalHcalCoeff.eps");
	gPad->SaveAs("ecalHcalCoeff.C");
	...
	grc->SetLineWidth(2);
	grc->Draw("P");
	gPad->SaveAs("hcalOnlyCoeff.eps");
	gPad->SaveAs("hcalOnlyCoeff.C");
	cout << "Done fitting b, c coefficients" << endl;

	double paramFa[6];
	double paramFb[6];
	double paramFc[6];
	fa->GetParameters(paramFa);
	fb->GetParameters(paramFb);
	fc->GetParameters(paramFc);
	...
	for (unsigned ip = 0; ip < 10; ++ip) {
		...
		if (!isEndcaps) {
			if (param_fa != 0.)
				cout << "  faBarrel->SetParameter(" << ip << "," << param_fa << ");" << endl;
			if (param_fb != 0.)
				cout << "  fbBarrel->SetParameter(" << ip << "," << param_fb << ");" << endl;
			if (param_fc != 0.)
				cout << "  fcBarrel->SetParameter(" << ip << "," << param_fc << ");" << endl;
		} else {
			if (param_fa != 0.)
				cout << "  faEndcap->SetParameter(" << ip << "," << param_fa << ");" << endl;
			if (param_fb != 0.)
				cout << "  fbEndcap->SetParameter(" << ip << "," << param_fb << ");" << endl;
			if (param_fc != 0.)
				cout << "  fcEndcap->SetParameter(" << ip << "," << param_fc << ");" << endl;
		}

	}
	ofstream file("coeffFunctions.dat");
	if (file.is_open()) {
		file << paramFa[0] << "+(" << paramFa[1] << "+" << paramFa[2] << "/sqrt(x))*exp(-x/" << paramFa[3] << ")-exp(-x^2/"
				<< paramFa[5] << ")*" << paramFa[4] << "\n";
		file << paramFb[0] << "+(" << paramFb[1] << "+" << paramFb[2] << "/sqrt(x))*exp(-x/" << paramFb[3] << ")-exp(-x^2/"
				<< paramFb[5] << ")*" << paramFb[4] << "\n";
		file << paramFc[0] << "+(" << paramFc[1] << "+" << paramFc[2] << "/sqrt(x))*exp(-x/" << paramFc[3] << ")-exp(-x^2/"
				<< paramFc[5] << ")*" << paramFc[4] << "\n";

		file.close();
	} else {
		cout << __PRETTY_FUNCTION__ << ": Unable to open coeffFunctions.dat file\n";
	}

	cout << "//MATLAB equivalents (written to coeffFunctions.dat) :\n";
	cout << "//ecalInt_f = ' " << paramFa[0] << "+ (" << paramFa[1] << " + " << paramFa[2] << "/sqrt(x))*exp(-x/"
			<< paramFa[3] << ") - exp(-x^2/" << paramFa[5] << ")* " << paramFa[4] << "'\n";
	cout << "//hcalInt_f =' " << paramFb[0] << "+ (" << paramFb[1] << " + " << paramFb[2] << "/sqrt(x))*exp(-x/"
			<< paramFb[3] << ") - exp(-x^2/" << paramFb[5] << ")* " << paramFb[4] << "'\n";
	cout << "//hcalMip_f = '" << paramFc[0] << "+ (" << paramFc[1] << " + " << paramFc[2] << "/sqrt(x))*exp(-x/"
			<< paramFc[3] << ") - exp(-x^2/" << paramFc[5] << ")* " << paramFc[4] << "'\n";
	cout << "\n";
	...
	TH2F* etadepJam_100_1000 = new TH2F("etadepJam_100_1000", "Eta Dependence 1 a 10", 150, etamin, etamax, 150, -1.5,
			1.5);
	...
	for (unsigned entry = 0; entry < nEntries; ++entry) {
		...
		for (unsigned ifit = 0; ifit < fits.size(); ++ifit) {
			...
			if (fits[ifit]->fill(a * e, b * h, t, eta))
				break;
	...
	for (unsigned ifit = 4; ifit < fits.size() - 1; ++ifit) {
		...
		cout << "Bin " << ifit << "; a0, b0 = " << "   " << fits[ifit]->Eta2Coeffs(0) << " +/- " << sqrt(fits[ifit]->Eta2(
				0, 0)) << ", " << fits[ifit]->Eta2Coeffs(1) << " +/- " << sqrt(fits[ifit]->Eta2(1, 1)) << endl
				<< "a1, b1, a2, b2 = " << "   " << fits[ifit]->Eta4Coeffs(0) << " +/- " << sqrt(fits[ifit]->Eta4(0, 0)) << ", "
				<< fits[ifit]->Eta4Coeffs(1) << " +/- " << sqrt(fits[ifit]->Eta4(1, 1)) << ", " << fits[ifit]->Eta4Coeffs(2)
				<< " +/- " << sqrt(fits[ifit]->Eta4(2, 2)) << ", " << fits[ifit]->Eta4Coeffs(3) << " +/- " << sqrt(
				fits[ifit]->Eta4(3, 3)) << endl;
	...
	TGraphErrors* graEta1 = new TGraphErrors(xEta.size(), &xEta[0], &aEta1[0], &sxEta[0], &saEta1[0]);
	TGraphErrors* grbEta1 = new TGraphErrors(xEta.size(), &xEta[0], &bEta1[0], &sxEta[0], &sbEta1[0]);
	TGraphErrors* graEta2 = new TGraphErrors(xEta.size(), &xEta[0], &aEta2[0], &sxEta[0], &saEta2[0]);
	TGraphErrors* grbEta2 = new TGraphErrors(xEta.size(), &xEta[0], &bEta2[0], &sxEta[0], &sbEta2[0]);
	...
	TF1* faEta = new TF1("faEta", "[0]+[1]*x+[2]*exp(-x/[3])+[4]*[4]*exp(-x*x/([5]*[5]))", 0, 1000);
	faEta->SetParameters(-0.02, -5E-6, -0.05, 50, +0.2, 22);
	if (etamin > 0.1)
		faEta->FixParameter(1, 0.);
	graEta0->Fit("faEta");
	graEta0->Fit("faEta", "W");
	...
	TF1* faEta1 = etamin < 0.1 ? new TF1("faEta1", "[0]+[1]*x+[2]*exp(-x/[3])", 0, 1000) : new TF1("faEta1",
			"[0]+[1]*x+[2]*exp(-x/[3])+[4]*exp(-x*x/([5]*[5]))", 0, 1000);
	faEta1->SetParameters(-0.05, -2E-5, -0.05, 150, 0.1, 10);
	graEta1->Fit("faEta1");
	graEta1->Fit("faEta1", "W");

	TF1* fbEta1 = etamin < 0.1 ? new TF1("fbEta1", "[0]+[1]*x+[2]*exp(-x/[3])", 0, 1000) : new TF1("fbEta1",
			"[0]+[1]*x+[2]*exp(-x/[3])+[4]*exp(-x*x/([5]*[5]))", 0, 1000);
	...
	TF1* faEta2 = etamin < 0.1 ? new TF1("faEta2", "[0]+[1]*x+[2]*exp(-x/[3])", 0, 1000) : new TF1("faEta2",
			"[0]+[1]*x+[2]*exp(-x/[3])+[4]*exp(-x*x/([5]*[5]))", 0, 1000);
	faEta2->SetParameters(-0.05, -2E-5, -0.05, 150, 0.1, 20);
	graEta2->Fit("faEta2");
	graEta2->Fit("faEta2", "W");

	TF1* fbEta2 = etamin < 0.1 ? new TF1("fbEta2", "[0]+[1]*x+[2]*exp(-x/[3])", 0, 1000) : new TF1("fbEta2",
			"[0]+[1]*x+[2]*exp(-x/[3])+[4]*exp(-x*x/([5]*[5]))", 0, 1000);
	...
	TH2F* etadep_100_1000_Cor = new TH2F("etadep_100_1000_Cor", "Eta Dependence 1 a 10", 150, etamin, etamax, 150, -1.5,
			1.5);
	...
	for (unsigned entry = 0; entry < nEntries; ++entry) {
		...
		double etaCorrE = etaEH ? 1. + faEta1->Eval(t) + fbEta1->Eval(t) * (eta - etamin) * (eta - etamin) : 1.
				+ faEta->Eval(t) + fbEta->Eval(t) * (eta - etamin) * (eta - etamin);
		double etaCorrH = etaEH ? 1. + faEta2->Eval(t) + fbEta2->Eval(t) * (eta - etamin) * (eta - etamin) : 1.
				+ faEta->Eval(t) + fbEta->Eval(t) * (eta - etamin) * (eta - etamin);
		...
		for (unsigned ifit = 0; ifit < fits.size(); ++ifit) {
			if (fits[ifit]->fill(etaCorrE * a * e, etaCorrH * b * h, t, eta))
				break;
		}
	...
	for (unsigned ip = 0; ip < 10; ++ip) {
		...
		if (etamin < 0.1) {
			...
			if (param_fc != 0.)
				cout << "  fcBarrel->SetParameter(" << ip << "," << param_fc << ");" << endl;
			if (param_faEta != 0.)
				cout << "  faEtaBarrel->SetParameter(" << ip << "," << param_faEta << ");" << endl;
			if (param_fbEta != 0.)
				cout << "  fbEtaBarrel->SetParameter(" << ip << "," << param_fbEta << ");" << endl;
		} else if (etamin < 1.7) {
			...
			if (param_fc != 0.)
				cout << "  fcEndcap->SetParameter(" << ip << "," << param_fc << ");" << endl;
			if (param_faEta != 0.)
				cout << "  faEtaEndcap->SetParameter(" << ip << "," << param_faEta << ");" << endl;
			if (param_fbEta != 0.)
				cout << "  fbEtaEndcap->SetParameter(" << ip << "," << param_fbEta << ");" << endl;
		} else {
		}
	...
	if (etamin < 0.1) {
		...
		cout << "  double a = h>0. ? faEndcap->Eval(t) : faEndcap->Eval(t);" << endl
				...
				<< ")*(fabs(eta)-" << etamin << ");" << endl << "  double thresh = e > 0. ? threshE : threshH;" << endl
				<< "  double eCorr = thresh + etaCorr * ( a*e + b*h );" << endl;
	} else {
	}
