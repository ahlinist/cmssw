...
void TestbeamPreprocessor::processEvents(std::string filename, int runNumber) {
	...
	processEvents(runNumber, events);
...
void TestbeamPreprocessor::processCerenkovCounters(TTree* events, RunInfo& ri) {
	...
	if (withMonitoring_) {
		events->Draw("HcalTBBeamCounters_tbunpacker__RECO.obj.CK2adc_>>ck2(100,0,1000)");
		TH1* ck2 = (TH1*) gROOT->FindObject("ck2");
		events->Draw("HcalTBBeamCounters_tbunpacker__RECO.obj.CK3adc_>>ck3(100,0,1000)");
...
void TestbeamPreprocessor::processMuonVetos(TTree* events, RunInfo& ri) {
	...
	if (ri.beamEnergy_ < 10) {
		events->Draw("HcalTBBeamCounters_tbunpacker__RECO.obj.VMFadc_>>vmf(500, 0, 500");
...
void TestbeamPreprocessor::processBeamCounters(TTree* events, RunInfo& ri) {
	...
	events->Draw("HcalTBBeamCounters_tbunpacker__RECO.obj.BH1adc_>>bh1(500,0,500)");
	...
	events->Draw("HcalTBBeamCounters_tbunpacker__RECO.obj.BH2adc_>>bh2(500,0,500)");
	...
	events->Draw("HcalTBBeamCounters_tbunpacker__RECO.obj.BH3adc_>>bh3(500,0,500)");
	...
	events->Draw("HcalTBBeamCounters_tbunpacker__RECO.obj.BH4adc_>>bh4(500,0,500)");
...
void processPositioning(TTree* events, RunInfo& ri) {
	events->Draw("HcalTBEventPosition_tbunpacker__RECO.obj.hbheTableEta_>>tableEta");
	events->Draw("HcalTBEventPosition_tbunpacker__RECO.obj.hbheTablePhi_>>tablePhi");
	TH1* tableEta = (TH1*) gROOT->FindObject("tableEta");
	TH1* tablePhi = (TH1*) gROOT->FindObject("tablePhi");
	ri.tableEta_ = tableEta->GetMean();
	ri.tablePhi_ = tablePhi->GetMean();

}

std::pair<double, double> TestbeamPreprocessor::fitStabilisedGaussian(TH1* histo) {
...
void TestbeamPreprocessor::populateEnergiesAndRuns() {
	...
	int a9GeVs[] = { 29650, 29737, 29738, 30109 };
	int a20GeVs[]= { 30379 };
	int a30GeVs[]= { 30553 };
	int a50GeVs[]= { 30384 };
	int a100GeVs[]= { 30386 };
