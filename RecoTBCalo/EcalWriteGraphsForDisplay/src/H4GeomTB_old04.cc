// 
// File H4GeomTB_old04.cxx
//
/*! \class H4GeomTB_old04
 ...
 *
 * The H4GeomTB_old04 class is providing methods to convert these numbers into
 * each other.
*/
...
const int H4GeomTB_old04::crystalChannelMap[5][5] = {
  { 4, 5, 14, 15, 24},
  { 3, 6, 13, 16, 23},
  { 2, 7, 12, 17, 22},
  { 1, 8, 11, 18, 21},
...
const int H4GeomTB_old04::crystalMap[25] = {
  340, 255, 170,  85,   0,
    1,  86, 171, 256, 341,
  342, 257, 172,  87,   2,
    3,  88, 173, 258, 343,
...
const int H4GeomTB_old04::WhichHalf[69] = {
...
H4GeomTB_old04::GeomPeriod_t H4GeomTB_old04::geometry_ = H4GeomTB_old04::Undef;

// initializes geometry since we always use Automn2004
// (previously done in init() )
H4GeomTB_old04::H4GeomTB_old04()
{ 
  GeomPeriod_t ForEcalMonitoring;    ForEcalMonitoring = Automn2004;
  H4GeomTB_old04::SetGeomPeriod(ForEcalMonitoring);
}
...
bool H4GeomTB_old04::init()
{
  //   if (gConfigParser->isDefined("Geometry::Period"))
  //     geometry_ = H4GeomTB_old04::GeomPeriod_t(gConfigParser->
...
int H4GeomTB_old04::getSMCrystalNumber(int tower, int crystal) const 
{
  if (!IsGeomPeriodDefined()) 
    {cout << "[H4GeomTB_old04] geometry period not defined, aborting.\t (should be  ForEcalMonitoring = Automn2004;)" << endl;
      abort();}

  if (geometry_ == H4GeomTB_old04::Spring2004) {
    int smCrystalNbr = 0;
    if ((crystal/5)%2 == 0)
	...
	crystal%kChannelsPerCard + (tower - 1)*kCrystalsPerTower;
    return smCrystalNbr;
  } else if (geometry_ == H4GeomTB_old04::Automn2004) {
...
//! Retuns the crystal number in the super module for a given
//! tower number in the super module and crystal number in the tower
int H4GeomTB_old04::getSMCrystalNumber(int tower, int strip_id, int crystal_id) const {
  if (
    ...
    {
      //      cout << "[H4GeomTB_old04][getSMCrystalNumber] doing nothing for now" << endl;
      ...
      return 
	H4GeomTB_old04::getSMCrystalNumber(tower, crystalInTower); // should be ok for TT right
    }
}
...
//! Retuns the crystal number in the super module for a given
//! tower number in the super module and crystal number in the tower
void H4GeomTB_old04::getTowerStripChannelNumber(int& tower, int& strip_id, int& crystal_id, int sm_num) const {
  int CryNum;
...
void H4GeomTB_old04::getTowerCrystalNumber(int &tower, int &crystal, int smCrystal) const 
{
  if (!IsGeomPeriodDefined()) 
    {cout << "[H4GeomTB_old04] geometry period not defined, aborting.\t (should be  ForEcalMonitoring = Automn2004;)" << endl;
      abort();}
...
//! see scheme in H4GeomTB_old04::getTower)
int H4GeomTB_old04::getTowerCrystalNumber(int smTowerNb, int crystalNbGeom) const
{
...
//! crystal number in the super module
void H4GeomTB_old04::getCrystalCoord(int &eta, int &phi, int smCrystal) const
{
  if (geometry_ == H4GeomTB_old04::Spring2004) {
...
int H4GeomTB_old04::getSMCrystalFromCoord(int eta, int phi) const
{
  if (geometry_ == H4GeomTB_old04::Spring2004) {
  ...
  if (eta < 0 || eta >= kCrystalsInEta || phi < 0 || phi >= kCrystalsInPhi) 
    ...
    if (geometry_ == H4GeomTB_old04::Automn2004)
      return eta*kChannelsPerCard*kTowersInPhi + phi + 1;
    else
...
int H4GeomTB_old04::getLeft(int smCrystal) const
{
  if (!IsGeomPeriodDefined()) 
    {cout << "[H4GeomTB_old04] geometry period not defined, aborting.\t (should be  ForEcalMonitoring = Automn2004;)" << endl;
      abort();}
...
//! Input and output are crystal numbers in the super module.
//! A negative output means outside of the supermodule.
int H4GeomTB_old04::getRight(int smCrystal) const
{
  if (!IsGeomPeriodDefined()) 
    {cout << "[H4GeomTB_old04] geometry period not defined, aborting.\t (should be  ForEcalMonitoring = Automn2004;)" << endl;
      abort();}
...
//! Input and output are crystal numbers in the super module.
//! A negative output means outside of the supermodule.
int H4GeomTB_old04::getUpper(int smCrystal) const{
  if (!IsGeomPeriodDefined()) 
    {cout << "[H4GeomTB_old04] geometry period not defined, aborting.\t (should be  ForEcalMonitoring = Automn2004;)" << endl;
      abort();}
  if (geometry_ == H4GeomTB_old04::Spring2004) {
...
//! Input and output are crystal numbers in the super module.
//! A negative output means outside of the supermodule.
int H4GeomTB_old04::getLower(int smCrystal) const
{
  if (!IsGeomPeriodDefined()) 
    {cout << "[H4GeomTB_old04] geometry period not defined, aborting.\t (should be  ForEcalMonitoring = Automn2004;)" << endl;
      abort();}
...
void H4GeomTB_old04::mvLeft(int &eta, int &phi) const
{
  if (eta >= kCrystalsInEta)
    std::cout << "H4GeomTB_old04::mvLeft: eta is too large " << eta << std::endl;
  eta++;
}
...
//! New coordonates overwrite the old ones. No check is done to see
//! if it corresponds to a real crystal. To be used with caution. 
void H4GeomTB_old04::mvRight(int &eta, int &phi) const
{
  if (eta < 0)
    std::cout << "H4GeomTB_old04::mvRight: eta is too small " << eta << std::endl;
  eta--;
}
...
//! New coordonates overwrite the old ones. No check is done to see
//! if it corresponds to a real crystal. To be used with caution. 
void H4GeomTB_old04::mvUp(int &eta, int &phi) const
{
  if (phi >= kCrystalsInPhi)
    std::cout << "H4GeomTB_old04::mvUp: phi is too large " << phi << std::endl;
  phi++;
}
...
//! New coordonates overwrite the old ones. No check is done to see
//! if it corresponds to a real crystal. To be used with caution. 
void H4GeomTB_old04::mvDown(int &eta, int &phi) const
{
  if (phi < 0)
    std::cout << "H4GeomTB_old04::mvDown: phi is too small " << phi << std::endl;
  phi--;
}
 ...
 * \endverbatim
 */ 
void H4GeomTB_old04::getTower(int * tower, int towerNb, std::string order) const 
{
  if (!IsGeomPeriodDefined()) 
    {cout << "[H4GeomTB_old04] geometry period not defined, aborting.\t (should be  ForEcalMonitoring = Automn2004;)" << endl;
      abort();}
 ...
 * \endverbatim
 */ 
void H4GeomTB_old04::getVFE(int * VFE, int smCrystal, std::string order) const 
{
 ...
 * \endverbatim
 */
void H4GeomTB_old04::getWindow(int * window, int smCrystal, int width, int height) const
{
  if (width <= 0 || width%2 == 0) {
    std::cout << "H4GeomTB_old04::getWindow, width should be >0 and odd!" << std::endl;
    return;
  }
  if (height <= 0 || height%2 == 0) {
    std::cout << "H4GeomTB_old04::getWindow, height should be >0 and odd!" 
	      << std::endl;
    return;
  }
  int eta, phi;
 ...
 * \endverbatim
 */ 
bool H4GeomTB_old04::rightTower(int tower) const 
{
  if (!IsGeomPeriodDefined()) 
    {cout << "[H4GeomTB_old04] geometry period not defined, aborting.\t (should be  ForEcalMonitoring = Automn2004;)" << endl;
      abort();}
 ...
 * \endverbatim
 */ 
bool H4GeomTB_old04::leftTower(int tower) const
{
  return !rightTower(tower);
}

void H4GeomTB_old04::SetGeomPeriod(GeomPeriod_t geometry)
{
  geometry_ = geometry;
}

bool H4GeomTB_old04::IsGeomPeriodDefined() const {
  if (geometry_ == H4GeomTB_old04::Undef) {
    std::cout << "Class H4GeomTB_old04 : geometry is not defined. You should call in"
	      << " your program H4GeomTB_old04::init (and fill the [Geometry] section"
	      << " of your config file) or call H4GeomTB_old04::SetGeomPeriod." 
	      << std::endl;
    return false;
  }
  return true;
...
int H4GeomTB_old04::getHalf (int TT){
