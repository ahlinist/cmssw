...
void CompositePtrCandidateT1T2MEtHistManager<T1,T2>::bookHistogramsImp()
{
  ...
  hGenLeg1Pt_ = book1D("GenLeg1Pt", "gen. leg_{1} P_{T}",  125, 0., 250.);
  hGenLeg1Phi_ = book1D("GenLeg1Phi", "gen. leg_{1} #phi", 36, -TMath::Pi(), TMath::Pi());
  hGenLeg1Eta_ = book1D("GenLeg1Eta", "gen. leg_{1} #eta", 52, -2.6, 2.6);

  hGenLeg2En_ = book1D("GenLeg2En", "gen. leg_{2} Energy", 125, 0., 250.);
  hGenLeg2Pt_ = book1D("GenLeg2Pt", "gen. leg_{2} P_{T}",  125, 0., 250.);
  hGenLeg2Phi_ = book1D("GenLeg2Phi", "gen. leg_{2} #phi", 36, -TMath::Pi(), TMath::Pi());
  hGenLeg2Eta_ = book1D("GenLeg2Eta", "gen. leg_{2} #eta", 52, -2.6, 2.6);
...
void CompositePtrCandidateT1T2MEtHistManager<T1,T2>::fillHistogramsImp(const edm::Event& evt, const edm::EventSetup& es, double evtWeight)
{
  ...
  for ( typename CompositePtrCandidateCollection::const_iterator diTauCandidate = diTauCandidates->begin();
    ...
    if ( diTauCandidate->p4Leg1gen().energy() > epsilon &&
      ...
      hGenLeg1Pt_->Fill(diTauCandidate->p4Leg1gen().pt(), weight);
      hGenLeg1Phi_->Fill(diTauCandidate->p4Leg1gen().phi(), weight);
      hGenLeg1Eta_->Fill(diTauCandidate->p4Leg1gen().eta(), weight);

      hGenLeg2En_->Fill(diTauCandidate->p4Leg2gen().energy(), weight);
      hGenLeg2Pt_->Fill(diTauCandidate->p4Leg2gen().pt(), weight);
      hGenLeg2Phi_->Fill(diTauCandidate->p4Leg2gen().phi(), weight);
      hGenLeg2Eta_->Fill(diTauCandidate->p4Leg2gen().eta(), weight);
