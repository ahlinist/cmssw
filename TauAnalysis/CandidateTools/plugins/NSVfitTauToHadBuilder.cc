...
class NSVfitTauToHadBuilder : public NSVfitTauDecayBuilderBase {
  public:
    ...
    NSVfitTauDecayHypothesis* buildSpecific(
      ...
      NSVfitTauToHadHypothesis* hypothesis =
        new NSVfitTauToHadHypothesis(particle, label, barcode);
      return hypothesis;
    }
    ...
    virtual int getDecayMode(const reco::Candidate* cand) const {
      const pat::Tau* tauPtr = dynamic_cast<const pat::Tau*>(cand);
      assert(tauPtr);
    ...
    virtual void applyFitParameterSpecific(
        NSVfitTauDecayHypothesis* hypothesis, double *param) const {
