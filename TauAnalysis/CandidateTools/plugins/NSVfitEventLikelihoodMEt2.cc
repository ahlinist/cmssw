...
{
  std::list<const reco::PFCandidate*> pfCandidatesToBeRemoved;
  
  for ( std::list<const reco::PFCandidate*>::const_iterator pfCandidate = pfCandidates.begin();
    ...
    if ( isOverlap ) pfCandidatesToBeRemoved.push_back(*pfCandidate);
  }
  
  for ( std::list<const reco::PFCandidate*>::const_iterator pfCandidateToBeRemoved = pfCandidatesToBeRemoved.begin();
	pfCandidateToBeRemoved != pfCandidatesToBeRemoved.end(); ++pfCandidateToBeRemoved ) {
    pfCandidates.remove(*pfCandidateToBeRemoved);
  }
}
...
{
  std::list<const reco::PFJet*> pfJetsToBeRemoved;

  for ( std::list<const reco::PFJet*>::const_iterator pfJet = pfJets.begin();
	pfJet != pfJets.end(); ++pfJet ) {
    bool isOverlap = false;
    ...
    if ( isOverlap ) pfJetsToBeRemoved.push_back(*pfJet);
  }
  
  for ( std::list<const reco::PFJet*>::const_iterator pfJetToBeRemoved = pfJetsToBeRemoved.begin();
	pfJetToBeRemoved != pfJetsToBeRemoved.end(); ++pfJetToBeRemoved ) {
    pfJets.remove(*pfJetToBeRemoved);
  }
}
...
void NSVfitEventLikelihoodMEt2::beginCandidate(const NSVfitEventHypothesis* hypothesis) const
{
  ...
  removePFJetOverlaps(pfJetList_hypothesis, daughterHypothesesList, dRoverlapPFJet_, dRoverlapPFCandidate_);  

  std::list<const reco::PFCandidate*> pfCandidateList_hypothesis = pfCandidateList_;
  removePFCandidateOverlaps(pfCandidateList_hypothesis, daughterHypothesesList, dRoverlapPFCandidate_);
 
  std::vector<metsig::SigInputObj> pfMEtSignObjects;
  addPFMEtSignObjects(pfMEtSignObjects, daughterHypothesesList);
...
//    in case resolution matrix cannot be inverted
  ...
  pfMEtCovInverse_ = pfMEtCov.Invert();
}

template <typename T>
void NSVfitEventLikelihoodMEt2::addPFMEtSignObjects(std::vector<metsig::SigInputObj>& metSignObjects, 
...
//
  ...
  TVectorD residual_fitted(2);
  residual_fitted(0) = hypothesis->dp4MEt_fitted().px();
  residual_fitted(1) = hypothesis->dp4MEt_fitted().py();
  if ( this->verbosity_ ) {
    std::cout << " pxResidual_fitted = " << residual_fitted(0) << std::endl;
    std::cout << " pyResidual_fitted = " << residual_fitted(1) << std::endl;
  }
