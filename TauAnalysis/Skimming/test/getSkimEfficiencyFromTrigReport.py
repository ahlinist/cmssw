...
### Second loop on muCaloTauFulter
    ...
    # Reg. Expr. patterns to find right lines
    fullEntryPattern = re.compile('TrigReport ---------- Modules in Path: '+PathName2+' ----[\t\n\r\f\v\W\w]+TrigReport ------ Modules in End-Path:')
    ppat = 'TrigReport [\W\w]+ '
    p = []
    for i in range(0,len(Filters2)):
        p.append(re.compile(ppat+Filters2[i]))
    ...
    if (float(NEvents!=0)):
        eff = float(NPassed[0])/float(NEvents)
    print '%24s: %8d     %5.3f       %5.3f'%(Filters2[0],NPassed[0],float(NPassed[0])/float(NEvents),float(NPassed[0])/float(NEvents))
    for i in range(1,len(Filters2)):
        if (float(NPassed[i-1]==0)):
            eff = -1.
        else:
            eff = float(NPassed[i])/float(NPassed[i-1])                
        print '%24s: %8d     %5.3f       %5.3f'%(Filters2[i],NPassed[i],eff,float(NPassed[i])/float(NEvents))
    print '---------------------------------------------------------------'
    last = len(Filters2)-1
    print '%24s: %8d                 %5.3f'%('Total',NPassed[last],float(NPassed[last])/float(NEvents))
