    ...
    error_(0)
{
  ...
  for ( vstring::const_iterator isoType_string = isoTypes_string.begin();
    ...
    else if ( (*isoType_string) == "PfChargedHadronIso" ) isoType = pat::ChargedParticleIso;
    else if ( (*isoType_string) == "PfNeutralHadronIso" ) isoType = pat::NeutralParticleIso;
    else if ( (*isoType_string) == "PfGammaIso" ) isoType = pat::GammaParticleIso;
/*
    // CMSSW_3_1_x 
