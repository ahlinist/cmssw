...
//    as expected for binning used by generalized matrix method
  if ( (int)binGrid->numBins() != (2 << binGrid->dimensions()) ) {
  ...
  for ( unsigned iDimension = 0; iDimension < numDimensions; ++iDimension ) {
    ...
    std::vector<double> binCenter1 = binGrid->binCenter(2 << iDimension);
    ...
    for ( unsigned iBin_ref = 0; iBin_ref < numBins; ++iBin_ref ) {
      ...
      if ( (iBin_ref & (2 << iDimension)) == 1 ) continue;
      ...
      unsigned iBin_flip = iBin_ref + (2 << iDimension);
