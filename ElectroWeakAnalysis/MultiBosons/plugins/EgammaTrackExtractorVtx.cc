#include "ElectroWeakAnalysis/MultiBosons/plugins/EgammaTrackExtractorVtx.h"
...
EgammaTrackExtractorVtx::EgammaTrackExtractorVtx( const ParameterSet& par ) :
...
reco::IsoDeposit::Vetos EgammaTrackExtractorVtx::vetos(const edm::Event & ev,
...
reco::IsoDeposit::Veto EgammaTrackExtractorVtx::veto(const reco::IsoDeposit::Direction & dir) const
{
...
IsoDeposit EgammaTrackExtractorVtx::deposit(const Event & event, const EventSetup & eventSetup, const Candidate & candTk) const
{
    static std::string metname = "EgammaIsolationAlgos|EgammaTrackExtractorVtx";
	...
	if ( !theBeamlineOption.compare("UseVertex") ) {
	  if ( candTk.isElectron() ) {
	    const reco::GsfElectron* elec = dynamic_cast<const reco::GsfElectron*>(&candTk);
	    if(fabs(itrTr->dxy( elec->gsfTrack()->vertex() ) ) > theDiff_r) 
	      continue;
	  } else {
	    if(fabs(itrTr->dxy( candTk.vertex() ) ) > theDiff_r)
	      continue;
	  }
	} else {
...
DEFINE_EDM_PLUGIN(IsoDepositExtractorFactory, EgammaTrackExtractorVtx,  "EgammaTrackExtractorVtx");
