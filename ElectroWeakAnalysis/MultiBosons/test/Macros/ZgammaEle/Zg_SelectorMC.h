...
//   TBranch        *b_muChambers;   //!
   ...
   Zg_SelectorMC(Int_t sample_index);
   virtual ~Zg_SelectorMC();
...
// used to generate this class and read the Tree.
   ...
   while (flag != -1){ 
      ...
      flag=fscanf(Table, "%s", tmp);
      flag=fscanf(Table, "%s", tmp);
      if (nfile==sample_index) {
        LoadFileName=Form("%s",filename);
        cout<<" --> File path:"<<LoadFileName<<endl;
      }
      nfile++;
   }
   fclose(Table);

   TFile *f = new TFile(LoadFileName);
   f->cd(LoadFileName+":/VgAnalyzerKit");
   tree = (TTree*)gDirectory->Get("EventTree");

   Init(tree);
}

Zg_SelectorMC::~Zg_SelectorMC()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t Zg_SelectorMC::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Zg_SelectorMC::LoadTree(Long64_t entry)
{
...
void Zg_SelectorMC::Init(TTree *tree)
{
...
void Zg_SelectorMC::Show(Long64_t entry)
{
...
Int_t Zg_SelectorMC::Cut(Long64_t entry)
{
