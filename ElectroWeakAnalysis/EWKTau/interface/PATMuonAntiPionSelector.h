...
public:
  ...
  void select( const HandleToCollection & hc, 
    ...
    for(edm::View<pat::Muon>::const_iterator trk = muons.begin();trk != muons.end(); ++trk) {
      bool goodMuon = true;
      if(((CaloCompCoefficient_ * muonid::getCaloCompatibility(*trk))+
          (SegmCompCoefficient_ * muonid::getSegmentCompatibility(*trk)))<=AntiPionCut_) {goodMuon = false;}
      if (goodMuon) {selected_.push_back( & * trk );}
      std::cout << "caloCompatibility :  " << muonid::getCaloCompatibility(*trk) << std::endl;
      std::cout << "segmentCompatibility :  " << muonid::getSegmentCompatibility(*trk) << std::endl;
