...
{ 
  FUi2oSender *sd = (FUi2oSender *)SMfudata.app;
  sd->setDestinations();
...
//--------------------------------------------------------------
FUi2oSender::FUi2oSender(xdaq::ApplicationStub * s)
  throw (xdaq::exception::Exception): xdaq::Application(s)
{
  ...
  LOG4CPLUS_INFO(this->getApplicationLogger(),"Making FUi2oSender");
  // Create a memory pool

  std::string poolName = "FUi2oPool";
  ...
  try{
    ...
    getApplicationDescriptors("StorageManager"); // hardwire here for now
  }
  ...
  xgi::bind(this,&FUi2oSender::defaultWebPage, "Default");
  xgi::bind(this,&FUi2oSender::css, "styles.css");
}

void FUi2oSender::setDestinations()
{
  ...
  {
    LOG4CPLUS_ERROR(this->getApplicationLogger(),
                    "FUi2oSender::No receiver in configuration");
  }
}

void FUi2oSender::defaultWebPage(xgi::Input *in, xgi::Output *out)
...
extern "C" xdaq::Application * instantiate_FUi2oSender(xdaq::ApplicationStub * stub )
{
        std::cout << "Going to construct a FUi2oSender instance " << endl;
        return new FUi2oSender(stub);
}
...
toolbox::mem::Reference
    *FUi2oSender::createI2OFragmentChain(char *rawData,
                                         ...
                                         unsigned int& numBytesSent)
{
  ...
  // is the case for FUStreamerI2OWriter (test performed during ctor)
  ...
  FUi2oSender::waitForPoolSpaceIfNeeded(fragmentPool);
    ...
    for(int fragIdx = 0; fragIdx < (int)fragmentCount; ++fragIdx)
    {
      ...
      catch (xdaq::exception::ApplicationDescriptorNotFound excpt)
      {
        edm::LogError("createI2OFragmentChain") 
                      << "FUi2oSender::createI2OFragmentChain: exception in "
                      << "getting source tid "
      ...
      {
        edm::LogError("createI2OFragmentChain") 
                  << "FUi2oSender::createI2OFragmentChain: "
      ...
      if(url.size() > MAX_I2O_SM_URLCHARS)
      {
        edm::LogInfo("createI2OFragmentChain")
                     << "FUi2oSender: Error! Source URL truncated";
      ...
      {
        edm::LogInfo("createI2OFragmentChain")
                     << "FUi2oSender: Error! Source ClassName truncated";
        copySize = MAX_I2O_SM_URLCHARS;
      }
      else
      {
 ...
 */
void FUi2oSender::debugFragmentChain(toolbox::mem::Reference *head)
{
  cout << "FUi2oSender::debugFragmentChain for "
       << hex << head << dec << endl;
 ...
 */
void FUi2oSender::waitForPoolSpaceIfNeeded(toolbox::mem::Pool *targetPool)
{
