/*
   ...
   $Id: FUStreamerI2OWriter.cc,v 1.1 2007/02/04 06:15:24 hcheung Exp $
*/
...
namespace edm
{
  ...
  {
    I2OStreamWorker(const std::string& s, const int dsi);
  ...
  I2OStreamWorker::I2OStreamWorker(const std::string& s = "StorageManager", const int dsi = -1):
                      destinationName_(s),
                      app_(getMyXDAQPtr()),
                      pool_(getMyXDAQPool())
  {
    ...
    FDEBUG(9) << "FUStreamerI2OWriter: Making I2OStreamWorker" << std::endl;
  }

  // ----------------------------------

  FUStreamerI2OWriter::FUStreamerI2OWriter(edm::ParameterSet const& ps):
    worker_(new I2OStreamWorker(ps.getParameter<std::string>("DestinationName"),
				ps.getUntrackedParameter<int>("smInstance",-1))),
    i2o_max_size_(ps.getUntrackedParameter<int>("i2o_max_size",I2O_MAX_SIZE))
  {
    FDEBUG(10) << "FUStreamerI2OWriter: Constructor" << std::endl;
    ...
    if(i2o_max_size_ > I2O_ABSOLUTE_MAX_SIZE) {
      ...
      edm::LogWarning("FUStreamerI2OWriter") <<
    ...
    FDEBUG(9) <<"FUStreamerI2OWriter: Ctor i2o_max_size_: "<< i2o_max_size_ << std::endl;
    FDEBUG(9) <<"FUStreamerI2OWriter: Ctor max_i2o_sm_datasize_: "<< max_i2o_sm_datasize_ << std::endl;
    FDEBUG(9) <<"FUStreamerI2OWriter: Ctor max_i2o_registry_datasize_: "<< max_i2o_registry_datasize_ << std::endl;
    FDEBUG(9) <<"FUStreamerI2OWriter: Ctor max_i2o_other_datasize_: "<< max_i2o_other_datasize_ << std::endl;
    FDEBUG(9) <<"FUStreamerI2OWriter: Ctor max_i2o_DQM_datasize_: "<< max_i2o_DQM_datasize_ << std::endl;
    FDEBUG(9) << "FUStreamerI2OWriter: constructor" << endl;
  }
  
  FUStreamerI2OWriter::~FUStreamerI2OWriter()
  {
    FDEBUG(9) << "FUStreamerI2OWriter: FUStreamerI2OWriter destructor" << endl;
    delete worker_;
  }

  void FUStreamerI2OWriter::doOutputHeader(InitMsgBuilder const& initMessage)
  {
    writeI2ORegistry(initMessage);
    // what happens to memory when initMessage was passed as an auto_ptr?
  }

  void FUStreamerI2OWriter::doOutputEvent(EventMsgBuilder const& eventMessage)
  {
    // First test if memory pool can hold another event
    FDEBUG(10) << "FUStreamerI2OWriter: write event to destination" << std::endl;
  ...
  {
    FDEBUG(9) << "FUStreamerI2OWriter: sending terminate run" << std::endl;
    edm::LogInfo("FUStreamerI2OWriter") << "stop called";
    // make a DONE message
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  void FUStreamerI2OWriter::writeI2ORegistry(InitMsgBuilder const& initMessage)
  {
    ...
    if(head != NULL) {
      FDEBUG(10) << "FUStreamerI2OWriter::WriteI2ORegistry: checking if destination exist" << std::endl;
      if(worker_->destination_ !=0)
      {
        FDEBUG(10) << "FUStreamerI2OWriter::WriteI2ORegistry: posting registry frame " << std::endl;
        try{
        ...
        edm::LogError("writeI2ORegistry") << "FUStreamerI2OWriter::WriteI2ORegistry: No " 
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  ...
  void FUStreamerI2OWriter::writeI2OData(EventMsgBuilder const& eventMessage)
  {
    ...
    FDEBUG(10) << "FUStreamerI2OWriter::writeI2OData: data size (in bytes) = " << size << std::endl;
    FDEBUG(10) << "FUStreamerI2OWriter::writeI2OData: run, event = " 
    ...
    if(head != NULL) {
      if(worker_->destination_ !=0)
      {
        FDEBUG(10) << "FUStreamerI2OWriter::writeI2OData: posting data chain frame " << std::endl;
        try{
          worker_->app_->getApplicationContext()->postFrame(head,
                   worker_->app_->getApplicationDescriptor(),worker_->destination_);
        }
        ...
        edm::LogError("writeI2ORegistry") 
                      << "FUStreamerI2OWriter::writeI2OData: No " << worker_->destinationName_
                      << "destination in configuration";
    }
    // Keep the note below!
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
  void FUStreamerI2OWriter::writeI2OOther(OtherMessageBuilder othermsg)
  {
    char* buffer = (char*) othermsg.startAddress();
    unsigned int size = othermsg.size();
    FDEBUG(9) << "FUStreamerI2OWriter: write other message size = " << size << std::endl;
    // Use common chain creation routine
    toolbox::mem::Reference *head = 0;
    unsigned int numBytesSent = 0;
    head = FUi2oSender::createI2OFragmentChain(buffer, size,
    ...
    if(head != NULL) {
      if(worker_->destination_ !=0)
      {
        FDEBUG(10) << "FUStreamerI2OWriter::writeI2OOther: posting data chain frame " << std::endl;
        try{
          worker_->app_->getApplicationContext()->postFrame(head,
                   worker_->app_->getApplicationDescriptor(),worker_->destination_);
        }
        ...
        edm::LogError("writeI2OOther") 
                      << "FUStreamerI2OWriter::writeI2OOther: No " << worker_->destinationName_
                      << "destination in configuration";
    }
  }
