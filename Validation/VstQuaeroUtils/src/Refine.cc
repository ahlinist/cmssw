      ...
      if(experiment=="l3")
	{
	  ...
	  */
	  ...
	  if((e.getEventType()!="data")&&(level>=5))
	    for(size_t i=0; i<o.size(); i++)
	      {
		HepLorentzVector p = o[i].getFourVector();
		double pt = p.perp();
		double phi = p.phi()+Math::gasdev(0,0.005);
		p = HepLorentzVector(p.e(), Hep3Vector(pt*cos(phi),pt*sin(phi),p.pz()));
      ...
      if(experiment=="aleph")
	{
	  ...
	  if((e.numberOfObjects("mu+",10)==1)&&
	     (e.numberOfObjects("mu-",10)==1))
	    {
	      ...
	      HepLorentzVector mu2 = e.getThisObject("mu-",1)->getFourVector();
	      //	      double m = (mu1+mu2).m();
	  ...
	  if((e.numberOfObjects("e+",10)==1)&&
	     ...
	     (e.numberOfObjects("ph",10)==1))
	    {
	      ...
	      HepLorentzVector e2 = e.getThisObject("e-",1)->getFourVector();
  ...
  **************************************************/
  ...
  if((collider=="tev2")&&
      ...
      // This is a reconstruction screw-up
	{
	  ...
	  if(iph>=0)
	    {	      
	      HepLorentzVector ph = o[iph].getFourVector();
	      bool killThePhoton = false;
	      for(size_t i=0; i<o.size(); i++)
		if(o[i].getObjectTypeSansSign()=="e")
		  {
		    HepLorentzVector ele = o[i].getFourVector();
