...
// probability to observe d if bkg=b0+/-deltab. Convolute the poisson with gaussian. The gaussian represents the uncertainty in b0 by deltab. We pull b from that gaussian, and then we pull d from a poisson of mean b.
  ...
  //like probOfThisEffect, but more accurate. -- Georgios 2008
  double accurateProbOfThisEffect(int d, double b0, double deltab, std::string direction=">=",bool adjustTrapezoidSize=true);
  ...
  //that will necessarily be between nmin and nmax.
  int poissonConstrained(double mu, int nmin=0, int nmax=-1);
  ...
  double totalWeightUncertainty(const std::vector<double>& w);
 ...
 /**   Minimization routines  **/
  ...
  int criticalD(double bkg, double pvalueTarget, double deltaBkg=0) ;

}

#include "Validation/VstMathUtils/interface/Math.ii"
