...
// probability to observe d if bkg=b0+/-deltab. Convolute the poisson with gaussian. The gaussian represents the uncertainty in b0 by deltab. We pull b from that gaussian, and then we pull d from a poisson of mean b.
  ...
  const int k=4; //the bigger the k the finer the sampling, the longer it takes. The integration step is equal to deltab/k.
  const int s=8; // number of standard deviations (deltab) up and down from b0 to sample. The more the better and the slower.
  double positivePart=(deltab!=0)?0.5*(1+erf(b0/sqrt(2.)/deltab)):1;
  double renormalizationFactor=1+(1-positivePart)/positivePart; // this factor increases the positive piece of the gaussian G(b-b0,sigmab) to compensate for the area that is lost because it falls on the negative side of b.
  static vector<double> sliceAreas;
  if (sliceAreas.empty()) {
    //    cout << "filling for first time." << endl;
    int counter=0;
    for(int n=-s*k; n<=s*k; ++n){
      sliceAreas.push_back(0.5*(erf((n+0.5)/k/sqrt(2.))-erf((n-0.5)/k/sqrt(2.))));
      ++counter;
    }
  }
...
//   cout << endl;
  ...
  for(int n=-s*k; n<=s*k; ++n){
    if(b0+n*deltab/k > 0) {
      ...
      ans += sliceAreas[counter] * renormalizationFactor * poisson(d,b0+n*deltab/k) ;
    }
    ++counter;
  }
  return(ans);
...
double Math::accurateProbOfThisEffect(int d, double b0, double deltab, string direction,bool adjustTrapezoidSize)
{
  int trapezoidSize=1;//maximum accuracy: 1.  The bigger the worse.
  //  bool adjustTrapezoidSize=true; //will save time and give up a little precision, by deciding the trapezoidSize as we go.  If you want absolute precision set to false.  That will keep whatever trapezoidSize you have in the previous line.  Set that to 1 for maximal precision.
  int lowerSumLimit=0;
  int upperSumLimit=0;
  double sigmaInGaussianApproximation=sqrt(deltab*deltab+b0);
  double effectProbability=0;
  if(direction==">=" && d==0) return(1.0);
  if(direction=="<" && d==0) return(0);
  if((direction==">=" || direction==">") && d>0 && b0==0 && deltab==0) return(0);
  if((direction=="=="||direction=="<=") && d==0 && b0==0 && deltab==0) return(1.0);
  if((direction=="<=" || direction=="<") && d>0 && b0==0 && deltab==0) return(1.0);
  if(direction==">=" && d>0 && b0==0 && deltab==0) return(0);
  if(direction=="==") effectProbability=ppg(d,b0,deltab);
  if(direction==">=" || direction==">") {
    if(direction==">=") lowerSumLimit=d;
    if(direction==">")  lowerSumLimit=d+1;
    upperSumLimit=(int)(b0+20*sigmaInGaussianApproximation);
    double tmpppg=ppg(lowerSumLimit,b0,deltab);
    for (int n=lowerSumLimit + trapezoidSize; n<=upperSumLimit; n += trapezoidSize) {
      //      cout << "trapezoidSize= " << trapezoidSize << " n= " << n << endl;
      double firstPpg = tmpppg;
      tmpppg=ppg(n,b0,deltab);
      double secondPpg = tmpppg;
      double trapezoid = (firstPpg+secondPpg)*(trapezoidSize+1.0)/2.;
      if(n!=lowerSumLimit+trapezoidSize) trapezoid -= firstPpg; //to avoid double counting of edge
      effectProbability += trapezoid;
      if (trapezoid/effectProbability < 1e-10) break;
      if(adjustTrapezoidSize) {
	//decide next step size
	double position = ((double)n-b0)/sigmaInGaussianApproximation; //in sigmas
	if (fabs(position)<0.5)
	  trapezoidSize = (int)(sigmaInGaussianApproximation*0.5/0.25);
	else if (fabs(position)<2)
	  trapezoidSize = (int)(sigmaInGaussianApproximation*2/1);
	else if (fabs(position)<4)
	  trapezoidSize = (int)(sigmaInGaussianApproximation*2/1);
	else if (fabs(position)<6)
	  trapezoidSize = (int)(sigmaInGaussianApproximation*2/1);
	else if (fabs(position)<8)
	  trapezoidSize = (int)(sigmaInGaussianApproximation*2/1);
	else if (fabs(position)<10)
	  trapezoidSize = (int)(sigmaInGaussianApproximation*2/1);
	else if (fabs(position)<12)
	  trapezoidSize = (int)(sigmaInGaussianApproximation*2/1);
	else trapezoidSize = (int)sigmaInGaussianApproximation;
	if (trapezoidSize < 1) trapezoidSize=1;
      }//end of adjustment
    }//next n
    //    a small correction to compensate a little for the coarse sampling
    double position = ((double)d-b0)/sigmaInGaussianApproximation; //in sigmas
    if (adjustTrapezoidSize && position>1) effectProbability *= 1+(0.2-1)*(position-1)/(10-1);

  }//end of >= or > case.
  if(direction=="<" || direction=="<=") {
    if(direction=="<") upperSumLimit=d-1;
    if(direction=="<=") upperSumLimit=d;
    lowerSumLimit=(int) max(0,b0-20*sigmaInGaussianApproximation);
    double tmpppg=ppg(upperSumLimit,b0,deltab);;
    for (int n=upperSumLimit-trapezoidSize; n>=lowerSumLimit; n -= trapezoidSize) {
      //   cout << "trapezoidSize= " << trapezoidSize << " n= " << n << endl;
      double firstPpg = tmpppg;
      tmpppg=ppg(n,b0,deltab);
      double secondPpg = tmpppg;
      double trapezoid = (firstPpg+secondPpg)*(trapezoidSize+1.0)/2.;
      if(n!=upperSumLimit-trapezoidSize) trapezoid -= firstPpg; //to avoid double counting of edge
      effectProbability += trapezoid;
      if (trapezoid/effectProbability < 1e-10) break;
      if(adjustTrapezoidSize) {
	//decide next step size
    ...
    //    a small correction to compensate a little for the coarse sampling
    double position = ((double)d-b0)/sigmaInGaussianApproximation; //in sigmas
    if (adjustTrapezoidSize && position<1) effectProbability *= 0.2+(1-0.2)*(position-(-5))/(1-(-5));
  }//end of < or <= case.
  if(effectProbability > 1) effectProbability = 1;
  if(effectProbability < 0) effectProbability = 0;
...
double Math::totalWeightUncertainty(const vector<double>& w) {
  //Georgios: We have an array of weights. We want the statistical uncertainty on the total weight.
  //The effective number of events is defined as:
  //   Neff * <w> = W
  //    where W = sum{w_i}
  //    and <w> = weighted average weight = sum{w_i/W * w_i} = sum{w_i^2}/W
  //   The poisson standard deviation of Neff is sqrt(Neff).							      
...
int Math::poissonConstrained(double mu, int nmin, int nmax) {
  //return a poisson distributed number, from poisson with mean mu, but
  //that will necessarily be between nmin and nmax.  That means, a number n
  //following P(n|n in [nmin,nmax]) = Poisson(n;mu)/P(n in [nmin,nmax]).
  //So, P(n|n in [nmix,nmax]) is like Poisson(n;mu) in shape, but is cut off
  //at nmin and nmax, and is renormalized to unity.
  //Georgios 2008
  
  assert(nmin>=0);

  double probOfnLessThanMin = 0;
  if(nmin>0)
    probOfnLessThanMin = Math::accurateProbOfThisEffect(nmin,mu,0,"<",false);
  double probOfnMoreThanMax = 0;
  if(nmax!=-1)
    probOfnMoreThanMax=Math::accurateProbOfThisEffect(nmax,mu,0,">",false);

  cout << "probOfnLessThanMin=" << probOfnLessThanMin << " probOfnMoreThanMax=" << probOfnMoreThanMax << endl;

  //  double renormalizationFactor = 1/(probOfnMoreThanMax+probOfnLessThanMin);
  assert((1-probOfnMoreThanMax)-probOfnLessThanMin>0);

  double r = probOfnLessThanMin+((1-probOfnMoreThanMax)-probOfnLessThanMin)*drand48();
  cout << "r=" << r << endl;
  int n = nmin;
  double lognfactorial = 0;
  for ( int i=1; i <= nmin; ++i)
    lognfactorial += log((double)i);
  double P = probOfnLessThanMin;
  cout << "n= " << n << " ";
  cout << "P=" << P << " ";
  while( r > (P += exp(-mu + n*log(mu) - lognfactorial) ))
    {
...
int Math::criticalD(double bkg, double pvalueTarget, double deltaBkg) {
  // Georgios: If we expect bkg, what is the minimum number of data we need to observe to get a pvalue <= pvalueTarget ?
  //cout << "\ncriticalD: bkg=" << bkg << " pvalueTarget=" << pvalueTarget << endl;
  double debug=false;
  if(bkg==0 && deltaBkg==0) {
    if(debug) cout << "even 1 would be a miracle if I expect exactly 0" << endl;
    return(1); //even one would be a miracle if I expect exactly 0
  }
  if(bkg==0 && deltaBkg>0) {
    if(debug) cout << "be conservative, assume bkg is equal to deltaBkg." << endl;
    return(criticalD(deltaBkg,pvalueTarget,deltaBkg)); //be conservative, assume bkg is equal to deltaBkg.
  }
  assert(bkg > 0);
  assert(deltaBkg >= 0);
  int d = (int) max(0, (int)floor(bkg + Math::prob2sigma(pvalueTarget)*sqrt(bkg)) ) ;
  if(debug) cout << "Starting d= " << d << endl;
  double pval=Math::probOfThisEffect(bkg,d,deltaBkg,">=");
  if ( pval < pvalueTarget ) {
    while ( pval < pvalueTarget ) {
      if(debug) cout << "d= " << d << " pval=" << pval << "  ";
      --d;
      double previousPval=pval;
      pval=Math::probOfThisEffect(bkg,d,deltaBkg,">=");
  ...
  if(debug) cout << " Returning: " << d << endl;
  return(d);
}
