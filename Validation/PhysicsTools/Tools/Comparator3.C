...
public:
  ...
  void DrawSlice( const char* key, 
    ...
    TH2* Lx345_SL4_2d = dynamic_cast< TH2* >(h0);
    TH2* Lx345_SL5_2d = dynamic_cast< TH2* >(h1);
    TH2* Lx432_SL4_2d = dynamic_cast< TH2* >(h2);
    
    if(Lx345_SL4_2d->GetNbinsY() == 1 || 
       Lx345_SL5_2d->GetNbinsY() == 1 )||
       Lx432_SL4_2d->GetNbinsY() == 1 ) {
    ...
    TH1D* Lx345_SL4_slice = Lx345_SL4_2d->ProjectionY(name0.c_str(),
					binxmin, binxmax, "");
    TH1D* Lx345_SL5_slice = Lx345_SL5_2d->ProjectionY(name1.c_str(),
					binxmin, binxmax, "");
    TH1D* Lx432_SL4_slice = Lx432_SL4_2d->ProjectionY(name2.c_str(),
                                        binxmin, binxmax, "");   

    TH1::AddDirectory( true );
    Draw( Lx345_SL4_slice, Lx345_SL5_slice, Lx432_SL4_slice, mode);        
  }
  ...
  void Draw( const char* key, Mode mode) {
    ...
    int Lx345_SL4_entries = h0->GetEntries();
    int Lx345_SL5_entries = h1->GetEntries();
    int Lx432_SL4_entries = h2->GetEntries();

    cout << " Lx345_SL4_entries =" << Lx345_SL4_entries << " Lx345_SL5_entries =" <<  Lx345_SL5_entries << " Lx432_SL4_entries =" <<  Lx432_SL4_entries << endl;
//    gStyle->SetStatColor( (Lx345_SL4_entries == Lx345_SL5_entries == Lx432_SL4_entries) ? 3 : 2); 
 ...
 //   gStyle->SetStatColor(pv > 0.1 ? 3 : 2);
  ...
  // been plotted
  TH1* h0() {return Lx345_SL4_;}
  TH1* h1() {return Lx345_SL5_;}
  TH1* h2() {return Lx432_SL4_;} 
...
private:
  ...
  void Draw( TH1* h0, TH1* h1, TH1* h2, Mode mode ) {
    ...
    TH1::AddDirectory( false );
    Lx345_SL4_ = (TH1*) h0->Clone( "Lx345_SL4_");
    Lx345_SL5_ = (TH1*) h1->Clone( "Lx345_SL5_");
    Lx432_SL4_ = (TH1*) h2->Clone( "Lx432_SL4_");
    ...
    Lx345_SL4_->SetStats(1);
    Lx345_SL5_->SetStats(1);
    Lx432_SL4_->SetStats(1);

    if(rebin_>1) {
      Lx345_SL4_->Rebin( rebin_);
      Lx345_SL5_->Rebin( rebin_);
      Lx432_SL4_->Rebin( rebin_);
    }
    if(resetAxis_) {
      Lx345_SL4_->GetXaxis()->SetRangeUser( xMin_, xMax_);
      Lx345_SL5_->GetXaxis()->SetRangeUser( xMin_, xMax_);
      Lx432_SL4_->GetXaxis()->SetRangeUser( xMin_, xMax_);
    }

    switch(mode) {
    case SCALE:
      Lx345_SL5_->Scale( Lx345_SL4_->GetEntries()/Lx345_SL5_->GetEntries() );
    case NORMAL:
      if(s0_)
	FormatHisto( Lx345_SL4_ , s0_);
      if(s1_)
 	FormatHisto( Lx345_SL5_ , s1_);
      if(s2_)
        FormatHisto( Lx432_SL4_ , s2_);

      gStyle->SetStatColor(2);
      Lx345_SL4_->Draw();
      gPad->Update();
      TPaveStats *st = (TPaveStats*)Lx345_SL4_ ->FindObject("stats");
      st->SetY1NDC(0.52);
      st->SetY2NDC(0.68);
      gStyle->SetStatColor(3);
      Lx345_SL5_->Draw("sames");
      gPad->Update();
      TPaveStats *st = (TPaveStats*)Lx345_SL5_ ->FindObject("stats");
      st->SetY1NDC(0.68);
      st->SetY2NDC(0.84);
      gStyle->SetStatColor(4);
      Lx432_SL4_->Draw("sames");
      break;
    case EFF:
      Lx345_SL5_->Divide( Lx345_SL4_ );
      if(s1_)
 	FormatHisto( Lx345_SL5_ , s0_);
      Lx345_SL5_->Draw();
    default:
      break;
    }
