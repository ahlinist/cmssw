...
#include "CSCZSensorsDataConverter.h"
#include <fstream>
#include <iostream>
CSCZSensorsDataConverter::CSCZSensorsDataConverter(const edm::ParameterSet& iConfig):m_inFileName( iConfig.getUntrackedParameter< std::string >("inputFile") ){}
void CSCZSensorsDataConverter::endJob()
{
  std::ifstream myfile(m_inFileName.c_str());
  if ( !myfile.is_open() ) throw cms::Exception("unable to open file");
  CSCZSensors* myobj=new CSCZSensors;
  ...
  while (! myfile.eof() ){
    ...
    myobj->cscZSens_.push_back(data);
    ++counter;
  }
  myfile.close();
  ...
  if( mydbservice.isAvailable() ){
    try{
      mydbservice->createNewIOV<CSCZSensors>(myobj,mydbservice->endOfTime(),"CSCZSensorsRcd");
