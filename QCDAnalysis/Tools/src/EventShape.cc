...
int EventShape::calculate(){
  ...
  for(unsigned int k =0; k<length; k++){
    ...
    if(Object_P[k]>Object_E[k] + 1E-4){
      cout << "ERROR!!! Object " << k <<" has P = " << Object_P[k] << " which is bigger than E = " << Object_E[k] <<endl;
      return 0;}
  ...
  for(unsigned int i=0;i<length;i++){
    dirglthrmin+= fabs(-sin(alpha)*Object_Px[i]+cos(alpha)*Object_Py[i]);
  }
  ...
  if(nin==0){
    cout<<"WARNING!! No momenta in the central region!! Central event shapes won't be calculated and set = -1.0 per default!"<<endl;
  ...
  if (nin>0){
    ...
    for(unsigned int i=0;i<nin;i++){
      centhrmin+= fabs(-sin(alpha_c)*Object_Px_in[i]+cos(alpha_c)*Object_Py_in[i]);
    }
    ...
    for(unsigned int i=0;i<nin;i++){
      dot_product_b =sqrt(Object_Px_in[i]*ThrustAxis_c[0]+Object_Py_in[i]*ThrustAxis_c[1]);
      if(dot_product_b>=0){
	...
	cenbroad_up+=Object_Pt_in[i]*sqrt(pow(Object_Eta_in[i]-eta_up,2)+pow(DeltaPhi(phi_up[index_up],phi_up_aver),2));
	index_up+=1;
      }else{
	cenbroad_down+=Object_Pt_in[i]*sqrt(pow(Object_Eta_in[i]-eta_down,2)+pow(DeltaPhi(phi_down[index_down],phi_down_aver),2));
	index_down+=1;
      }
    }
...
double EventShape::three_jet_res(std::vector<double> In_Object_Px, std::vector<double> In_Object_Py, std::vector<double> In_Object_Pz, std::vector<double> In_Object_E, int rap){
  ...
  if(y3_length<3){
    ...
    for(unsigned int i = 0; i<y3_length; i++){
      ...
      distance_jk_min=min(pow(input_Pt[1],2),pow(input_Pt[0],2))*(pow(input_Eta[1]-input_Eta[0],2)+pow(DeltaPhi(input_Phi[1],input_Phi[0]),2)) + 10;
	...
	for(unsigned int j=0; j<rest;j++){
	  ...
	  if(j>0){
	    for(unsigned int k=0; k<j;k++){
	      ...
	      distance_jk = min(pow(input_Pt[j],2),pow(input_Pt[k],2))*(pow(input_Eta[j]-input_Eta[k],2)+pow(DeltaPhi(input_Phi[j],input_Phi[k]),2));
