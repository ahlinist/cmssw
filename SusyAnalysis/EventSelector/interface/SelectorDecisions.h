...
public:
   SelectorDecisions(const std::vector<bool> decisions) :
   ...
   /// selector results
   const std::vector<bool>& decisions() const {
      return decisions_;
   }

   /// global decision (AND of all selectors)
   bool globalDecision() const {
      for (std::vector<bool>::const_iterator i = decisions_.begin(); i != decisions_.end(); ++i) {
         if (!(*i))
            return false;
      }
      return true;
