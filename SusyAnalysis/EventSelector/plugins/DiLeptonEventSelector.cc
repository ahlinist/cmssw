...
DiLeptonEventSelector::DiLeptonEventSelector (const edm::ParameterSet& pset) :
  ...
  diLeptonId_  (pset.getParameter <std::string>          ("diLeptonId")),
  diLeptonSign_(pset.getParameter <int>                  ("diLeptonSign")),
  ...
  minIso_      (pset.getParameter <std::vector <double> >("minIso"))
{
  ...
  edm::LogInfo("DiLeptonEventSelector") << "constructed with " << std::endl <<
    "electronTag  = " << electronTag_  << std::endl <<
    "muonTag      = " << muonTag_      << std::endl <<
    "diLeptonId   = " << diLeptonId_   << std::endl <<
    "diLeptonSign = " << diLeptonSign_ << std::endl;
}

//______________________________________________________________________________
bool
DiLeptonEventSelector::select (const edm::Event& event) const {
  ...
  if (minPt_.size()  != N_LEPTONS ||
      maxEta_.size() != N_LEPTONS ||
      minIso_.size() != N_LEPTONS) {
    edm::LogError("DiLeptonEventSelector") << "Incorrect vector length of " <<
      "selection values";
  ...
  if (diLeptonId_ == "ee") {
    ...
    if (!electronHandle.isValid()) {
      edm::LogWarning("DiLeptonEventSelector") << "No electron results for" <<
    ...
    if (!muonHandle.isValid()) {
      edm::LogWarning("DiLeptonEventSelector") << "No muon results for" <<
        "InputTag " << muonTag_;
...
//______________________________________________________________________________
template <class T> bool DiLeptonEventSelector::
hasSameIdLeptons (const std::vector <T>& leptonHandle, const int leptonId) const {
  ...
  for (unsigned int i = 0; i < N_LEPTONS; i++) {
    for (unsigned int j = i + 1; j < leptonHandle.size(); j++) {
      if ((leptonHandle)[i].charge() == diLeptonSign_ *
          (leptonHandle)[j].charge() &&
          (leptonHandle)[i].pt() + (leptonHandle)[j].pt() > scalarPtSum) {
        ...
        lepton[0].iso   = (leptonHandle)[i].pt() / ((leptonHandle)[i].pt() +
                          (leptonHandle)[i].trackIso());
        lepton[1].iso   = (leptonHandle)[j].pt() / ((leptonHandle)[j].pt() +
                          (leptonHandle)[j].trackIso());
        break;
      }
    }
  }
  ...
  for (int iLepton = 0; iLepton < N_LEPTONS; iLepton++) {
    if (      lepton[iLepton].pt   < minPt_ [iLepton] ||
        fabs (lepton[iLepton].eta) > maxEta_[iLepton] ||
              lepton[iLepton].iso  < minIso_[iLepton]) {
      LogTrace("DiLeptonEventSelector") << "DiLeptonEventSelector: failed " <<
        "at electron " << iLepton;
      result = false;
    }
  ...
  if (result)
    LogTrace("DiLeptonEventSelector") << "DiLeptonEventSelector: all " <<
      "leptons passed";
...
//______________________________________________________________________________
template <class C, class D> bool DiLeptonEventSelector::
hasDiffIdLeptons (const std::vector <C>& lepton1Handle,
  ...
  for (unsigned int i = 0; i < lepton1Handle.size(); i++) {
    for (unsigned int j = 0; j < lepton2Handle.size(); j++) {
      if ((lepton1Handle)[i].charge() == diLeptonSign_ *
          (lepton2Handle)[j].charge() &&
          (lepton1Handle)[i].pt() + (lepton2Handle)[j].pt() > scalarPtSum) {
        ...
        lepton[0].iso   = (lepton1Handle)[i].pt() / ((lepton1Handle)[i].pt() +
                          (lepton1Handle)[i].trackIso());
        lepton[1].iso   = (lepton2Handle)[j].pt() / ((lepton2Handle)[j].pt() +
                          (lepton2Handle)[j].trackIso());
        break;                        
      }
    }
  }
