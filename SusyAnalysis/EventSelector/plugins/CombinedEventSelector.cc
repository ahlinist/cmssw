...
CombinedEventSelector::CombinedEventSelector(const edm::ParameterSet& pset) :
   SusyEventSelector(pset), sequence_(pset.getParameter<std::vector<std::string> > ("components"), pset.getParameter<
            edm::ParameterSet> ("_AllFilters")) {

}

//________________________________________________________________________________________
double CombinedEventSelector::value(const std::string& name) const {
