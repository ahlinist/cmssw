...
MHTEventSelector::MHTEventSelector(const edm::ParameterSet& pset) :
	...
	//  jetTag_( pset.getParameter<edm::InputTag>("jetTag") ),
			mhtTag_(pset.getParameter<edm::InputTag> ("mhtTag")), minMHT_(pset.getParameter<double> ("minMHT")), maxMHT_(
					pset.getParameter<double> ("maxMHT")),
...
//__________________________________________________________________________________________________
bool MHTEventSelector::select(const edm::Event& event) const {
	...
	//
	if (mhtHandle->size() != 1) {
		edm::LogWarning("MHTEventSelector") << "MHT collection size is " << mhtHandle->size() << " instead of 1";
		return false;
	}
	...
	if (maxMHT_ > 0)
		result_max = (iMHT->mht() < maxMHT_);
	bool result_min = true;
	if (minMHT_ > 0)
		result_min = (iMHT->mht() > minMHT_);
