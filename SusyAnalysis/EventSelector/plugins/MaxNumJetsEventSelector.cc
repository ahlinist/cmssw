...
MaxNumJetsEventSelector::MaxNumJetsEventSelector(const edm::ParameterSet& pset) :
   SusyEventSelector(pset), jetTag_(pset.getParameter<edm::InputTag> ("jetTag")), maxEt_(pset.getParameter<double> (
            "maxEt")), // et cut on N+1th jet
...
//________________________________________________________________________________________
bool MaxNumJetsEventSelector::select(const edm::Event& event) const {
   ...
   for (unsigned int i = 0; (i != jetHandle->size()) && (i != maxNumJets_ + 1); ++i) {
      ...
      tempStream << "jet" << i << "Et";
      if (i >= jetHandle->size())
         setVariable(tempStream.str().c_str(), 0.0);
      else
         setVariable(tempStream.str().c_str(), (*jetHandle)[i].et());
   }
