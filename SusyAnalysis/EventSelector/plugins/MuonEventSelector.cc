...
MuonEventSelector::MuonEventSelector(const edm::ParameterSet& pset) :
   SusyEventSelector(pset), muonTag_(pset.getParameter<edm::InputTag> ("muonTag")), minPt_(pset.getParameter<
            std::vector<double> > ("minPt")), maxEta_(pset.getParameter<std::vector<double> > ("maxEta")), minIso_(
   ...
   edm::LogInfo("MuonEventSelector") << "constructed with " << std::endl << "muonTag     = " << muonTag_ << std::endl
            << "min # muons = " << minPt_.size() << std::endl;
}

//________________________________________________________________________________________
bool MuonEventSelector::select(const edm::Event& event) const {
   // Reset cached variables
   resetVariables();

   if (minPt_.size() != maxEta_.size() || minPt_.size() != minIso_.size()) {
   ...
   if (muonHandle->size() < minPt_.size())
      return false;
   ...
   for (unsigned int i = 0; i < minPt_.size(); i++) {
      if ((*muonHandle)[i].pt() < minPt_[i] || fabs((*muonHandle)[i].eta()) > maxEta_[i] || (*muonHandle)[i].pt()
               / ((*muonHandle)[i].pt() + (*muonHandle)[i].trackIso()) < minIso_[i]) {
   ...
   if (result)
      LogTrace("MuonEventSelector") << "MuonEventSelector: all muons passed";
