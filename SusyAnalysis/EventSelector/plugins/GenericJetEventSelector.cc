...
//________________________________________________________________________________________
bool
GenericJetEventSelector::select (const edm::Event& event) const
{
  ...
  bool result(false);
  unsigned int i=0;
  for (edm::View<reco::Jet>::const_iterator jet=jetHandle->begin();
       jet!=jetHandle->end();++jet) {
     float EMFRAC=0;
     const reco::CaloJet * cj = static_cast<const reco::CaloJet*>( &(*jet) );
     if (!cj) EMFRAC=cj->emEnergyFraction();
     else {
       const reco::PFJet * pj   = static_cast<const reco::PFJet*>( &(*jet) );
       if (!pj) EMFRAC=pj->neutralEmEnergyFraction()+pj->chargedEmEnergyFraction();
     }

     if ( jet->pt()>=minPt_[i] &&
	  fabs(jet->eta())<=maxEta_[i] &&
	  EMFRAC<=maxFem_[i] ) 
      {
   ...
   if (result) LogTrace("GenericJetEventSelector") << "GenericJetEventSelector: all jets passed";
   else        LogTrace("GenericJetEventSelector") << "GenericJetEventSelector: failed";
   return result;
}
