...
LeptonVetoSelector::LeptonVetoSelector(const edm::ParameterSet& pset) :
  ...
  minPtEle_    ( pset.getParameter<double>       ("minElectronPt")     ),
  maxEtaEle_   ( pset.getParameter<double>       ("maxElectronEta")    ),
  eleIso_      ( pset.getParameter<double>       ("electronIsolation") ),
  eleDxy_      ( pset.getParameter<double>       ("maxElectronDxy")    ),
  minPtMuon_   ( pset.getParameter<double>       ("minMuonPt")         ),
  maxEtaMuon_  ( pset.getParameter<double>       ("maxMuonEta")        ),
  muonIso_     ( pset.getParameter<double>       ("muonIsolation")     ),
  muonHits_    ( pset.getParameter<unsigned int> ("muonMinHits")       ),
  muonDxy_     ( pset.getParameter<double>       ("maxMuonDxy")        )
...
//  event.getByLabel(tauTag_, tauHandle);
  ...
  if ( eleHandle.isValid() )
    for ( std::vector<pat::Electron>::const_iterator ie=(*eleHandle).begin();
      ...
      if( ie->pt() < minPtEle_ ) continue;

      if( fabs( ie->eta() ) > maxEtaEle_ ) continue;

      if( (ie->trackIso()+ie->ecalIso()+ie->hcalIso())/ie->pt() > eleIso_ ) continue;

      if( fabs( ie->gsfTrack()->dxy(myPosition) ) > eleDxy_ ) continue;
...
//       std::cout << "-----------electron end"<<std::endl;
  ...
  if ( muonHandle.isValid() )
    for ( std::vector<pat::Muon>::const_iterator im=(*muonHandle).begin();
      ...
      if( im->pt() < minPtMuon_ ) continue;

      if( fabs( im->eta() ) > maxEtaMuon_ ) continue;

      if( (im->trackIso()+im->ecalIso()+im->hcalIso())/im->pt() > muonIso_ ) continue;

      if( fabs( im->innerTrack()->dxy(myPosition) ) > muonDxy_ ) continue;

      if( im->innerTrack()->numberOfValidHits() < muonHits_ ) continue;
