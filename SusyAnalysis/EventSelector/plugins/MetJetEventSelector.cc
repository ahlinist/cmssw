...
MetJetEventSelector::MetJetEventSelector(const edm::ParameterSet& pset) :
   SusyEventSelector(pset), metTag_(pset.getParameter<edm::InputTag> ("metTag")), jetTag_(pset.getParameter<
            edm::InputTag> ("jetTag")), metDphiMin_(pset.getParameter<double> ("metDPhiMin")), dPhiJet2MetMin_(
            pset.getParameter<double> ("dPhiJet2MetMin")), rDistJetsMin_(pset.getParameter<double> ("rDistJetsMin")),
...
//________________________________________________________________________________________
bool MetJetEventSelector::select(const edm::Event& event) const {
   ...
   if (metHandle->size() != 1) {
      edm::LogWarning("MetJetEventSelector") << "MET collection size is " << metHandle->size() << " instead of 1";
      return false;
   }
   ...
   //
   if (metIso < metDphiMin_)
      return false;
   if (dPhiJet2Met < dPhiJet2MetMin_)
      return false;
   if (R1 < rDistJetsMin_)
      return false;
   if (R2 < rDistJetsMin_)
