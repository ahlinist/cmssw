C-+++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C- Version EDDE2.1 /R.Ryutin,A.Sobol ++++++++++++++++++++++
C-+++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
C-...User process initialization commonblock.
     ...
     &LPRUP(MAXPUP)
      SAVE /HEPRUP/
C      
ccc      CALL KI_PYTS2PYTD(1)         ! Interface between Pythia single ...
ccc      CALL EDDEFFR                 ! Read the data cards
cc      CALL EDDEFFRTEST                 ! Read the data cards
ccc      CALL KI_PYTS2PYTD(2)         ! ... DOUBLE PRECISION
...
C
      ...
      EBMUP(2)  = SQS/2
      IDWTUP    = 1
      IF(IPROC.EQ.400)   IDWTUP    = 3
c      IF(IPROC.EQ.406)   IDWTUP    = 3   !soboladd
      NPRUP     = 1
      LPRUP(1)  = IPROC
      XSECUP(1) = 1.D0 
      XMAXUP(1) = 1.D0 
C--
      ...
      PARI(7)   = 1.D0 
      IF(IPROC.EQ.400) PMAS(IPARS(4),1)=AM0          ! Higgs mass 
c      IF(IPROC.EQ.406) PMAS(IPARS(4),1)=AM0          ! Higgs mass soboladd
C--
CCCC Kirill      CALL PYINIT('USER','p','p',SQS)
cccc      print*,'SQS',SQS
      CALL PYINIT('CMS','p','p',SQS)
      XSEC(4,3) = PARI(1)
      NGEN(4,3) = 0
...
c       KDUMMY=IPROC
      ...
      IF(IPROC.EQ.400) CALL EDDERES              ! pp -> pHp (pH*p,pR*p)
      IF(IPROC.EQ.401) CALL EDDEQQ               ! pp -> p QQbar p, Q=b default
      IF(IPROC.EQ.402) CALL EDDEGG               ! pp -> p gg p
      IF(IPROC.EQ.403) CALL EDDE2GAM          ! pp -> p gammagamma p
      IF(IPROC.EQ.404) CALL EDDEQQG            ! pp -> p QQbarg g p
      IF(IPROC.EQ.405) CALL EDDE3G               ! pp -> p gg g p
C- SEMI-INCLUSIVE ------ 
      IF(IPROC.EQ.406) CALL SIDDERES              ! pp -> p{X H Y}p (p{X H*,R* Y}p)
      IF(IPROC.EQ.407) CALL SIDDEQQ                ! pp -> p {X QQbar Y} p
      IF(IPROC.EQ.408) CALL SIDDEGG                ! pp -> p {X gg Y} p
      IF(IPROC.EQ.409) CALL SIDDE2GAM            ! pp -> p {X gammagamma Y} p
...
C----limits for the integration in MGG -----------------------
       ...
       IF (IP.EQ.402) THEN
        ...
        PRINT*,'GGGGGGGGGGGGGGGGG-GGGGGGGGGGGGG'
       ENDIF       
...
c----- f_g/g lower limit and minimal generated PT_soft_g
       ...
       FGGQS=1.D0
c--- RS1 data from FFR ----------------
       NRS0=IPARS(6) ! FFRDAT IPARS(6)
       RSXI0=RPARS(5) ! FFRDAT RPARS(5)
       RSGAM0=RPARS(6) ! FFRDAT RPARS(6)
       RSMH0=RPARS(7) ! FFRDAT RPARS(7)
       RSMR0=RPARS(8) ! FFRDAT RPARS(8)
c--- other parameters
c--- code of central particle
       KCP=IPARS(4) ! FFRDAT IPARS(4)
c--- code of the process
       IPROC=IPARS(1) ! FFRDAT IPARS(1)
c--- mass of the central particle
       AM0=RPARS(3) ! FFRDAT RPARS(3)
      IF (NRS0.GT.0) THEN
      CALL EDDERS1C(NRS0,RSXI0,RSGAM0,RSMH0,RSMR0,AUX1,AUX2,AUX3)
      AM0=AUX1
      ENDIF
c--- phi dependence code
       NFI=IPARS(5) ! FFRDAT IPARS(5)
c------ constants -----------------------------------------
       CF=(NC*NC-1)*TF/NC
...
c- SIDDE calculations for all rapidities by def.
       ETASIMAX=RPARS(9) ! FFRDAT RPARS(9)     
       PSURV=0.66D-01
