...
c     brick+
      else if(IGEO.eq.4) then
         if (z .ge. 0.d0 .and. z .le. xl)
     >        then
            if (bz .gt. 0.d0) then
               ttpp=(xl-z)/bz
               xlp=dsqrt((bx*ttpp)**2.d0+(by*ttpp)**2.d0+
     >              (xl-z)**2.d0)
            else
               ttpp=z/dabs(bz)
               xlp=dsqrt((bx*ttpp)**2.d0+(by*ttpp)**2.d0+
     >              (z)**2.d0)
         ...
         elseif (z .lt. 0.d0) then
            if (bz .lt. 0.d0) then
               ...
               yypp1=y+by*ttpp1
               yypp2=y+by*ttpp2
               xlp=dsqrt((xxpp1-xxpp2)**2.d0+(yypp1-yypp2)**2.d0+
     >              xl**2.d0)
            endif
         elseif (z .gt. xl) then
            if (bz .gt. 0.d0) then
