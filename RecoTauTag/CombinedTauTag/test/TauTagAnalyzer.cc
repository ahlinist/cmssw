...
void TauTagAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup){
  ...
  */
  ...
  const JetTagCollection& jetTagCollection = *(jetTagHandle.product()); 
  
  Handle<CombinedTauTagInfoCollection> tagInfoHandle;
  iEvent.getByLabel(CombinedTauTagProd_, tagInfoHandle);
  const CombinedTauTagInfoCollection & tagInfoCollection = *(tagInfoHandle.product()); 
  
  for (unsigned int i = 0; i < tagInfoCollection.size(); ++i) 
  {
    float discriminator                = jetTagCollection[i].second;
    const CombinedTauTagInfo & tagInfo = tagInfoCollection[i];

    HepLorentzVector TheCombinedTauTagJet_HepLV(tagInfo.jet()->px(),tagInfo.jet()->py(),tagInfo.jet()->pz(),tagInfo.jet()->energy());
    ...
    CombinedTauTagInfo_mode    = tagInfo.alternatrecJet_HepLV().e();
    CombinedTauTagInfo_modet   = tagInfo.alternatrecJet_HepLV().et();
    CombinedTauTagInfo_modeta  = tagInfo.alternatrecJet_HepLV().eta();
    CombinedTauTagInfo_modphi  = tagInfo.alternatrecJet_HepLV().phi();
    CombinedTauTagInfo_invmass = tagInfo.alternatrecJet_HepLV().m();
    CombinedTauTagInfo_passed_tracksel = tagInfo.passed_trackerselection();
    CombinedTauTagInfo_is_GoodTauCandidate = tagInfo.is_GoodTauCandidate();
    CombinedTauTagInfo_infact_GoodElectronCandidate = tagInfo.infact_GoodElectronCandidate();
    CombinedTauTagInfo_infact_GoodMuonCandidate = tagInfo.infact_GoodMuonCandidate();
    if (!isnan(tagInfo.ECALEt_o_leadTkPt())) CombinedTauTagInfo_ECALEt_o_leadtkPt = tagInfo.ECALEt_o_leadTkPt();
    else CombinedTauTagInfo_ECALEt_o_leadtkPt = -100.;
    if (!isnan(tagInfo.HCALEt_o_leadTkPt())) CombinedTauTagInfo_HCALEt_o_leadtkPt = tagInfo.HCALEt_o_leadTkPt();
    else CombinedTauTagInfo_HCALEt_o_leadtkPt = -100.;
    CombinedTauTagInfo_needs_LikelihoodRatio_discrimination = tagInfo.needs_LikelihoodRatio_discrimination();
    CombinedTauTagInfo_signalrectks_number = (int)tagInfo.signalTks().size();
    CombinedTauTagInfo_neutralECALclus_number = tagInfo.neutralECALClus_number();
    if (!isnan(tagInfo.neutralECALClus_radius())) CombinedTauTagInfo_neutralECALclus_radius = tagInfo.neutralECALClus_radius();
    else CombinedTauTagInfo_neutralECALclus_radius = -100.;
    if (!isnan(tagInfo.neutralE_ratio())) CombinedTauTagInfo_neutrE_ratio = tagInfo.neutralE_ratio();
    else CombinedTauTagInfo_neutrE_ratio = -100.;
    CombinedTauTagInfo_isolneutrE_o_tksEneutrE_ratio = tagInfo.isolneutralE_o_TksEneutralE();
    CombinedTauTagInfo_tksEtJetEt_ratio = tagInfo.TksEt_o_JetEt();
    CombinedTauTagInfo_neutrEtksE_ratio = tagInfo.neutralE_o_TksEneutralE();   
    if (!isnan(tagInfo.leadTk_signedipt_significance())) CombinedTauTagInfo_leadtk_signedipt_significance = tagInfo.leadTk_signedipt_significance();
    else CombinedTauTagInfo_leadtk_signedipt_significance = -100.;
    if (!isnan(tagInfo.leadTk_signedip3D_significance())) CombinedTauTagInfo_leadtk_signedip3D_significance = tagInfo.leadTk_signedip3D_significance();
    else CombinedTauTagInfo_leadtk_signedip3D_significance = -100.;
    if (!isnan(tagInfo.signedflightpath_significance())) CombinedTauTagInfo_signedflightpath_significance = tagInfo.signedflightpath_significance();
    else CombinedTauTagInfo_signedflightpath_significance = -100.;
    if (!isnan(discriminator)) CombinedTauTagInfo_discriminator = discriminator;
