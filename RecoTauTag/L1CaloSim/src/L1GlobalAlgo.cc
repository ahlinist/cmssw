...
//
L1GlobalAlgo::L1GlobalAlgo(const edm::ParameterSet& iConfig)
{  
  m_RMap = L1RegionMap::getL1RegionMap();
  ...
  m_L1Config.EMActiveLevelIso = iConfig.getParameter<double>("EMActiveLevelIso");
  m_L1Config.HadActiveLevelIso = iConfig.getParameter<double>("HadActiveLevelIso");
...
void
L1GlobalAlgo::CaloTowersDump(edm::Event const& e) {
  /*
    std::vector<edm::Handle<CaloTowerCollection> > prods;
    
    edm::LogInfo("L1GlobalAlgo::CaloTowersDump") << "Start!";
    ...
    edm::LogInfo("L1GlobalAlgo::CaloTowersDump") << *j;
    }
    }
    edm::LogInfo("L1GlobalAlgo::CaloTowersDump") << "End!";
  */
...
namespace myspace {
  bool 
  //L1GlobalAlgo::greaterEt( const reco::Candidate& a, const reco::Candidate& b ) {
...
L1GlobalAlgo::findJets() {
  ...
  for (int i=0; i<396; i++) {
	...
	if (m_GctIso == true) { 
	  ...
	  if (m_DoBitInfo) checkPartialTauIsolation(i); 	 
...
// ------------ Add a jet  ------------
void
L1GlobalAlgo::addJet(int iRgn, bool taubit) {
...
void
L1GlobalAlgo::FillEgammasTP(edm::Event const& e) {
  m_Egammas.clear();
  m_isoEgammas.clear();
...
void
L1GlobalAlgo::FillEgammas(edm::Event const& e) {
  m_Egammas.clear();
  m_isoEgammas.clear();
...
void
L1GlobalAlgo::FillMET(edm::Event const& e) {
  m_METs.clear();
...
void 
L1GlobalAlgo::InitL1Regions() 
{
  m_Regions.clear();
  m_Regions = std::vector<L1Region>(396); // ieta: 1-22, iphi: 1-18
...
void 
L1GlobalAlgo::FillL1RegionsTP(edm::Event const& e, const edm::EventSetup& s) 
{
...
void 
L1GlobalAlgo::FillL1Regions(edm::Event const& e, const edm::EventSetup& c) 
{
  InitL1Regions();
...
bool 
L1GlobalAlgo::isTauJet(int cRgn) {
...
int
L1GlobalAlgo::isEMCand(CaloTowerDetId cid, l1extra::L1EmParticle* ph,const edm::Event& iEvent) {
...
bool 
L1GlobalAlgo::isMaxEtRgn_Window33(int crgn) {
  ...
  if (nwid==999 || neid==999 || nid==999 || swid==999 || seid==999 || sid==999 || wid==999 || 
      eid==999 ) { 
    //std::cerr << "L1GlobalAlgo::isMaxEtRgn_Window33(): RegionId out of bounds: " << std::endl
...
double
L1GlobalAlgo::hcaletValue(const int ieta,const int compET) {
...
// Et Check 	 
bool 	 
L1GlobalAlgo::TauIsolation(int cRgn) { 	 
...
void 	 
L1GlobalAlgo::checkPartialTauIsolation(int cRgn) { 	 
