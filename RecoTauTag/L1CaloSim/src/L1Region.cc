...
void
L1Region::SetParameters(L1Config iconfig) 
{ 
  Config = iconfig;
}

void 
L1Region::SetRegionEnergy()
{
  sumE = CalcSumE();
  sumEt = CalcSumEt();
}
...
//
void
L1Region::FillEMCrystals(const CaloTowerConstituentsMap* theTowerConstituentsMap,
...
//
void
L1Region::FillTowerZero(const CaloTower& t, int& tid) 
{
...
void 
L1Region::SetFGBit(int twrid,bool FGBIT)
{
  fgBit[twrid] = FGBIT;
}
void 
L1Region::SetHCFGBit(int twrid,bool FGBIT)
{
  ;
}
void 
L1Region::SetHOEBit(int twrid,bool FGBIT)
{
  hOeBit[twrid] = FGBIT;
}
