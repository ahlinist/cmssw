...
void analysisClass::Loop()
{
  ...
  TH1F *h_calometPt_PFMET   = new TH1F ("h_calometPt_PFMET","h_calometPt_PFMET",Nbins_METSumET,0,Max_METSumET);
  ...
  */
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) 
	...
	pass_PhysicsBit = 1;
      ...
      //PFMET
      vector<float> ECALspikes_PFMET_Px;
      vector<float> ECALspikes_PFMET_Py;
      vector<float> ECALspikes_PFMET_Pt;
      vector<float> ECALspikes_PFMET_Eta;
      vector<float> ECALspikes_PFMET_Phi;

      vector<float> HFspikes_PFMET_ShortPx;
      vector<float> HFspikes_PFMET_ShortPy;
      vector<float> HFspikes_PFMET_LongPx;
      vector<float> HFspikes_PFMET_LongPy;
      vector<float> HFspikes_PFMET_ShortPt;
      vector<float> HFspikes_PFMET_LongPt;
      vector<float> HFspikes_PFMET_Pt;
      vector<float> HFspikes_PFMET_Eta;
      vector<float> HFspikes_PFMET_Phi;
      vector<float> HFspikes_PFMET_iEta;
      vector<float> HFspikes_PFMET_iPhi;
      vector<float> HFspikes_PFMET_EMis0_HADis1;
      ...
      for (int i = 0; i<int(CaloTowersEmEt->size()); i++)
	{
	  if( fabs(CaloTowersIeta->at(i)) >= 29 ) //HF only
	      ...
	      if( Applyhfpfcleaning == 1 )
		{
		  ...
		  int isHFEMnoise_PFcuts = 0;
		  int isHFHADnoise_PFcuts = 0;
		  ...
		  for (int j = 0; j<int(CaloTowersEmEt->size()); j++)
		      ...
		      if( CaloTowersIeta->at(j) == CaloTowersIeta->at(i) && CaloTowersIphi->at(j) == ( CaloTowersIphi->at(i) - 2 ) )
			{
		      ...
		      //ieta = 39 
		      if( CaloTowersIeta->at(i) == 39) 
			{
			  //right
			  if( CaloTowersIeta->at(j) == 40 
			      && ( CaloTowersIphi->at(j) == CaloTowersIphi->at(i) || CaloTowersIphi->at(j) == CaloTowersIphi->at(i) + 2  ) )
			    {
			      E_em_right = CaloTowersEmEt->at(j) / sin(theta);
			      E_had_right = CaloTowersHadEt->at(j) / sin(theta);
			      E_em_right_const = CaloTowersEmEt->at(j) / sin(theta);
		      ...
		      if( CaloTowersIeta->at(i) == -39) 
			{
		      ...
		      //ieta = 40 
		      if( CaloTowersIeta->at(i) == 40) 
			{			 
			  //left
			  if( CaloTowersIeta->at(j) == 39 
			      && ( CaloTowersIphi->at(j) == CaloTowersIphi->at(i) || CaloTowersIphi->at(j) == CaloTowersIphi->at(i) - 2  ) )
			    {
			      E_em_left = CaloTowersEmEt->at(j) / sin(theta);
			      E_had_left = CaloTowersHadEt->at(j) / sin(theta);
			...
			{			 
		      ...
		      //iphi = 71
		      if( CaloTowersIphi->at(i) == 71) 
			{
			  //top
			  if( CaloTowersIeta->at(j) == CaloTowersIeta->at(i) && CaloTowersIphi->at(j) == 1 )
			    {
			      E_em_top = CaloTowersEmEt->at(j) / sin(theta);
			      E_had_top = CaloTowersHadEt->at(j) / sin(theta);
			      E_em_top_const = CaloTowersEmEt->at(j) / sin(theta);
		      ...
		      //iphi = 1
		      if( CaloTowersIphi->at(i) == 1) 
			{
			  //bottom
			  if( CaloTowersIeta->at(j) == CaloTowersIeta->at(i) && CaloTowersIphi->at(j) == 71 )
			    {
			      E_em_bottom = CaloTowersEmEt->at(j) / sin(theta);
			      E_had_bottom = CaloTowersHadEt->at(j) / sin(theta);
			      E_em_bottom_const = CaloTowersEmEt->at(j) / sin(theta);
		  ...
		  if(isHFEMnoise_PFcuts)
		    HFspikes_PFMET_EMis0_HADis1.push_back(0);

		  if(isHFHADnoise_PFcuts)		  
		    HFspikes_PFMET_EMis0_HADis1.push_back(1);

		  if(isHFEMnoise_PFcuts || isHFHADnoise_PFcuts)
		    {
		      HFspikes_PFMET_ShortPt.push_back(towerShortPt);
		      HFspikes_PFMET_LongPt.push_back(towerLongPt);
		      HFspikes_PFMET_ShortPx.push_back(towerShortPx);
		      HFspikes_PFMET_ShortPy.push_back(towerShortPy);
		      HFspikes_PFMET_LongPx.push_back(towerLongPx);
		      HFspikes_PFMET_LongPy.push_back(towerLongPy);
		      HFspikes_PFMET_Pt.push_back(towerPt);
		      HFspikes_PFMET_Eta.push_back(towerEta);
		      HFspikes_PFMET_Phi.push_back(towerPhi);
		      HFspikes_PFMET_iEta.push_back(towerIEta);
		      HFspikes_PFMET_iPhi.push_back(towerIPhi);	
		    }	  
	  ...
	  if( S4_PFMEToverS1 < 0.04 * log10( seedEnergy / 4 ) )
	    {
	      ...
	      ECALspikes_PFMET_Px.push_back( seedex )  ;
	      ECALspikes_PFMET_Py.push_back( seedey )  ;
	      ECALspikes_PFMET_Pt.push_back( seedet )   ;
	      ECALspikes_PFMET_Eta.push_back( seedeta ) ;
	      ECALspikes_PFMET_Phi.push_back( seedphi ) ;
	    }
	}
      //       if(isData==0)
      ...
      for (int i = 0; i<int(CaloTowersHadEt->size()); i++)
	{
      ...
      //PFMET clean
      float calometPx_clean_PFMET = calometPx->at(0); 
      float calometPy_clean_PFMET = calometPy->at(0); 
      float calometPt_clean_PFMET = -99; 
      ...
      for (int es=0; es<ECALspikes_tcMET_Pt.size(); es++)
	{
      ...
      //## clean hf spikes      
      for (int hs=0; hs<HFspikes_PFMET_Pt.size(); hs++)
	{
	  
	  if( HFspikes_PFMET_EMis0_HADis1[hs] == 0 )//is EM = Long fiber
	    {
	      calometPx_clean_PFMET = calometPx_clean_PFMET + HFspikes_PFMET_LongPx[hs];
	      calometPy_clean_PFMET = calometPy_clean_PFMET + HFspikes_PFMET_LongPy[hs];
	    }

	  if( HFspikes_PFMET_EMis0_HADis1[hs] == 1 )//is HAD = Short fiber
	    {
	      calometPx_clean_PFMET = calometPx_clean_PFMET + HFspikes_PFMET_ShortPx[hs];
	      calometPy_clean_PFMET = calometPy_clean_PFMET + HFspikes_PFMET_ShortPy[hs];
	    }
	}

      //## clean ecal spikes
      for (int es=0; es<ECALspikes_PFMET_Pt.size(); es++)
	{
	  calometPx_clean_PFMET = calometPx_clean_PFMET + ECALspikes_PFMET_Px[es];
	  calometPy_clean_PFMET = calometPy_clean_PFMET + ECALspikes_PFMET_Py[es];
	}

      //## re-calculate met pt and phi
      calometPt_clean_PFMET = sqrt( calometPx_clean_PFMET * calometPx_clean_PFMET + calometPy_clean_PFMET * calometPy_clean_PFMET );      
      ...
      if( passedCut("0") )
	{
	  ...
	  h_calometPt_PFMET->Fill( calometPt_clean_PFMET );
      ...
      if( passedCut("0") && PrintOut == 1)
	{
	  ...
	  if( calometPt_clean_PFMET > 15 )
	    {
	      cout << "event: " << event << " " 
		   << "ls: " << ls << " "
		   << "run: " << run << "  "
		   << "--  calometPt_clean_PFMET : " <<  calometPt_clean_PFMET << " "
		   << endl;
	    }
