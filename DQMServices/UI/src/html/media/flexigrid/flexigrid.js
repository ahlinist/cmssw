...
(function($){
  ...
  $.addFlex = function(t,p)
  {
    ...
    var g = {
      ...
      fixHeight: function (newH) {
          ...
          newH = document.body.offsetHeight - 100;

          if ($(g.sDiv).data("visible")) newH = newH - $(g.sDiv).height() - 1;
          $(g.bDiv).height(newH);
      ...
      colToggler: function (h) {
                          $(g.nDiv).css({top: h.offsetTop + $(h).height(), left: h.offsetLeft});
                          $(g.nDiv).toggle(); 
                          return true;
                        },
        ...
        if (this.colresize)
          {
            ...
            var at = $('th:visible div:eq('+n+')',this.hDiv).parent().attr('abbr');;
            $.each(p.colModel, function (k, o) {
              if (o.name == at) {
                o.width = nw;
              }
            });
                 ...
                 function ()
                  {
                    ...
                    td.align = this.align;
                                                                                td.setAttribute("abbr", $(this).attr("abbr"));
                    td.setAttribute("field", $(this).attr("field"));
                                                                                td.innerHTML = row[$(this).attr("field")];
                    $(tr).append(td);
                    td = null;
                  }
              ); 
      ...
      saveState: function() {
        ...
        $.each(g.saveParams, function(key, obj) {
          $.cookie(p.savePrefix + "_" + obj, p[obj]);
        });

        var hidden = "";
        $.each(p.colModel, function(key, obj) {
          var h = "0";
          if (obj.hide)
            h = "1";
          hidden += obj.name + "/" + obj.width + "/" + h + "&";
        });
        $.cookie(p.savePrefix + "_fields", hidden);
      },
      restoreState: function() {
        if (! p.savePrefix) return;
        $.each(g.saveParams, function(key, obj) {
          var x = $.cookie(p.savePrefix + "_" + obj);
          if (x)
            p[obj] = x;
        });
        $.each(g.saveParamsInt, function(key, obj) { p[obj] = parseInt(p[obj]);  });

        var hidden = $.cookie(p.savePrefix + "_fields");
        if ((hidden) && (hidden != null)) {
          $.each(hidden.split("&"), function(key, obj) { 
            var sb = obj.split("/");
            if (sb.length == 3) {
              $.each(p.colModel, function(key2, obj2) {
                if (obj2.name == sb[0]) {
                  obj2.width = parseInt(sb[1]);
                  obj2.hide = (parseInt(sb[2]) == 1);
                }
              });
            }
          });
        ...
        if (! p.savePrefix) return;
        $.each(g.saveParams, function(key, obj) {
          $.cookie(p.savePrefix + "_" + obj, null);
        });
        $.cookie(p.savePrefix + "_fields", null);
      },
        ...
        if (p.params)
          {
            for (var pi = 0; pi < p.params.length; pi++) param[param.length] = p.params[pi];
             ...
             error: function(data) {
              try { if (p.onError) p.onError(data); else g.addData(undefined);} catch (e) {};
            }
           });
      },
      doSearch: function () {
      ...
      for (i=0;i<p.buttons.length;i++)
        {
          ...
          if (!btn.separator)
          {
            ...
            if (btn.dclass)
              btnDiv.className = 'fbutton ' + btn.dclass;
            else
    ...
    if (p.searchitems)
      {
        ...
        $(g.sDiv).data("visible", false);
        $('.pSearch',g.pDiv).click(function(){
          if ($(g.sDiv).data("visible")) {
            $(g.sDiv).hide();
            $(g.sDiv).data("visible", false);
          } else {
            $(g.sDiv).show();
            $(g.sDiv).data("visible", true);
            $('input:first', g.sDiv).trigger('focus');
          }
          g.fixHeight();
        });        
        //add search box
        ...
        {
          if ((p.qtype=='' && sitems[s].isdefault==true) || (p.qtype == sitems[s].name))
          {
         ...
         function ()
          {
            ...
            if (kcol.style.display=='none') chk = '';
                                                ...
                                                $('tbody',g.nDiv).append('<td class="ndcol1"><input type="checkbox" '+ chk +' class="togCol" value="'+ cn +'" /></td><td class="ndcol2">'+this.innerHTML+'</td>');
            cn++;
          }
      );
  ...
  $.fn.flexHeight = function(h) { // function to resize grid

    return this.each( function() {
        this.grid.fixHeight(h);
      });
  ...
  $.fn.flexReset = function(p) { // function to reset grid
    return this.each( function() {
        if (this.grid && this.p.savePrefix) this.grid.resetState();
      });
  }; //end flexReset
  ...
  $.fn.flexOptions = function(p) { //function to update general options
    ...
    return this.each( function() {
        ...
        $.fn.flexColToggler = function(handler) {

                return this.each( function() {
                              if (this.grid) this.grid.colToggler(handler);
                        });
