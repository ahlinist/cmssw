...
{var n=this.colresize.n;var nw=this.colresize.nw;var at=$('th:visible div:eq('+n+')',this.hDiv).parent().attr('abbr');;$.each(p.colModel,function(k,o){if(o.name==at){o.width=nw;}});$('th:visible div:eq('+n+')',this.hDiv).css('width',nw);$('tr',this.bDiv).each(function()
...
$('body').css('cursor','default');$('body').noSelect(false);},toggleCol:function(cid,visible){var pmcol={};var ncol=$("th[axis='col"+cid+"']",this.hDiv)[0];var n=$('thead th',g.hDiv).index(ncol);var cb=$('input[value='+cid+']',g.nDiv)[0];$.each(p.colModel,function(k,o){if(o.name==$(ncol).attr("abbr")){pmcol=o;}});if(visible==null)
{visible=ncol.hide;}
if($('input:checked',g.nDiv).length<p.minColToggle&&!visible)return false;if(visible)
{ncol.hide=false;pmcol.hide=false;$(ncol).show();cb.checked=true;}
else
{ncol.hide=true;pmcol.hide=true;$(ncol).hide();cb.checked=false;}
$('tbody tr',t).each
(function()
{if(visible)
...
p.page=data.page;this.buildpager();g.saveState();var tbody=document.createElement('tbody');if(p.dataType=='json')
{$.each
...
(function()
{var td=document.createElement('td');td.align=this.align;td.setAttribute("abbr",$(this).attr("abbr"));td.setAttribute("field",$(this).attr("field"));td.innerHTML=row[$(this).attr("field")];$(tr).append(td);td=null;});if($('thead',this.gDiv).length<1)
{for(idx=0;idx<cell.length;idx++)
{var td=document.createElement('td');td.setAttribute("abbr",$(this).attr("abbr"));td.setAttribute("field",$(this).attr("field"));td.innerHTML=row[$(this).attr("field")];$(tr).append(td);td=null;}}
...
$('tr',t).unbind();$(t).empty();$(t).append(tbody);this.addCellProp();this.addRowProp();this.rePosDrag();tbody=null;data=null;i=null;if(p.onSuccess)p.onSuccess();if(p.hideOnSubmit)$(g.block).remove();this.hDiv.scrollLeft=this.bDiv.scrollLeft;if($.browser.opera)$(t).css('visibility','visible');if(p.postProcess)
p.postProcess();},saveParams:['sortname','sortdir','page','rp','query','qtype','height'],saveParamsInt:['page','rp','height'],saveState:function(){if(!p.savePrefix)return;$.each(g.saveParams,function(key,obj){$.cookie(p.savePrefix+"_"+obj,p[obj]);});var hidden="";$.each(p.colModel,function(key,obj){var h="0";if(obj.hide)
h="1";hidden+=obj.name+"/"+obj.width+"/"+h+"&";});$.cookie(p.savePrefix+"_fields",hidden);},restoreState:function(){if(!p.savePrefix)return;$.each(g.saveParams,function(key,obj){var x=$.cookie(p.savePrefix+"_"+obj);if(x)
p[obj]=x;});$.each(g.saveParamsInt,function(key,obj){p[obj]=parseInt(p[obj]);});var hidden=$.cookie(p.savePrefix+"_fields");if((hidden)&&(hidden!=null)){$.each(hidden.split("&"),function(key,obj){var sb=obj.split("/");if(sb.length==3){$.each(p.colModel,function(key2,obj2){if(obj2.name==sb[0]){obj2.width=parseInt(sb[1]);obj2.hide=(parseInt(sb[2])==1);}});}});}},resetState:function(){if(!p.savePrefix)return;$.each(g.saveParams,function(key,obj){$.cookie(p.savePrefix+"_"+obj,null);});$.cookie(p.savePrefix+"_fields",null);},changeSort:function(th){if(this.loading)return true;$(g.nDiv).hide();$(g.nBtn).hide();if(p.sortname==$(th).attr('abbr'))
...
$.ajax({type:p.method,url:p.url,data:param,dataType:p.dataType,success:function(data){g.addData(data);},error:function(data){try{if(p.onError)p.onError(data);else g.addData(undefined);}catch(e){};}});},doSearch:function(){p.query=$('input[name=q]',g.sDiv).val();p.qtype=$('select[name=qtype]',g.sDiv).val();p.newp=1;this.populate();},changePage:function(ctype){if(this.loading)return true;switch(ctype)
...
(function()
{$(this).dblclick(function(e){if(p.onRowDblClick){p.onRowDblClick(this);}}).click(function(e)
{var obj=(e.target||e.srcElement);if(obj.href||obj.type)return true;$(this).toggleClass('trSelected');if(p.singleSelect)$(this).siblings().removeClass('trSelected');if(p.onRowSelected)
p.onRowSelected(this);}).mousedown(function(e)
{if(e.shiftKey)
...
{$(this).hover(function(){$(this).addClass('trOver');},function(){$(this).removeClass('trOver');});}});},pager:0};g.restoreState();if(p.colModel)
...
$(th).attr('abbr',cm.name);if(cm.name)
...
{var btnDiv=document.createElement('div');if(btn.dclass)
btnDiv.className='fbutton '+btn.dclass;else
btnDiv.className='fbutton';btnDiv.innerHTML="<div><span>"+btn.name+"</span></div>";if(btn.bclass)
...
if(p.qtype=='')p.qtype=sitems[0].name;var query="";if(p.query)
query=p.query;$(g.sDiv).append("<div class='sDiv2'>Quick Search <input type='text' size='30' name='q' class='qsbox' value='"+query+"'/><select name='qtype'>"+sopt+"</select> <input type='button' value='Clear' /></div>");$('input[name=q],select[name=qtype]',g.sDiv).keydown(function(e){if(e.keyCode==13)g.doSearch()});$('input[value=Clear]',g.sDiv).click(function(){$('input[name=q]',g.sDiv).val('');p.query='';g.doSearch();});$(g.bDiv).after(g.sDiv);if(p.query)
$(g.sDiv).show();}}
